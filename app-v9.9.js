...
function bindScreeningConfigControls(){
 ...
 if(save)save.onclick=()=>{const ps=loadNamedScreeningProfiles(),name=$("screenProfileName")?.value.trim();if(!name){box("screenCfgResult","warn","保存する設定名を入力してください。");return}if(name==="標準設定"){box("screenCfgResult","warn","「標準設定」は固定のため上書きできません。");return}const hit=ps.findIndex(p=>p.name===name),item={name,config:readScreeningConfigUi()};let idx=hit;if(hit>=0)ps[hit]=item;else if(ps.length<3){ps.push(item);idx=ps.length-1}else{box("screenCfgResult","warn","保存できる設定は3件までです。不要な設定を削除してから保存してください。");return}try{saveNamedScreeningProfiles(ps)}catch(e){box("screenCfgResult","fail","設定を保存できませんでした。\n"+(e?.message||e));return}renderScreeningNamedProfiles(`user:${idx}`);if($("screenProfileName"))$("screenProfileName").value="";box("screenCfgResult","pass",`「${name}」を保存しました。`)};
