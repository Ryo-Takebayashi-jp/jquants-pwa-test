...
- Screening設定の保存動作を修正。名前付き設定を「保存」した時点で、その設定を現在のScreening設定として同時に適用するよう変更。
- 保存時に active config の読み戻し検証を追加し、母集団・候補・Factor系のインメモリキャッシュを破棄。
- 日々運用のScreening再実行は、保存済みの現在設定から母集団を再構築して5戦略を再計算する。
- 設定画面の説明を「保存と同時に適用」に合わせて明確化。
...
- 「中」は従来の標準Screeningと同じ重み。弱=0.6倍、強=1.4倍として各戦略内で再正規化するため、標準設定の計算結果は維持。
- 出遅れ再評価2軸、業績モメンタム2軸、決算後4軸、業績加速5軸、割安・リレーティング3軸の計16軸を調整可能。
- 過熱・需給・品質ミスマッチのPenaltyは安全補正として自動適用し、ユーザー設定対象外。
- 対象セクターは17業種へ一本化し、17業種を常時チェックボックス表示。全セクターと個別チェックを同期。
- 保存設定リスト先頭に削除不可の「標準設定」を常設。ユーザー設定は3件まで保存し、保存直後にリストへ反映。
- 各戦略の候補数は従来どおりTop20固定。

# v9.6 Screening compact preset UI
- 探索戦略の候補数UIを廃止し、各戦略Top20固定へ戻した。
- 条件は「緩め／標準／厳しめ」の1行プルダウンへ戻した。
- 選択中プリセットが実際に意味する閾値・ランキング差分を横に表示する。表示値は実装ロジックと一致させ、存在しない閾値は表示しない。


## v9.5-screening-ui-compact (2026-09-13)
- Screeningの保存設定UIを2段のコンパクト構成へ変更（設定名＋保存／保存済みリスト＋読込＋削除）。保存上限3件は維持。
- セクター選択を静的フォールバック一覧から必ず描画し、全セクターOFF時に17/33業種チェック一覧を表示する構造へ整理。
- 5探索戦略の数値入力・折りたたみを廃止し、「条件：緩め／標準／厳しめ」「候補数：10／20／30」の2軸プリセットへ変更。
- 標準は従来Screening互換。厳しめは戦略別の追加条件、緩めは出遅れ再評価・業績モメンタムで過熱Penaltyを緩和してランキング。
# v9.4 - 2026-09-13

- Screening設定を実際の指標条件ベースへ再設計。従来の「弱め / 標準 / 強め = Top10 / 20 / 30」を廃止。
- 5探索戦略ごとに、実際の評価に使う指標の追加しきい値を指定可能。空欄なら従来ロジックをそのまま使用。
- 各戦略の候補数（標準20）を詳細条件とは独立して設定可能。
- 「標準設定」は従来条件（全市場・全セクター・追加条件なし・各戦略Top20）として固定復帰可能。
- 名前付きScreening設定を3枠まで保存・読込可能。
- 17/33業種の一覧をアプリ側にも保持し、DataLake銘柄マスター取得前でもセクター選択UIを表示可能にした。
- Screening Shareの screening_profile.csv に候補数と詳細条件を記録。

# v9.2 — Screening母集団カスタム設定

- 現行Screening条件を「標準設定」として固定し、ワンタップで復元可能にした。
- 対象市場、17/33業種セクター、最低20日平均売買代金、最低株価、最低価格履歴をユーザー設定可能にした。
- 保存した条件からDataLake上のScreening母集団を毎回再構築する。5戦略の評価ロジックはv9.0から変更していない。
- Screening Shareへ `screening_profile.csv` を追加し、実行時の設定スナップショットを記録する。
- 設定・保守の「⑤ 一時診断（開発用）」を削除した。
