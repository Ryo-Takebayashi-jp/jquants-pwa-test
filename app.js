...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v7e-alpha109");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v7e-alpha109").catch(()=>{}));
...
// v7e-alpha109: daily-use direct Investment Tracking + production UI architecture cleanup.
...
async function quickInvRenderState(st){
 ...
 $("quickInvTable").innerHTML=rows.length?`<div class="inv-grid">${rows.map(x=>{const status=x.w?.InvestmentStatus||x.d?.InvestmentStatus||x.d?.AnalystDecision||"TRACK",watch=x.w?.Status||"-",ref=Number(x.w?.ReferencePrice),close=Number(x.p?.close),chg=Number.isFinite(ref)&&ref>0&&Number.isFinite(close)?(close/ref-1)*100:null,active=["Active","ReviewDue"].includes(String(watch));return `<div class="inv-item"><div class="inv-title">${quickInvEsc(x.code)}　${quickInvEsc(x.name||"銘柄名未取得")}</div><div class="watch-badges"><span class="watch-badge status">${quickInvEsc(status)}</span><span class="watch-badge ${active?'active':'inactive'}">${active?'● Watch Active':quickInvEsc(watch)}</span></div><div class="inv-meta">登録日: ${quickInvEsc(x.w?.WatchStartDate||x.d?.EpisodeStartDate||x.d?.DiscoveryDate||"-")}<br>登録基準価格: ${Number.isFinite(ref)?pfMoney(ref)+'円':'-'}<br>最新終値: ${Number.isFinite(close)?pfMoney(close)+'円':'価格なし'}${x.p?.date?` <span class="price-date">${quickInvEsc(x.p.date)}</span>`:''}<br>登録時比: <b class="${chg==null?'':(chg>=0?'pnl-plus':'pnl-minus')}">${chg==null?'-':pfPct(chg)}</b></div><div class="inv-actions">${active?`<button class="watch-remove quickInvCloseRow" data-code="${quickInvEsc(x.code)}" data-status="${quickInvEsc(status)}">追跡終了</button>`:''}</div></div>`}).join("")}</div>`:"<p class='desc'>登録データなし</p>";
 for(const b of document.querySelectorAll(".quickInvCloseRow"))b.onclick=async()=>{const code=b.dataset.code||"",status=b.dataset.status||"WATCH";if(status==="TRACK_ONLY"){box("quickInvResult","warn",`${code} はTRACK_ONLYです。Discovery履歴を物理削除しない設計のため、一覧からの即時削除対象外です。`);return}if(!confirm(`${code} をWatchlistから終了しますか？\nDiscovery履歴は削除せず保持します。`))return;$("quickInvCode").value=code;$("quickInvStatus").value=status;$("quickInvAction").value="CLOSE";quickInvSyncFields();try{await $("quickInvPreviewBtn").onclick();if(latestQuickInvestmentPreview?.commitReady)await $("quickInvCommitBtn").onclick()}catch(e){box("quickInvResult","fail","削除 FAIL\n"+(e?.message||e))}}
}
