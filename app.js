...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v9.3");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v9.3").catch(()=>{}));
...
let lastGapPlan=[];
// v9.3: exactly one canonical J-Quants API key per browser session.
...
// v9.3: simplified Settings / Maintenance + read-only OPFS diagnostics.
...
if($("userBackupZipBtn")) $("userBackupZipBtn").onclick=async()=>{
 ...
 try{
  ...
  const bytes=new Uint8Array(r.buffer),manifest={format:"JQ-USER-BACKUP-v1",appVersion:"v9.3",createdAt:new Date().toISOString(),db:"jq_private_v1.sqlite",bytes:r.bytes,sha256:r.sha256||null,quickCheck:priv.quickCheck,tables:priv.tables||[],includes:["portfolio","watchlist","discovery","investment-tracking","web-trade-ledger","void-audit","pipeline-checkpoints"],excludes:["J-Quants API key","market DataLake"]};
  const blob=zipStoreBlob([{name:"jq_private_v1.sqlite",data:bytes},{name:"manifest.json",data:JSON.stringify(manifest,null,2)},{name:"README.txt",data:"J-Quants User Data Backup v9.3\nAPI key is intentionally excluded.\nRestore only from the Settings / Maintenance page.\n"}]);
...
const SCREENING_CONFIG_KEY="jq_screening_config_v2";
...
const SCREENING_DEFAULT_STRATEGIES=Object.fromEntries(SCREENING_STRATEGY_NAMES.map(k=>[k,{enabled:true,strength:"standard"}]));
...
function screeningStrengthTopN(v){return v==="weak"?10:v==="strong"?30:20}
function screeningConfigLabel(c=loadScreeningConfig()){c=normalizeScreeningConfig(c);const ms=c.markets.length===3?"全市場":c.markets.join("・"),sec=c.sectors.length?`${c.sectorMode}業種 ${c.sectors.length}件`:"全セクター",jp={FundamentalPriceGap:"出遅れ",FundamentalMomentum:"業績モメンタム",PostEarningsDrift:"決算後",GrowthAcceleration:"業績加速",QualityValueReRating:"割安・再評価"},lv={weak:"弱",standard:"標準",strong:"強"};const on=SCREENING_STRATEGY_NAMES.filter(k=>c.strategies[k]?.enabled);return `${ms} / ${sec}
${on.map(k=>`${jp[k]}:${lv[c.strategies[k].strength]}`).join(" / ")||"探索戦略なし"}`}
let screeningSectorOptions={"17":[],"33":[]};
...
function readScreeningConfigUi(){const markets=[...document.querySelectorAll('#screenCfgMarkets input[type="checkbox"]:checked')].map(x=>x.value),sectorMode=document.querySelector('input[name="screenCfgSectorMode"]:checked')?.value||"17",all=$("screenCfgAllSectors")?.checked!==false,sectors=all?[]:[...document.querySelectorAll('#screenCfgSectorList input[type="checkbox"]:checked')].map(x=>x.value),strategies={};document.querySelectorAll('#screenCfgStrategies .screen-strategy-card').forEach(card=>{const k=card.dataset.strategy;strategies[k]={enabled:card.querySelector('.screen-strategy-enabled')?.checked!==false,strength:card.querySelector('.screen-strategy-strength:checked')?.value||"standard"}});return normalizeScreeningConfig({markets,sectorMode,sectors,strategies})}
function renderScreeningSectorChoices(cfg=loadScreeningConfig()){const host=$("screenCfgSectorList");if(!host)return;const mode=String(document.querySelector('input[name="screenCfgSectorMode"]:checked')?.value||cfg.sectorMode||"17"),opts=screeningSectorOptions[mode]||[],selected=new Set(cfg.sectorMode===mode?cfg.sectors:[]);host.innerHTML=opts.length?opts.map(v=>`<label><input type="checkbox" value="${escapeHtml(v)}" ${selected.has(v)?"checked":""}> ${escapeHtml(v)}</label>`).join(""):'<span class="desc">業種マスターがまだありません。先にDataLake / 銘柄マスターを構築してください。</span>';const all=$("screenCfgAllSectors")?.checked!==false;const controls=$("screenCfgSectorControls");if(controls)controls.style.display=all?"none":"block";host.querySelectorAll('input').forEach(x=>x.disabled=all)}
function applyScreeningConfigUi(cfg=loadScreeningConfig()){cfg=normalizeScreeningConfig(cfg);document.querySelectorAll('input[name="screenCfgSectorMode"]').forEach(x=>x.checked=x.value===cfg.sectorMode);document.querySelectorAll('#screenCfgMarkets input[type="checkbox"]').forEach(x=>x.checked=cfg.markets.includes(x.value));if($("screenCfgAllSectors"))$("screenCfgAllSectors").checked=!cfg.sectors.length;document.querySelectorAll('#screenCfgStrategies .screen-strategy-card').forEach(card=>{const k=card.dataset.strategy,z=cfg.strategies[k]||{enabled:true,strength:"standard"};const e=card.querySelector('.screen-strategy-enabled'),sts=[...card.querySelectorAll('.screen-strategy-strength')];if(e)e.checked=z.enabled;for(const st of sts){st.checked=st.value===z.strength;st.disabled=!z.enabled}card.style.opacity=z.enabled?"1":".55"});renderScreeningSectorChoices(cfg);if($("screenCfgResult"))box("screenCfgResult",screeningConfigSig(cfg)===screeningConfigSig(SCREENING_DEFAULT_CONFIG)?"pass":"warn",`${screeningConfigSig(cfg)===screeningConfigSig(SCREENING_DEFAULT_CONFIG)?"標準設定":"カスタム設定"}
${screeningConfigLabel(cfg)}`)}
async function initScreeningConfigUi(){if(!$("screenCfgResult"))return;let fromMaster=false;try{const r=await workerCall("screening-master-options",120000);screeningSectorOptions={"17":Array.isArray(r.sector17)&&r.sector17.length?r.sector17:[...SCREENING_SECTOR_FALLBACK["17"]],"33":Array.isArray(r.sector33)&&r.sector33.length?r.sector33:[...SCREENING_SECTOR_FALLBACK["33"]]};fromMaster=!!((r.sector17||[]).length||(r.sector33||[]).length)}catch(_){screeningSectorOptions={"17":[...SCREENING_SECTOR_FALLBACK["17"]],"33":[...SCREENING_SECTOR_FALLBACK["33"]]}}applyScreeningConfigUi(loadScreeningConfig());const host=$("screenCfgSectorList");if(host&&$("screenCfgAllSectors")?.checked===false&&host.querySelector(".desc")){renderScreeningSectorChoices(loadScreeningConfig())}}
...
document.querySelectorAll('#screenCfgStrategies .screen-strategy-enabled').forEach(x=>x.onchange=()=>{const card=x.closest('.screen-strategy-card'),sts=[...(card?.querySelectorAll('.screen-strategy-strength')||[])];for(const st of sts)st.disabled=!x.checked;if(card)card.style.opacity=x.checked?"1":".55"});
...
function buildScreeningStrategies(rows){
 ...
 const cfg=loadScreeningConfig(),map=new Map();
 for(const [name,key] of defs){
   ...
   const z=cfg.strategies?.[name]||{enabled:true,strength:"standard"};
   if(!z.enabled)continue;
   const ranked=eligible.slice(0,screeningStrengthTopN(z.strength));
...
if($("screeningStrategyBtn")) $("screeningStrategyBtn").onclick=async()=>{
 ...
 try{
   ...
   box("screeningStrategyResult","pass",`Web Screening 5戦略\n母集団: ${enriched.length}\n候補ユニーク: ${latestScreeningCandidates.length}\n${Object.entries(counts).map(([k,v])=>{const z=loadScreeningConfig().strategies?.[k]||{enabled:true,strength:"standard"};return k+": "+v+"/"+(z.enabled?screeningStrengthTopN(z.strength):0)}).join("\n")}\nReactionPending: ${pending}\n\nPEDイベント窓: PC版定義で計算\n次段階: PC版候補とのParity`);
...
// v9.3: temporary read-only storage inventory. Remove after incident analysis.
...
// v9.3: destructive local reset with bottom-up OPFS deletion and physical verification.
