...
if($("aiShareZipBtn")) $("aiShareZipBtn").onclick=async()=>{
 ...
 try{
  ...
  const headers=Object.keys(rows[0]),csv=simpleCsv(rows,headers),manifest={schema:"web-ai-share-manifest-v1",version:"v7e-alpha95",generatedAt:new Date().toISOString(),asOf,canonicalAsOfSource:canon.source,dataLakeDate:canon.dataLakeDate,lastPassPipelineDate:canon.passDate,portfolioCount:rows.length,coverage:{technical:`${tech}/${rows.length}`,financial:`${fin}/${rows.length}`,marginInterest:`${margin}/${rows.length}`,shortSelling:`${shorts}/${rows.length}`},files:["web_jqp.json","web_portfolio_integrated.csv","manifest.json","README.txt"],note:"Web-first AI share stage 1. Generation/download separated; generated artifact can be downloaded repeatedly without recomputation."};
...
// v7e-alpha95: daily-use direct Investment Tracking registration / update / Watchlist close UI.
...
function quickInvRenderState(st){latestQuickInvestmentState=st||{discovery:[],master:[],state:[]};const latestD=new Map();for(const r of st.discovery||[]){const c=normalizeInvestmentCode(r.Code),d=String(r.EpisodeStartDate||r.DiscoveryDate||"");if(!c)continue;const old=latestD.get(c);if(!old||String(old.EpisodeStartDate||old.DiscoveryDate||"")<d)latestD.set(c,r)}const active=(st.master||[]).filter(r=>["Active","ReviewDue"].includes(String(r.Status||""))),codes=[...new Set([...latestD.keys(),...active.map(r=>normalizeInvestmentCode(r.Code))])].sort(),wm=new Map(active.map(r=>[normalizeInvestmentCode(r.Code),r]));const rows=codes.map(c=>({code:c,d:latestD.get(c),w:wm.get(c)}));$("quickInvTable").innerHTML=rows.length?`<div style="overflow:auto"><table style="width:100%;font-size:12px"><tr><th>Code</th><th>Discovery</th><th>Watchlist</th><th>操作</th></tr>${rows.map(x=>`<tr><td>${quickInvEsc(x.code)}</td><td>${x.d?quickInvEsc((x.d.InvestmentStatus||x.d.AnalystDecision||"TRACK")+" "+(x.d.EpisodeStartDate||x.d.DiscoveryDate||"")):"-"}</td><td>${x.w?quickInvEsc((x.w.InvestmentStatus||"WATCH")+" "+(x.w.Status||"")):"-"}</td><td>${x.w?`<button class="secondary quickInvCloseRow" data-code="${quickInvEsc(x.code)}" data-status="${quickInvEsc(x.w.InvestmentStatus||"WATCH")}">終了</button>`:""}</td></tr>`).join("")}</table></div>`:"<p class='desc'>登録データなし</p>";for(const b of document.querySelectorAll(".quickInvCloseRow"))b.onclick=()=>{$("quickInvCode").value=b.dataset.code||"";$("quickInvStatus").value=b.dataset.status||"WATCH";$("quickInvAction").value="CLOSE";quickInvSyncFields();$("quickInvCode").scrollIntoView({behavior:"smooth",block:"center"})}}
async function quickInvRefresh(){const btn=$("quickInvRefreshBtn");if(btn)btn.disabled=true;try{const st=await workerCall("investment-tracking-state-load",120000);quickInvRenderState(st);const d=await quickInvCanonicalDate();if($("quickInvStartDate")&&!$("quickInvStartDate").value)$("quickInvStartDate").value=d;box("quickInvResult","pass",`現在の登録一覧を読込 PASS\nDiscovery Episodes: ${st.discoveryCount||0}\nWatchlist: ${st.masterCount||0}\n基準候補日: ${d}`)}catch(e){box("quickInvResult","fail","一覧読込 FAIL\n"+(e?.message||e))}finally{if(btn)btn.disabled=false}}
