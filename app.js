...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v7e-alpha88");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v7e-alpha88").catch(()=>{}));
...
// v7e-alpha88: Watchlist private-state migration + unified Investment Tracking routing audit + Web-first alert layer.
...
// v7e-alpha88: Web-first Watchlist Re-Evaluation Alert engine.
// Preview is deliberately non-destructive. Commit persists state + current alerts + de-duplicated alert history.
const WATCHLIST_ALERT_ENGINE_VERSION="WatchlistWebV1-alpha88";
...
const WATCHLIST_ALERT_DIAG_FIELDS=["WatchID","Code","Status","ScreeningRowFound","FactorKey","FactorDate","FactorEngineVersion","CurrentPrice","ReferencePrice","PriceChangePct","PriceAlertLevel","PrevPriceAlertLevel","CurrentPER","TargetPER","CurrentPBR","TargetPBR","DividendYield","TargetDividendYieldPct","FinancialChanged","PrevFinancialFingerprint","CurrentFinancialFingerprint","FactorState","PrevFactorState","SeasonState","SeasonalAlert","PrevSeasonalAlert","TechnicalState","PrevTechnicalState","DaysToEarnings","CatalystLevel","PrevCatalystLevel","ReviewExpiry","Expired","MigrationBaseline","SuppressedReason","AlertTypes","AlertCount"];
...
function wFinancialFingerprint(row){return [String(row?.LatestDisclosureDate||row?.LatestFinancialDisclosureDate||""),wPyFloatStr(row?.ForecastPrimaryProfit),"",wPyFloatStr(row?.ForecastEPS),wPyFloatStr(row?.ForecastAnnualDividend),wPyFloatStr(row?.PrimaryProfitProgressPct)].join("|")}
