...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v9.1");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v9.1").catch(()=>{}));
...
let lastGapPlan=[];
// v9.1: exactly one canonical J-Quants API key per browser session.
...
// v9.1: simplified Settings / Maintenance + read-only OPFS diagnostics.
...
if($("userBackupZipBtn")) $("userBackupZipBtn").onclick=async()=>{
 ...
 try{
  ...
  const bytes=new Uint8Array(r.buffer),manifest={format:"JQ-USER-BACKUP-v1",appVersion:"v9.1",createdAt:new Date().toISOString(),db:"jq_private_v1.sqlite",bytes:r.bytes,sha256:r.sha256||null,quickCheck:priv.quickCheck,tables:priv.tables||[],includes:["portfolio","watchlist","discovery","investment-tracking","web-trade-ledger","void-audit","pipeline-checkpoints"],excludes:["J-Quants API key","market DataLake"]};
  const blob=zipStoreBlob([{name:"jq_private_v1.sqlite",data:bytes},{name:"manifest.json",data:JSON.stringify(manifest,null,2)},{name:"README.txt",data:"J-Quants User Data Backup v9.1\nAPI key is intentionally excluded.\nRestore only from the Settings / Maintenance page.\n"}]);
...
const SCREENING_CONFIG_KEY="jq_screening_config_v1";
const SCREENING_DEFAULT_CONFIG={markets:["プライム","スタンダード","グロース"],minTradingValue:50000000,minPrice:100,minHistoryDays:60,sectorMode:"17",sectors:[]};
function normalizeScreeningConfig(x={}){const markets=Array.isArray(x.markets)?x.markets.filter(v=>SCREENING_DEFAULT_CONFIG.markets.includes(String(v))):[...SCREENING_DEFAULT_CONFIG.markets];return{markets:markets.length?markets:[...SCREENING_DEFAULT_CONFIG.markets],minTradingValue:Number.isFinite(Number(x.minTradingValue))?Math.max(0,Number(x.minTradingValue)):SCREENING_DEFAULT_CONFIG.minTradingValue,minPrice:Number.isFinite(Number(x.minPrice))?Math.max(0,Number(x.minPrice)):SCREENING_DEFAULT_CONFIG.minPrice,minHistoryDays:Number.isFinite(Number(x.minHistoryDays))?Math.max(1,Math.round(Number(x.minHistoryDays))):SCREENING_DEFAULT_CONFIG.minHistoryDays,sectorMode:String(x.sectorMode)==="33"?"33":"17",sectors:Array.isArray(x.sectors)?[...new Set(x.sectors.map(String).filter(Boolean))]:[]}}
...
function screeningConfigLabel(c=loadScreeningConfig()){c=normalizeScreeningConfig(c);const ms=c.markets.length===3?"全市場":c.markets.join("・"),sec=c.sectors.length?`${c.sectorMode}業種 ${c.sectors.length}件`:"全セクター";return `${ms} / ${sec} / 売買代金 ${(c.minTradingValue/10000).toLocaleString("ja-JP")}万円以上 / 株価 ${c.minPrice.toLocaleString("ja-JP")}円以上 / 履歴 ${c.minHistoryDays}日以上`}
let screeningSectorOptions={"17":[],"33":[]};
function readScreeningConfigUi(){const markets=[...document.querySelectorAll('#screenCfgMarkets input[type="checkbox"]:checked')].map(x=>x.value),sectorMode=$("screenCfgSectorMode")?.value||"17",all=$("screenCfgAllSectors")?.checked!==false,sectors=all?[]:[...document.querySelectorAll('#screenCfgSectorList input[type="checkbox"]:checked')].map(x=>x.value);return normalizeScreeningConfig({markets,minTradingValue:Number($("screenCfgTradingValueMan")?.value||5000)*10000,minPrice:Number($("screenCfgMinPrice")?.value||100),minHistoryDays:Number($("screenCfgHistoryDays")?.value||60),sectorMode,sectors})}
...
function applyScreeningConfigUi(cfg=loadScreeningConfig()){cfg=normalizeScreeningConfig(cfg);if($("screenCfgTradingValueMan"))$("screenCfgTradingValueMan").value=cfg.minTradingValue/10000;if($("screenCfgMinPrice"))$("screenCfgMinPrice").value=cfg.minPrice;if($("screenCfgHistoryDays"))$("screenCfgHistoryDays").value=cfg.minHistoryDays;if($("screenCfgSectorMode"))$("screenCfgSectorMode").value=cfg.sectorMode;document.querySelectorAll('#screenCfgMarkets input[type="checkbox"]').forEach(x=>x.checked=cfg.markets.includes(x.value));if($("screenCfgAllSectors"))$("screenCfgAllSectors").checked=!cfg.sectors.length;renderScreeningSectorChoices(cfg);if($("screenCfgResult"))box("screenCfgResult",screeningConfigSig(cfg)===screeningConfigSig(SCREENING_DEFAULT_CONFIG)?"pass":"warn",`${screeningConfigSig(cfg)===screeningConfigSig(SCREENING_DEFAULT_CONFIG)?"標準設定":"カスタム設定"}\n${screeningConfigLabel(cfg)}`)}
...
if($("screenCfgSaveBtn"))$("screenCfgSaveBtn").onclick=()=>{const c=readScreeningConfigUi();if(!c.markets.length){box("screenCfgResult","warn","対象市場を1つ以上選択してください。");return}if(!$("screenCfgAllSectors").checked&&!c.sectors.length){box("screenCfgResult","warn","全セクターをOFFにする場合は、対象セクターを1つ以上選択してください。");return}localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(c));latestScreeningBaseRows=[];latestScreeningBaseAsOf="";latestScreeningBaseProfileSig="";applyScreeningConfigUi(c);box("screenCfgResult","pass",`設定を保存しました。次回Screeningから適用します。\n${screeningConfigLabel(c)}`)};
...
// v9.1: temporary read-only storage inventory. Remove after incident analysis.
...
// v9.1: destructive local reset with bottom-up OPFS deletion and physical verification.
