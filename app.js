...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v7e-alpha81");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v7e-alpha81").catch(()=>{}));
...
// v7e-alpha81: advance the local DataLake one trading day with one operation, with current-day optional repair.
...
// v7e-alpha81: Watchlist private-state migration + unified Investment Tracking routing audit.
...
// v7e-alpha81: Factor Monitor + Sector Seasonality independent parity.
...
let latestFactorWebRows=[],latestFactorDiffRows=[],latestFactorSeasonalityProfile=[],latestFactorSummaryRows=[];
...
function buildFactorSummaryWeb(rows){const ranked=phase=>rows.filter(r=>r.Phase===phase).sort((a,b)=>{const av=phase==="Emerging"||phase==="Improving"?-(a.Acceleration||-1):-(a.Strength||-1),bv=phase==="Emerging"||phase==="Improving"?-(b.Acceleration||-1):-(b.Strength||-1);return av-bv||String(a.FactorKey)<String(b.FactorKey)?-1:String(a.FactorKey)>String(b.FactorKey)?1:0}).slice(0,5),out=[],add=(bucket,arr)=>arr.forEach((r,i)=>out.push(Object.fromEntries(FACTOR_SUMMARY_FIELDS.map(f=>[f,f==="Bucket"?bucket:f==="Rank"?i+1:r[f]??""]))));add("TopEmerging",ranked("Emerging"));add("TopStrong",ranked("Strong"));add("TopFading",ranked("Fading"));const alerts=rows.filter(r=>r.Alert).sort((a,b)=>-(a.Acceleration||-1)-(-(b.Acceleration||-1))||-(a.FlowProxy||-1)-(-(b.FlowProxy||-1))||String(a.FactorKey)<String(b.FactorKey)?-1:String(a.FactorKey)>String(b.FactorKey)?1:0).slice(0,10);add("Alert",alerts);return out}
