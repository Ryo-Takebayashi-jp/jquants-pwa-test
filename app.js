...
async function quickInvRenderState(st){
 ...
 const active=(st.master||[]).filter(r=>["Active","ReviewDue"].includes(String(r.Status||""))),codes=[...new Set([...latestD.keys(),...active.map(r=>normalizeInvestmentCode(r.Code))])].sort(),wm=new Map(active.map(r=>[normalizeInvestmentCode(r.Code),r]));
 let names=new Map();try{const nm=await workerCall("equities-master-names",120000,null,null,{codes});for(const r of nm.rows||[])names.set(normalizeInvestmentCode(r.code),r.company_name||"")}catch(_){}
 let prices=new Map();try{const asOf=await quickInvCanonicalDate(),pr=await workerCall("portfolio-latest-prices",300000,null,null,{asOf,codes});prices=new Map((pr.rows||[]).map(r=>[normalizeInvestmentCode(r.code),r]))}catch(_){}
 const rows=codes.map(c=>({code:c,name:names.get(c)||"",d:latestD.get(c),w:wm.get(c),p:prices.get(c)||{}}));
 $("quickInvTable").innerHTML=rows.length?`<div class="inv-grid">${rows.map(x=>{const status=x.w?.InvestmentStatus||x.d?.InvestmentStatus||x.d?.AnalystDecision||"TRACK",watch=x.w?.Status||"-",watchActive=["Active","ReviewDue"].includes(String(watch)),dClosed=String(x.d?.PerfEpisodeStatus||"").toLowerCase()==="closed"||String(x.d?.PerfActiveWatchFlag||"")==="0",discoveryActive=!!x.d&&!dClosed,refW=Number(x.w?.ReferencePrice),refD=Number(x.d?.InitialPrice),ref=Number.isFinite(refW)&&refW>0?refW:(Number.isFinite(refD)&&refD>0?refD:NaN),close=Number(x.p?.close),chg=Number.isFinite(ref)&&ref>0&&Number.isFinite(close)?(close/ref-1)*100:null;return `<div class="inv-item"><div class="inv-title">${quickInvEsc(x.code)}　${quickInvEsc(x.name||"銘柄名未取得")}</div><div class="watch-badges"><span class="watch-badge status">${quickInvEsc(status)}</span>${x.d?`<span class="watch-badge ${discoveryActive?'active':'inactive'}">${discoveryActive?'● Discovery Active':'Discovery Closed'}</span>`:''}${x.w?`<span class="watch-badge ${watchActive?'active':'inactive'}">${watchActive?'● Watch Active':quickInvEsc(watch)}</span>`:''}</div><div class="inv-meta">登録日: ${quickInvEsc(x.w?.WatchStartDate||x.d?.EpisodeStartDate||x.d?.DiscoveryDate||"-")}<br>登録基準価格: ${Number.isFinite(ref)?pfMoney(ref)+'円':'未記録'}<br>最新終値: ${Number.isFinite(close)?pfMoney(close)+'円':'価格なし'}${x.p?.date?` <span class="price-date">${quickInvEsc(x.p.date)}</span>`:''}<br>登録時比: <b class="${chg==null?'':(chg>=0?'pnl-plus':'pnl-minus')}">${chg==null?'-':pfPct(chg)}</b></div><div class="inv-actions">${watchActive?`<button class="watch-remove quickInvCloseScope" data-code="${quickInvEsc(x.code)}" data-scope="watch">Watch終了</button>`:''}${discoveryActive?`<button class="watch-remove quickInvCloseScope" data-code="${quickInvEsc(x.code)}" data-scope="discovery">Discovery終了</button>`:''}</div></div>`}).join("")}</div>`:"<p class='desc'>登録データなし</p>";
 for(const b of document.querySelectorAll(".quickInvCloseScope"))b.onclick=async()=>{const code=b.dataset.code||"",scope=b.dataset.scope||"watch",label=scope==="discovery"?"Discovery Episode":"Watchlist";if(!confirm(`${code} の${label}追跡を終了しますか？\n履歴は削除せず監査用に保持します。`))return;try{const asOf=await quickInvCanonicalDate(),r=await workerCall("investment-tracking-close-code",120000,null,null,{asOf,code,scope});box("quickInvResult","pass",`追跡終了 PASS\nCode: ${code}\n対象: ${label}\nDiscovery終了: ${r.discoveryClosed||0} / Watch終了: ${r.watchClosed||0}\n\n【日本語まとめ】\n過去の履歴は削除せず、今後の追跡だけ終了しました。`);await quickInvRefresh()}catch(e){box("quickInvResult","fail",`追跡終了 FAIL\n${e?.message||e}\n\n【日本語まとめ】\n追跡終了は保存されていません。表示された技術情報をそのままアイへ送ってください。`)}}
}
...
let latestQuickCsvPreview=null,latestQuickCsvRoutes={discovery:[],watchlist:[]};
if($("quickInvImportPreviewBtn"))$("quickInvImportPreviewBtn").onclick=async()=>{const f=$("quickInvImportFile")?.files?.[0];if(!f){box("quickInvImportResult","warn","investment_tracking_input.csv を選択してください。");return}const btn=$("quickInvImportPreviewBtn");btn.disabled=true;$("quickInvImportCommitBtn").disabled=true;latestQuickCsvPreview=null;try{const rows=parseCsv(await f.text()).rows.filter(r=>Object.values(r).some(v=>String(v??"").trim())),audit=auditInvestmentTracking(rows);if(audit.errors.length){box("quickInvImportResult","fail",[`CSV検証 FAIL`,`入力: ${rows.length}`,`Warning: ${audit.warnings.length} / Error: ${audit.errors.length}`,...audit.warnings.map(x=>"WARN: "+x),...audit.errors.map(x=>"ERROR: "+x)].join("\n"));return}const asOf=await quickInvCanonicalDate();latestQuickCsvRoutes={discovery:audit.discovery,watchlist:audit.watchlist};const r=await workerCall("investment-tracking-preview",300000,s=>box("quickInvImportResult","run",`CSV PREVIEW計算中…\n${s.stage||""}`),null,{asOf,discovery:audit.discovery,watchlist:audit.watchlist});latestQuickCsvPreview=r;$("quickInvImportCommitBtn").disabled=!r.commitReady;box("quickInvImportResult",r.errorCount?"fail":r.warningCount?"warn":"pass",[`CSV PREVIEW ${r.commitReady!==false?"READY":"BLOCKED"}`,`入力: ${rows.length}`,`基準日: ${r.asOf||"-"}`,`Discovery: CREATE ${r.discoveryCreate||0} / UPDATE ${r.discoveryUpdate||0}`,`Watchlist: CREATE ${r.watchCreate||0} / UPDATE ${r.watchUpdate||0} / CLOSE ${r.watchClose||0} / REREGISTER ${r.watchReregister||0}`,`ReferencePrice解決: ${r.referenceResolved||0}`,`Warning: ${r.warningCount||0} / Error: ${r.errorCount||0}`,...(r.warnings||[]).map(x=>"WARN: "+x),...(r.errors||[]).map(x=>"ERROR: "+x),"","【日本語まとめ】",r.errorCount?"処理を止めています。ERROR欄の内容を確認してください。":r.warningCount?`CSVは正常に読めています。新規 ${r.watchCreate||0}件、既存更新 ${r.watchUpdate||0}件として処理します。Warningは続行可能な案内です。`:"CSVは正常です。このまま確定できます。","※まだ未保存です。内容確認後に一括確定してください。"].join("\n"))}catch(e){box("quickInvImportResult","fail","CSV PREVIEW FAIL\n"+(e?.message||e))}finally{btn.disabled=false}};
if($("quickInvImportCommitBtn"))$("quickInvImportCommitBtn").onclick=async()=>{const p=latestQuickCsvPreview;if(!p?.commitReady)return;const btn=$("quickInvImportCommitBtn");btn.disabled=true;try{const r=await workerCall("investment-tracking-commit",300000,null,null,{asOf:p.asOf,discovery:latestQuickCsvRoutes.discovery,watchlist:latestQuickCsvRoutes.watchlist});let recalc="";if((r.discoveryCreate||0)+(r.discoveryUpdate||0)>0){try{const dr=await workerCall("discovery-recalc",600000,s=>box("quickInvImportResult","run",`CSV COMMIT済み。Discovery再計算中…\n${s.stage||""}`),null,{asOf:r.asOf});latestDiscoveryWebRows=dr.rows||[];recalc=`\nDiscovery再計算: ${dr.count||latestDiscoveryWebRows.length} Episodes`}catch(e){recalc=`\nDiscovery再計算は要再実行: ${e?.message||e}`}}box("quickInvImportResult","pass",[`CSV COMMIT PASS`,`基準日: ${r.asOf||"-"}`,`Discovery: CREATE ${r.discoveryCreate||0} / UPDATE ${r.discoveryUpdate||0}`,`Watchlist: CREATE ${r.watchCreate||0} / UPDATE ${r.watchUpdate||0} / CLOSE ${r.watchClose||0} / REREGISTER ${r.watchReregister||0}`,`ImportID: ${r.importId||"-"}${recalc}`,"","Discovery履歴は保持されます。"].join("\n"));latestQuickCsvPreview=null;await quickInvRefresh()}catch(e){box("quickInvImportResult","fail","CSV COMMIT FAIL\n"+(e?.message||e));btn.disabled=false}};
quickInvSyncFields();
