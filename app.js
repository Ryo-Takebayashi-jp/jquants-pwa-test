...
function renderScreeningStrategyAxes(cfg=loadScreeningConfig()){cfg=normalizeScreeningConfig(cfg);document.querySelectorAll('#screenCfgStrategies .screen-strategy-card').forEach(card=>{const k=card.dataset.strategy,z=cfg.strategies[k],host=card.querySelector('.screen-axis-list');if(!host)return;host.innerHTML=(SCREENING_AXIS_DEFS[k]||[]).map(a=>`<div class="screen-axis-row"><div class="screen-axis-name"><b>${escapeHtml(a.label)}</b><small>${escapeHtml(a.desc)}</small></div><div class="screen-axis-choice">${[["weak","弱"],["medium","中"],["strong","強"]].map(([v,l])=>`<label><input type="radio" name="screenAxis_${k}_${a.key}" data-axis="${a.key}" value="${v}" ${z.axes[a.key]===v?"checked":""}><span>${l}</span></label>`).join("")}</div></div>`).join("");const e=card.querySelector('.screen-strategy-enabled');if(e)e.checked=z.enabled;card.classList.toggle('disabled',!z.enabled)})}
...
function renderScreeningSectorChoices(cfg=loadScreeningConfig()){cfg=normalizeScreeningConfig(cfg);const host=$("screenCfgSectorList");if(!host)return;const selected=new Set(cfg.sectors.length?cfg.sectors:SCREENING_SECTORS_17);host.innerHTML=SCREENING_SECTORS_17.map(v=>`<label><input type="checkbox" value="${escapeHtml(v)}" ${selected.has(v)?"checked":""}> ${escapeHtml(v)}</label>`).join("");if($("screenCfgAllSectors"))$("screenCfgAllSectors").checked=selected.size===SCREENING_SECTORS_17.length;host.querySelectorAll('input').forEach(x=>x.onchange=()=>{const n=host.querySelectorAll('input:checked').length;if($("screenCfgAllSectors"))$("screenCfgAllSectors").checked=n===SCREENING_SECTORS_17.length})}
