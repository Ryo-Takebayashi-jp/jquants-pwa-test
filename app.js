...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v9.4");
 jqSqliteWorker=w;
 w.onmessage=e=>{
   ...
   d.ok?p.resolve(d):p.reject(new Error(
     `[${d.stage||"worker"}] ${d.message||d.error||"Worker失敗"}`+
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v9.4").catch(()=>{}));
...
async function jqFetchEarningsDateByPubDate(date,token){
 return jqFetchV2Rows("/fins/earnings-date",{date:String(date||"").replaceAll("-","")},token);
}
async function jqValidateCanonicalApiKey(token){
  const t=String(token||"").trim();
  if(!t)throw new Error("APIキーを入力してください");
  // Cheap 1-day request. A valid key succeeds even when the date itself has no rows.
  const d=jstTodayIso();
  try{
    const r=await jqFetchV2Rows("/markets/calendar",{from:d.replaceAll("-",""),to:d.replaceAll("-","")},t);
    return {ok:true,date:d,endpoint:r.endpoint,rows:r.rows.length};
  }catch(e){
    const m=String(e?.message||e);
    if(/HTTP\s+401|HTTP\s+403/i.test(m))throw new Error(`APIキー確認FAIL: 入力中の共通APIキーがJ-Quantsに拒否されました。\n${m}`);
    throw new Error(`APIキー疎通確認FAIL: ${m}`);
  }
}
...
let lastGapPlan=[];
// v9.4: exactly one canonical J-Quants API key per browser session.
...
const JQ_LEGACY_TOKEN_MIRROR_IDS=["masterToken","prodToken","jqToken","shardApiToken","prodDailyToken","gapToken"];
function jqRenderTokenStatus(){
  const ok=!!sessionJqToken;
  if($("globalTokenStatus"))box("globalTokenStatus",ok?"pass":"warn",ok?"APIキー入力済み（全機能共通・セッションのみ）":"未入力");
  if($("dataPeriodTokenStatus"))box("dataPeriodTokenStatus",ok?"pass":"warn",ok?"APIキー入力済み（全機能共通・セッションのみ）":"未入力");
}
function jqSetCanonicalToken(value){
  sessionJqToken=String(value||"").trim();
  for(const id of JQ_CANONICAL_TOKEN_INPUT_IDS){const el=$(id);if(el&&el.value!==sessionJqToken)el.value=sessionJqToken}
  // Legacy functions may still read their old input directly. Mirror the canonical value into them, never the reverse.
  for(const id of JQ_LEGACY_TOKEN_MIRROR_IDS){const el=$(id);if(el)el.value=sessionJqToken}
  jqRenderTokenStatus();
  return sessionJqToken;
}
function prodTokenValue(preferredId=""){
  if(preferredId){const v=$(preferredId)?.value?.trim?.()||"";if(v!==sessionJqToken)return jqSetCanonicalToken(v)}
  if(sessionJqToken)return sessionJqToken;
  for(const id of JQ_CANONICAL_TOKEN_INPUT_IDS){const v=$(id)?.value?.trim?.()||"";if(v)return jqSetCanonicalToken(v)}
  return "";
}
function bindSessionTokenInputs(){
  for(const id of JQ_CANONICAL_TOKEN_INPUT_IDS){
    const el=$(id); if(!el||el.dataset.tokenBound==="1") continue;
    el.dataset.tokenBound="1";
    const sync=()=>jqSetCanonicalToken(el.value);
...
// v9.4: simplified Settings / Maintenance + read-only OPFS diagnostics.
...
async function jqPhysicalOpfsInventory(){
  ...
  const poolDirName=".jq-sahpool-v7c-r5";
  let dir=null;
  try{dir=await root.getDirectoryHandle(poolDirName,{create:false})}catch(e){return {origin:location.origin,poolDirName,dirFound:false,files:[],physicalBytes:0,usage:est.usage||0,quota:est.quota||0,persisted:await navigator.storage.persisted?.()}}
  const files=[];
  try{
    for await(const [name,h] of dir.entries()){
      if(h.kind==="file"){
        try{const f=await h.getFile();files.push({name,kind:"file",size:f.size,lastModified:f.lastModified||0})}catch(e){files.push({name,kind:"file",size:null,error:String(e?.message||e)})}
      }else files.push({name,kind:h.kind||"directory",size:null});
    }
  }catch(e){
    if(typeof dir.values==="function")for await(const h of dir.values()){const name=h.name||"(unknown)";if(h.kind==="file"){try{const f=await h.getFile();files.push({name,kind:"file",size:f.size,lastModified:f.lastModified||0})}catch(er){files.push({name,kind:"file",size:null,error:String(er?.message||er)})}}else files.push({name,kind:h.kind||"directory",size:null})}
    else throw e;
  }
  files.sort((a,b)=>String(a.name).localeCompare(String(b.name)));
  return {origin:location.origin,poolDirName,dirFound:true,files,physicalBytes:files.reduce((s,x)=>s+(Number(x.size)||0),0),usage:est.usage||0,quota:est.quota||0,persisted:await navigator.storage.persisted?.()};
}
if($("storageOriginText"))$("storageOriginText").textContent=location.origin;
if($("opfsPhysicalDiagBtn"))$("opfsPhysicalDiagBtn").onclick=async()=>{
  box("opfsPhysicalDiagResult","run","物理OPFSをread-onlyで確認中…SQLite DBは開きません。");
  try{
    const r=await jqPhysicalOpfsInventory(),free=Math.max(0,Number(r.quota||0)-Number(r.usage||0)),pct=r.quota?Number(r.usage||0)/Number(r.quota)*100:null;
    const detail=r.files.slice(0,64).map((x,i)=>`${i+1}. ${x.name} / ${x.kind}${x.size!=null?` / ${jqFmtBytes(x.size)}`:""}`).join("\n");
    box("opfsPhysicalDiagResult",r.dirFound?"pass":"warn",`${r.dirFound?"PASS":"要確認"}\nOrigin: ${r.origin}\nPool directory: ${r.poolDirName} / ${r.dirFound?"FOUND":"NOT FOUND"}\n物理エントリ: ${r.files.length}\nPool物理サイズ: ${jqFmtBytes(r.physicalBytes)}\nOrigin使用量: ${jqFmtBytes(r.usage)}\nQuota: ${jqFmtBytes(r.quota)}${pct!=null?`\n使用率: ${pct.toFixed(1)}% / 空き ${jqFmtBytes(free)}`:""}\nPersistent storage: ${r.persisted===true?"YES":r.persisted===false?"NO":"不明"}\n\n${detail||"（物理エントリなし）"}\n\n※この診断はread-onlyです。`);
  }catch(e){box("opfsPhysicalDiagResult","fail","FAIL\n"+(e?.message||e))}
};

let jqDataPeriodPlan=null;
function jqCompactRanges(dates){const a=[...new Set((dates||[]).filter(Boolean))].sort();if(!a.length)return[];const out=[];let s=a[0],p=a[0];for(let i=1;i<a.length;i++){const d=a[i];if(isoAddDays(p,1)===d){p=d;continue}out.push(s===p?s:`${s} ～ ${p}`);s=p=d}out.push(s===p?s:`${s} ～ ${p}`);return out}
async function jqRunDataPeriodCheck(){
  const from=$("dataPeriodFrom")?.value,to=$("dataPeriodTo")?.value;if(!from||!to||from>to)throw new Error("取得開始日・終了日を確認してください");
  const r=await workerCall("data-period-audit",600000,null,null,{from,to});jqDataPeriodPlan=r;
  const lines=[`指定期間: ${from} ～ ${to}`];let totalMissing=0;
  for(const x of r.datasets||[]){const n=Number(x.missingCount||0);totalMissing+=n;lines.push(`${x.label}: ${x.status}${x.coverageStart||x.coverageEnd?` / 収録 ${x.coverageStart||"-"} ～ ${x.coverageEnd||"-"}`:""}${n?` / 不足候補 ${n}`:""}${x.note?` / ${x.note}`:""}`);if(n&&x.missingDates?.length){const rg=jqCompactRanges(x.missingDates).slice(0,8);if(rg.length)lines.push("  "+rg.join("\n  ")+(jqCompactRanges(x.missingDates).length>8?"\n  …":""))}}
  const cls=totalMissing?"warn":"pass";box("dataPeriodCheckResult",cls,`${totalMissing?"要補完":"PASS"}\n${lines.join("\n")}\n\n不足候補合計: ${totalMissing.toLocaleString()}\n※休場日や公表0件日を含む場合があります。一括取り込み側は0件日も取得済みとして記録します。`);
  $("dataPeriodImportBtn").disabled=false;return r;
}
function jstYearsAgoIso(years){const t=jstTodayIso(),[y,m,d]=t.split("-").map(Number),ty=y-Number(years||0),last=new Date(Date.UTC(ty,m,0)).getUTCDate();return `${String(ty).padStart(4,"0")}-${String(m).padStart(2,"0")}-${String(Math.min(d,last)).padStart(2,"0")}`}
if($("dataPeriodFrom"))$("dataPeriodFrom").value=jstYearsAgoIso(5);
if($("dataPeriodTo"))$("dataPeriodTo").value=jstTodayIso();
if($("dataPeriodCheckBtn"))$("dataPeriodCheckBtn").onclick=async()=>{const b=$("dataPeriodCheckBtn");b.disabled=true;$("dataPeriodImportBtn").disabled=true;try{await jqRunDataPeriodCheck()}catch(e){box("dataPeriodCheckResult","fail","FAIL\n"+(e?.message||e))}finally{b.disabled=false}};

async function jqDataPeriodImportAll(){
  // The Settings input is the explicit source for this action. Sync it into the one session token before any write/open work.
  const token=prodTokenValue("simpleGapToken");if(!token)throw new Error("APIキーを入力してください");
  const from=$("dataPeriodFrom").value,to=$("dataPeriodTo").value;
  const lines=[],started=performance.now();const report=(msg)=>box("dataPeriodImportResult","run",`${msg}\n\n${lines.join("\n")}\n\n経過 ${((performance.now()-started)/60000).toFixed(1)}分`);
  report("APIキーを事前確認中…");
  const auth=await jqValidateCanonicalApiKey(token);
  lines.push(`APIキー確認: PASS (${auth.endpoint})`);
  let plan=jqDataPeriodPlan;if(!plan||plan.from!==from||plan.to!==to)plan=await workerCall("data-period-audit",600000,null,null,{from,to});
  const byKey=new Map((plan.datasets||[]).map(x=>[x.key,x]));
  // Fresh-install safe bars builder: write directly to canonical year shards.
  // This path must not require a pre-existing legacy market DB.
  const barsPlan=byKey.get("bars_daily");
  if(barsPlan?.missingDates?.length){
    let done=0,real=0;const touchedYears=new Set();
    for(const d of barsPlan.missingDates){
      report(`日足 ${d} (${done+1}/${barsPlan.missingDates.length})`);
      const got=await jqFetchDaily(d,token);
      await workerCall("shard-write-api-date",300000,null,null,{date:d,rows:got.rows});
      touchedYears.add(Number(d.slice(0,4)));done++;if(got.rows.length)real++;
    }
    for(const y of [...touchedYears].sort((a,b)=>a-b)){report(`日足 ${y}年Shardを確定中…`);await workerCall("shard-finalize-api-year",300000,null,null,{year:y});}
    lines.push(`日足: ${done}日確認 / 実データ ${real}日 / ${touchedYears.size}年Shard確定`);
  }else lines.push("日足: 補完不要");
  // Master snapshot: one current snapshot is sufficient for names/sectors.
  report("銘柄マスターを確認中…");try{const g=await jqFetchEquitiesMaster(to,token);const w=await workerCall("equities-master-write",300000,null,null,{date:to,rows:g.rows});lines.push(`銘柄マスター: ${g.rows.length.toLocaleString()} rows`)}catch(e){lines.push(`銘柄マスター: NG ${e?.message||e}`)}
  const finsPlan=byKey.get("fins_summary");if(finsPlan?.missingDates?.length){let done=0,rows=0;for(const d of finsPlan.missingDates){report(`財務サマリー ${d} (${done+1}/${finsPlan.missingDates.length})`);const g=await jqFetchFinsSummary(d,token);await workerCall("fins-summary-write",300000,null,null,{date:d,rows:g.rows,replaceDate:true});done++;rows+=g.rows.length;if(done%10===0)await sleep(150)}lines.push(`財務サマリー: ${done}日 / ${rows.toLocaleString()} rows`)}else lines.push("財務サマリー: 補完不要");
  const erPlan=byKey.get("earnings_date");if(erPlan?.missingDates?.length){let done=0,rows=0;for(const d of erPlan.missingDates){report(`決算予定日 ${d} (${done+1}/${erPlan.missingDates.length})`);const g=await jqFetchEarningsDateByPubDate(d,token);await workerCall("earnings-date-write",300000,null,null,{rows:g.rows,queryDate:d});done++;rows+=g.rows.length;if(done%20===0)await sleep(120)}lines.push(`決算予定日: ${done}日 / ${rows.toLocaleString()} rows`)}else lines.push("決算予定日: 補完不要");
  // Range-friendly market base datasets.
  report("TOPIXを確認・補完中…");try{const g=await jqFetchTopix(from,to,token);const cov=isoWeekdays(from,to);const w=await workerCall("topix-write",300000,null,null,{from,to,rows:g.rows,coverageDates:cov});lines.push(`TOPIX: ${g.rows.length.toLocaleString()} rows`)}catch(e){lines.push(`TOPIX: NG ${e?.message||e}`)}
  report("営業日カレンダーを確認・補完中…");try{const g=await jqFetchMarketCalendar(from,to,token);const cov=isoDays(from,to);const w=await workerCall("market-calendar-write",300000,null,null,{from,to,rows:g.rows,coverageDates:cov});lines.push(`営業日カレンダー: ${g.rows.length.toLocaleString()} rows`)}catch(e){lines.push(`営業日カレンダー: NG ${e?.message||e}`)}
  const jobs=[
    ["信用取引週末残高",jqFetchMarginInterest,"margin-interest-write"],["日々公表信用",jqFetchMarginAlert,"margin-alert-write"],["空売り比率",jqFetchShortRatio,"short-ratio-write"],["空売り報告",jqFetchShortSaleReport,"short-sale-report-write"],["投資部門別",jqFetchInvestorTypes,"investor-types-write"]
  ];
  for(const [label,fetcher,cmd] of jobs){report(`${label}を確認・補完中…`);try{let rows=0,calls=0;for(let s=from;s<=to;){const e0=isoAddDays(s,119),e=e0>to?to:e0;const r=await fetchAndPersistSupplyRange(fetcher,cmd,s,e,token,(done,total,n)=>report(`${label} ${s}～${e} / ${done}/${total} / rows ${n}`));rows+=r.got.rows.length;calls+=Number(r.got.calls||0);s=isoAddDays(e,1)}lines.push(`${label}: API ${calls}回 / ${rows.toLocaleString()} rows`)}catch(e){lines.push(`${label}: NG ${e?.message||e}`)}}
  try{await workerCall("supply-demand-normalize",180000)}catch(_){ }
  const after=await workerCall("data-period-audit",600000,null,null,{from,to});jqDataPeriodPlan=after;const rem=(after.datasets||[]).reduce((s,x)=>s+Number(x.missingCount||0),0);box("dataPeriodImportResult",rem?"warn":"pass",`${rem?"主要取得完了 / 残候補あり":"PASS"}\n${lines.join("\n")}\n\n再チェック不足候補: ${rem.toLocaleString()}\n処理時間: ${((performance.now()-started)/60000).toFixed(1)}分`);await jqRunDataPeriodCheck();
}
if($("dataPeriodImportBtn"))$("dataPeriodImportBtn").onclick=async()=>{const b=$("dataPeriodImportBtn");b.disabled=true;try{await jqDataPeriodImportAll()}catch(e){box("dataPeriodImportResult","fail",`FAIL\n${e?.message||e}\n\n保存済み分は保持されています。もう一度①不足チェック→②取り込みで続行できます。`)}finally{b.disabled=false}};
...
function copyTokenToAdvanced(){
 ...
 // Canonical -> legacy mirrors only. Stale legacy values are always overwritten.
 for(const id of JQ_LEGACY_TOKEN_MIRROR_IDS){const el=$(id);if(el)el.value=t}
 return t;
}
...
// v7e-alpha84: advance the local DataLake one trading day with one operation, with current-day optional repair.
function addIsoDays(iso,n){return isoAddDaysUtc(String(iso||"").slice(0,10),Number(n||0))}
function syncDailyDateInputs(date){
 const single=["simpleDailyDate","masterDate","finsDate","earningsDate","screeningAsOf","screeningBaseAsOf","parityAsOf","discoveryAsOf","factorAsOf","watchlistAlertAsOf","investmentTrackingAsOf"];
...
if($("nextTradingDayAllBtn")) $("nextTradingDayAllBtn").onclick=async()=>{
 ...
 try{
   ...
   await run("決算予定",async()=>{const g=await jqFetchEarningsDateByPubDate(target,token);const w=await workerCall("earnings-date-write",300000,null,null,{rows:g.rows});return {rows:g.rows.length,total:w.total,codes:w.codes}});
...
function zipStoreBlob(files){
 ...
 for(const f of files){const name=enc.encode(f.name),data=typeof f.data==="string"?enc.encode(f.data):f.data,crc=crc32Bytes(data),h=[];u32(h,0x04034b50);u16(h,20);u16(h,0);u16(h,0);u16(h,0);u16(h,0);u32(h,crc);u32(h,data.length);u32(h,data.length);u16(h,name.length);u16(h,0);const local=new Uint8Array(h.length+name.length+data.length);local.set(h);local.set(name,h.length);local.set(data,h.length+name.length);locals.push(local);
   const c=[];u32(c,0x02014b50);u16(c,20);u16(c,20);u16(c,0);u16(c,0);u16(c,0);u16(c,0);u32(c,crc);u32(c,data.length);u32(c,data.length);u16(c,name.length);u16(c,0);u16(c,0);u16(c,0);u16(c,0);u32(c,0);u32(c,offset);const ce=new Uint8Array(c.length+name.length);ce.set(c);ce.set(name,c.length);centrals.push(ce);offset+=local.length;
 }
 const centralSize=centrals.reduce((n,x)=>n+x.length,0),end=[];u32(end,0x06054b50);u16(end,0);u16(end,0);u16(end,files.length);u16(end,files.length);u32(end,centralSize);u32(end,offset);u16(end,0);return new Blob([...locals,...centrals,new Uint8Array(end)],{type:"application/zip"});
}

function unzipStoreEntries(arrayBuffer){
 const u=new Uint8Array(arrayBuffer),dv=new DataView(u.buffer,u.byteOffset,u.byteLength),dec=new TextDecoder(),out=new Map();let p=0;
 const u16=o=>dv.getUint16(o,true),u32=o=>dv.getUint32(o,true);
 while(p+4<=u.length){const sig=u32(p);if(sig===0x04034b50){const method=u16(p+8),comp=u32(p+18),nameLen=u16(p+26),extraLen=u16(p+28);if(method!==0)throw new Error('このバックアップZIPはSTORE形式ではありません');const ns=p+30,ds=ns+nameLen+extraLen,de=ds+comp;if(de>u.length)throw new Error('ZIPが途中で切れています');const name=dec.decode(u.slice(ns,ns+nameLen));out.set(name,u.slice(ds,de));p=de;continue}if(sig===0x02014b50||sig===0x06054b50)break;throw new Error('ZIP構造を解釈できません')}
 return out;
}
let latestUserBackupArtifact=null;
if($("userBackupZipBtn")) $("userBackupZipBtn").onclick=async()=>{
 const btn=$("userBackupZipBtn");btn.disabled=true;box("userBackupResult","run","private DBを監査・Export中…");
 try{
  ...
  const bytes=new Uint8Array(r.buffer),manifest={format:"JQ-USER-BACKUP-v1",appVersion:"v9.4",createdAt:new Date().toISOString(),db:"jq_private_v1.sqlite",bytes:r.bytes,sha256:r.sha256||null,quickCheck:priv.quickCheck,tables:priv.tables||[],includes:["portfolio","watchlist","discovery","investment-tracking","web-trade-ledger","void-audit","pipeline-checkpoints"],excludes:["J-Quants API key","market DataLake"]};
...
if($("userBackupDownloadBtn")) $("userBackupDownloadBtn").onclick=()=>{try{if(!latestUserBackupArtifact)throw new Error("先にユーザーデータZIPを作成してください");downloadBlob(latestUserBackupArtifact.blob,latestUserBackupArtifact.name)}catch(e){box("userBackupResult","fail",String(e?.message||e))}};
if($("userRestoreZipBtn")) $("userRestoreZipBtn").onclick=async()=>{
 const f=$("userRestoreZipFile")?.files?.[0];if(!f){box("userRestoreResult","warn","復元するユーザーデータZIPを選択してください。 current private DBは変更しません。");return}
 const btn=$("userRestoreZipBtn");btn.disabled=true;box("userRestoreResult","run","ZIP内容とSQLite headerを検証中…");
 try{
  const entries=unzipStoreEntries(await f.arrayBuffer()),dbBytes=entries.get("jq_private_v1.sqlite"),manBytes=entries.get("manifest.json");if(!dbBytes)throw new Error("jq_private_v1.sqlite がZIP内にありません");if(!sqliteHeaderOk(dbBytes.slice(0,16)))throw new Error("private DBのSQLite header不一致");let man={};if(manBytes){try{man=JSON.parse(new TextDecoder().decode(manBytes))}catch(_){}}
  if(man.format&&man.format!=="JQ-USER-BACKUP-v1")throw new Error(`未対応バックアップ形式: ${man.format}`);
  const restoreFile=new File([dbBytes],"jq_private_v1.sqlite",{type:"application/vnd.sqlite3"});
  const ok=window.confirm(`ユーザーデータを復元します。\n現在のPortfolio / Watchlist / Discovery / 売買履歴はバックアップ内容で置き換わります。\n\nBackup: ${man.createdAt||f.name}\nSize: ${fmt(dbBytes.length)}\n\n続行しますか？`);if(!ok){box("userRestoreResult","warn","復元をキャンセルしました。データは変更していません。");return}
  const r=await workerCall("shard-restore-import",1800000,s=>box("userRestoreResult","run",`${s.stage||"復元中"}\n${s.detail||""}`),restoreFile,{name:"/jq_private_v1.sqlite"});if(r.quickCheck!=="ok")throw new Error(`復元後 quick_check=${r.quickCheck}`);
  box("userRestoreResult","pass",`RESTORE PASS\nprivate DB: ${fmt(r.importedBytes||dbBytes.length)}\nquick_check: ${r.quickCheck}\n\nアプリを再読み込みしてPortfolio / Watchlistを確認してください。`);
 }catch(e){box("userRestoreResult","fail","RESTORE FAIL\n"+(e?.message||e))}finally{btn.disabled=false}
};

const EARNINGS_DATE_BF_KEY="jq_beta3_earnings_date_backfill_v1";
async function earningsDateStatusRender(){try{const r=await workerCall("earnings-date-status",120000);if($("initialDataBuildStatus"))box("initialDataBuildStatus",r.total>0?"pass":"warn",r.total>0?`決算予定日履歴: ${r.total.toLocaleString()}行 / ${r.codes.toLocaleString()}銘柄\n公表日: ${r.minPubDate} ～ ${r.maxPubDate}\nquick_check: ${r.quickCheck}`:"決算予定日履歴は未構築です。新規環境の初期データ構築、または今回だけの移行を実行してください。");return r}catch(e){if($("initialDataBuildStatus"))box("initialDataBuildStatus","warn","決算予定日履歴: 未構築");return null}}
async function runEarningsDateBackfill(resume=false){const token=copyTokenToAdvanced();if(!token)throw new Error("APIキーを入力してください");let from=$("earningsDateBackfillFrom")?.value||addIsoDays(jstTodayIso(),-3652),to=$("earningsDateBackfillTo")?.value||jstTodayIso();if(resume){try{const cp=JSON.parse(localStorage.getItem(EARNINGS_DATE_BF_KEY)||"null");if(cp?.nextDate){from=cp.nextDate;to=cp.to||to}}catch(_){}}if(from>to)throw new Error("取得開始日が終了日より後です");const dates=[];for(let d=from;d<=to;d=addIsoDays(d,1))dates.push(d);let calls=0,apiRows=0,written=0,empty=0,t0=performance.now();for(let i=0;i<dates.length;i++){const day=dates[i];box("earningsDateBackfillResult","run",`決算予定日履歴を構築中…\n公表日: ${day}\n進捗: ${i+1}/${dates.length}\nAPI rows: ${apiRows.toLocaleString()} / 保存処理: ${written.toLocaleString()}\n0件日: ${empty}\n経過: ${((performance.now()-t0)/60000).toFixed(1)}分\n\n中断しても「中断地点から再開」で続けられます。`);const g=await jqFetchEarningsDateByPubDate(day,token);calls++;apiRows+=g.rows.length;if(!g.rows.length)empty++;const w=await workerCall("earnings-date-write",300000,null,null,{rows:g.rows});written+=w.written||0;const next=i+1<dates.length?dates[i+1]:null;if(next)localStorage.setItem(EARNINGS_DATE_BF_KEY,JSON.stringify({nextDate:next,to}));if(i<dates.length-1)await sleep(250)}localStorage.removeItem(EARNINGS_DATE_BF_KEY);const st=await earningsDateStatusRender();box("earningsDateBackfillResult","pass",`決算予定日履歴 BACKFILL PASS\n範囲: ${from} ～ ${to}\nAPI照会: ${calls.toLocaleString()}回 / API rows: ${apiRows.toLocaleString()}\n0件日: ${empty.toLocaleString()}\nDataLake: ${st?.total?.toLocaleString()||"-"}行 / ${st?.codes?.toLocaleString()||"-"}銘柄\n公表日: ${st?.minPubDate||"-"} ～ ${st?.maxPubDate||"-"}\nquick_check: ${st?.quickCheck||"-"}\n\nこのDataLakeは恒久利用します。今回だけの移行ボタンは確認後の次版で削除できます。`)}
if($("earningsDateBackfillTo"))$("earningsDateBackfillTo").value=jstTodayIso();
if($("earningsDateBackfillBtn"))$("earningsDateBackfillBtn").onclick=async()=>{const b=$("earningsDateBackfillBtn");b.disabled=true;try{await runEarningsDateBackfill(false)}catch(e){box("earningsDateBackfillResult","fail","BACKFILL FAIL\n"+(e?.message||e)+"\n\n保存済み分は保持されています。再開ボタンで続けられます。")}finally{b.disabled=false}};
if($("earningsDateBackfillResumeBtn"))$("earningsDateBackfillResumeBtn").onclick=async()=>{const b=$("earningsDateBackfillResumeBtn");b.disabled=true;try{await runEarningsDateBackfill(true)}catch(e){box("earningsDateBackfillResult","fail","RESUME FAIL\n"+(e?.message||e))}finally{b.disabled=false}};
...
async function buildWebPortfolioIntegrated(asOf){
 ...
 const er=await workerCall("earnings-date-next",120000,null,null,{asOf,codes:stocks.map(x=>x.code)}),emap=new Map((er.rows||[]).map(x=>[String(x.Code),x]));
 for(const row of (pr.rows||[])){const z=emap.get(String(row.code));row.NextEarningsDate=z?.NextEarningsDate??null;row.DaysToEarnings=z?.DaysToEarnings??null;row.EarningsFQName=z?.EarningsFQName??null;row.EarningsFYEnd=z?.EarningsFYEnd??null;row.EarningsSchedulePubDate=z?.EarningsSchedulePubDate??null}
...
function screeningParityTraceRows(rows){const fields=["Date","NormalizedCode","CompanyName","Market","Sector33","FinancialDataFlag","EarningsReactionPending","LatestEarningsEventDate","EarningsElapsedTradingDays","PriceHistoryDays","ForecastSalesGrowthPct","ForecastPrimaryProfitGrowthPct","GuidanceDirectionScore","FundamentalScore","PriceRecognitionScore","FundamentalPriceGap","GapPercentile","ReRatingScore","GapCompositeScore","FundamentalMomentumScore","PostEarningsDriftScore","GrowthAccelerationScore","QualityValueReRatingScore","FundamentalPriceGapUniverseRank","FundamentalMomentumUniverseRank","PostEarningsDriftUniverseRank","GrowthAccelerationUniverseRank","QualityValueReRatingUniverseRank","FactorPreviousFYResolver","FactorPreviousFYCandidateCount","FactorLatestFYDisclosureDate","FactorTargetFYEnd","FactorPreviousFYDisclosureDate","FactorPreviousFYEnd"];return (rows||[]).map(r=>Object.fromEntries(fields.map(k=>[k,r?.[k]??""])))}
const CANDIDATE_EARNINGS_FIELDS=["Code","CompanyName","AnnouncementDate","AnnouncementTime","FYEnd","PeriodEnd","Quarter","DocumentType","DuplicateSourceCount","Sales","SalesYoY","OperatingProfit","OperatingProfitYoY","OrdinaryProfit","OrdinaryProfitYoY","NetIncome","NetIncomeYoY","EPS","EPSYoY","OperatingMargin","StandaloneCalculationValid","StandaloneCalculationReason","StandaloneSales","StandaloneOperatingProfit","StandaloneOrdinaryProfit","StandaloneNetIncome","StandaloneOperatingMargin","StandaloneSalesYoY","StandaloneOperatingProfitYoY","ForecastSales","ForecastOperatingProfit","ForecastOrdinaryProfit","ForecastNetIncome","ForecastEPS","SalesProgressRate","SalesProgressMeaningful","OperatingProfitProgressRate","OperatingProfitProgressMeaningful","OrdinaryProfitProgressRate","OrdinaryProfitProgressMeaningful","NetIncomeProgressRate","NetIncomeProgressMeaningful","ForecastRevisionDirection","ForecastRevisionPct","ForecastRevisionMetric","EventBaseDate","EventTiming","MarketCloseTime","AnnouncementTimeValid","PreReturn5D","PreReturn10D","PreReturn20D","PostReturn0D","PostReturn1D","PostReturn3D","PostReturn5D","PostReturn10D","PostReturn20D","EarningsVolumeRatio","EarningsTradingValueRatio","ReactionTradingDate","FirstPostTradingDate","TOPIXPreReturn5D","TOPIXPreReturn10D","TOPIXPreReturn20D","TOPIXPostReturn0D","TOPIXPostReturn1D","TOPIXPostReturn3D","TOPIXPostReturn5D","TOPIXPostReturn10D","TOPIXPostReturn20D","PreRelativeToTOPIX5D","PreRelativeToTOPIX10D","PreRelativeToTOPIX20D","PostRelativeToTOPIX0D","PostRelativeToTOPIX1D","PostRelativeToTOPIX3D","PostRelativeToTOPIX5D","PostRelativeToTOPIX10D","PostRelativeToTOPIX20D"];
const GUIDANCE_SUMMARY_FIELDS=["Code","CompanyName","GuidanceBiasClass","GuidanceBiasConfidence","GuidanceBiasConfidenceScore","RevenueBiasPct","OpProfitBiasPct","BeatRate","MissRate","LargeBeatRate","LargeMissRate","RevisionRate","UpwardRevisionRate","DownwardRevisionRate","RevisionStyle","PostRevisionAccuracyPct","GuidanceErrorDispersion","EffectiveYears","ObservedFiscalYears","EarlyObservedYears","ObservationCoverage","GuidanceBiasStatus","GuidanceEngineVersion"];
if($("screeningShareZipBtn")) $("screeningShareZipBtn").onclick=async()=>{
 ...
 try{
  ...
  const codes=candidates.map(x=>x.NormalizedCode||x.Code).filter(Boolean);
  progress("候補銘柄の決算履歴をWeb DataLakeから構築中…");const eh=await workerCall("candidate-earnings-history",600000,null,null,{asOf,codes});
  progress("会社予想の履歴特性を集計中…");const gh=await workerCall("management-guidance-summary",600000,null,null,{asOf,codes});
  ...
  progress("Watchlist価格条件の完成版を構築中…");const watchlistConditions=await buildWatchlistConditionsComplete(asOf);
  const parityTrace=screeningParityTraceRows(latestScreeningScoredRows||[]);const scfg=loadScreeningConfig(),profileRows=[{Profile:screeningConfigSig(scfg)===screeningConfigSig(SCREENING_DEFAULT_CONFIG)?"Standard":"Custom",Markets:scfg.markets.join(";"),SectorMode:scfg.sectorMode,Sectors:scfg.sectors.join(";"),MinAverageTradingValue20D:scfg.minTradingValue,MinPrice:scfg.minPrice,MinPriceHistoryDays:scfg.minHistoryDays,FundamentalPriceGap:`${scfg.strategies.FundamentalPriceGap.enabled?"ON":"OFF"}:${scfg.strategies.FundamentalPriceGap.preset||"standard"}:Top${scfg.strategies.FundamentalPriceGap.topN}`,FundamentalPriceGapFilters:JSON.stringify(scfg.strategies.FundamentalPriceGap.filters),FundamentalMomentum:`${scfg.strategies.FundamentalMomentum.enabled?"ON":"OFF"}:${scfg.strategies.FundamentalMomentum.preset||"standard"}:Top${scfg.strategies.FundamentalMomentum.topN}`,FundamentalMomentumFilters:JSON.stringify(scfg.strategies.FundamentalMomentum.filters),PostEarningsDrift:`${scfg.strategies.PostEarningsDrift.enabled?"ON":"OFF"}:${scfg.strategies.PostEarningsDrift.preset||"standard"}:Top${scfg.strategies.PostEarningsDrift.topN}`,PostEarningsDriftFilters:JSON.stringify(scfg.strategies.PostEarningsDrift.filters),GrowthAcceleration:`${scfg.strategies.GrowthAcceleration.enabled?"ON":"OFF"}:${scfg.strategies.GrowthAcceleration.preset||"standard"}:Top${scfg.strategies.GrowthAcceleration.topN}`,GrowthAccelerationFilters:JSON.stringify(scfg.strategies.GrowthAcceleration.filters),QualityValueReRating:`${scfg.strategies.QualityValueReRating.enabled?"ON":"OFF"}:${scfg.strategies.QualityValueReRating.preset||"standard"}:Top${scfg.strategies.QualityValueReRating.topN}`,QualityValueReRatingFilters:JSON.stringify(scfg.strategies.QualityValueReRating.filters),GeneratedAsOf:asOf}];const files=[{name:"screening_profile.csv",data:csvAllColumns(profileRows)},{name:"watchlist_conditions_complete.csv",data:simpleCsv(watchlistConditions,WATCHLIST_CONDITION_COMPLETE_FIELDS)},{name:"screening_candidates.csv",data:csvAllColumns(enrichedCandidates)},{name:"screening_ai.csv",data:csvAllColumns(enrichedCandidates)},{name:"screening_parity_trace.csv",data:csvAllColumns(parityTrace)},{name:"factor_monitor_latest.csv",data:simpleCsv(factors,FACTOR_FIELDS)},{name:"factor_summary.csv",data:simpleCsv(summary,FACTOR_SUMMARY_FIELDS)},{name:"candidate_earnings_history.csv",data:csvWithHeaders(eh.rows||[],CANDIDATE_EARNINGS_FIELDS)},{name:"management_guidance_summary.csv",data:csvWithHeaders(gh.rows||[],GUIDANCE_SUMMARY_FIELDS)},{name:"discovery_episode_master.csv",data:discoveryCsv(episodes)},{name:"discovery_episode_analysis.csv",data:discoveryCsv(episodes)},{name:"discovery_episode_daily.csv",data:discoveryDailyCsv(daily)}];
  const omitted=[],manifest={bundle:"Web Screening Share",version:"v9.4",asOf,generatedAt:new Date().toISOString(),canonical:"Web-first",canonicalAsOfSource:canon.source,dataLakeDate:canon.dataLakeDate,lastPassPipelineDate:canon.passDate,screeningProfile:profileRows[0],counts:{watchlistConditions:watchlistConditions.length,screeningCandidates:enrichedCandidates.length,screeningParityTrace:parityTrace.length,factors:factors.length,factorSummary:summary.length,candidateEarningsHistory:eh.count||0,candidateEarningsCodes:eh.codes||0,managementGuidance:gh.count||0,managementGuidanceCodes:(gh.rows||[]).filter(x=>Number(x.ObservedFiscalYears||0)>0).length,discoveryEpisodes:episodes.length,discoveryDaily:daily.length},earningsDataQuality:{requestedCodes:new Set(codes.map(String)).size,candidateHistoryCodes:eh.codes||0,candidateHistoryRows:eh.count||0,guidanceRows:gh.count||0,guidanceObservedCodes:(gh.rows||[]).filter(x=>Number(x.ObservedFiscalYears||0)>0).length},files:files.map(x=>x.name),omitted,engines:{candidateEarnings:eh.engineVersion||"WebNativeV1",managementGuidance:gh.engineVersion||"WebNativeV1"},notes:["screening_parity_trace.csv is an audit-only compact trace of the full Web scored universe for PC/Web migration diagnostics.","PC refresh is not required.","ZIP generation and download are separated; re-download never reruns analysis.","Candidate earnings history and management guidance are generated natively from the Web DataLake. Guidance confidence is explicitly versioned WebNativeV1 and is not claimed to be byte-for-byte PC parity until real-device comparison is completed."]};
...
if($("aiShareZipBtn")) $("aiShareZipBtn").onclick=async()=>{
 ...
 try{
  ...
  if((h.counts?.marketShortRatioHistory??0)>0 && (!(h.counts?.marketShortRatioValid>0)||!(h.counts?.marketShortRatioComponentValid>0)||!(h.counts?.marketShortRatioNonZero>0)))throw new Error(`Data Quality FAIL: 市場空売り比率 ${h.counts?.marketShortRatioHistory??0}行に対し、short成分=${h.counts?.marketShortRatioComponentValid??0} / ratio有効=${h.counts?.marketShortRatioPctValid??0} / ratio>0=${h.counts?.marketShortRatioNonZero??0} です。空売り成分を確認できないため共有ZIPは生成しません。`);
  ...
  const extra=[
   ...
   {name:"market_short_ratio_history.csv",data:csvWithHeaders(h.marketShortRatioHistory||[],["Date","Section","SellExShortValue","ShortWithRestrictionValue","ShortNoRestrictionValue","ShortTotalValue","ShortRatioPct"])},
...
const SCREENING_CONFIG_KEY="jq_screening_config_v3";
const SCREENING_PROFILE_KEY="jq_screening_named_profiles_v1";
const SCREENING_STRATEGY_NAMES=["FundamentalPriceGap","FundamentalMomentum","PostEarningsDrift","GrowthAcceleration","QualityValueReRating"];
const SCREENING_DEFAULT_FILTERS={
 FundamentalPriceGap:{minFundamentalScore:"",minFundamentalPriceGap:"",minReRatingScore:""},
 FundamentalMomentum:{minFundamentalScore:"",minReRatingScore:""},
 PostEarningsDrift:{minElapsed:3,maxElapsed:20,minEarningsReturn5D:"",minFollowThrough5D:"",minVolumeRatio5To20:""},
 GrowthAcceleration:{minSalesYoY:"",minPrimaryProfitYoY:"",minOperatingMarginChangePt:"",minForecastSalesGrowthPct:"",minForecastPrimaryProfitGrowthPct:"",requirePositiveCFO:false},
 QualityValueReRating:{maxForecastPER:"",maxPBR:"",minForecastDividendYieldPct:"",minROE:"",minQVRReRatingScore:""}
};
const SCREENING_DEFAULT_STRATEGIES=Object.fromEntries(SCREENING_STRATEGY_NAMES.map(k=>[k,{enabled:true,preset:"standard",topN:20,filters:{...SCREENING_DEFAULT_FILTERS[k]}}]));
const SCREENING_DEFAULT_CONFIG={markets:["プライム","スタンダード","グロース"],minTradingValue:50000000,minPrice:100,minHistoryDays:60,sectorMode:"17",sectors:[],strategies:SCREENING_DEFAULT_STRATEGIES};
function screeningMaybeNum(v,def=""){if(v==null||String(v).trim()==="")return def;const n=Number(v);return Number.isFinite(n)?n:def}
function normalizeScreeningConfig(x={}){const markets=Array.isArray(x.markets)?x.markets.filter(v=>SCREENING_DEFAULT_CONFIG.markets.includes(String(v))):[...SCREENING_DEFAULT_CONFIG.markets],src=x.strategies&&typeof x.strategies==="object"?x.strategies:{};const strategies={};for(const k of SCREENING_STRATEGY_NAMES){const z=src[k]||{},oldTop=z.strength==="weak"?10:z.strength==="strong"?30:20,topN=Math.max(1,Math.min(100,Math.round(screeningMaybeNum(z.topN,oldTop))));const df=SCREENING_DEFAULT_FILTERS[k],sf=z.filters&&typeof z.filters==="object"?z.filters:{};const filters={};for(const [fk,dv] of Object.entries(df)){filters[fk]=typeof dv==="boolean"?!!sf[fk]:screeningMaybeNum(sf[fk],dv)}const preset=["relaxed","standard","strict"].includes(z.preset)?z.preset:"standard";strategies[k]={enabled:z.enabled!==false,preset,topN,filters}}return{markets:markets.length?markets:[...SCREENING_DEFAULT_CONFIG.markets],minTradingValue:SCREENING_DEFAULT_CONFIG.minTradingValue,minPrice:SCREENING_DEFAULT_CONFIG.minPrice,minHistoryDays:SCREENING_DEFAULT_CONFIG.minHistoryDays,sectorMode:String(x.sectorMode)==="33"?"33":"17",sectors:Array.isArray(x.sectors)?[...new Set(x.sectors.map(String).filter(Boolean))]:[],strategies}}
function loadScreeningConfig(){try{return normalizeScreeningConfig(JSON.parse(localStorage.getItem(SCREENING_CONFIG_KEY)||"{}"))}catch(_){return normalizeScreeningConfig()}}
function screeningConfigSig(c=loadScreeningConfig()){return JSON.stringify(normalizeScreeningConfig(c))}
function screeningFilterText(name,z){const f=z.filters||{},a=[];const add=(label,v,suffix="")=>{if(v!==""&&v!=null)a.push(`${label}${v}${suffix}`)};if(name==="FundamentalPriceGap"){add("Fundamental≥",f.minFundamentalScore);add("業績-株価Gap≥",f.minFundamentalPriceGap);add("再評価≥",f.minReRatingScore)}else if(name==="FundamentalMomentum"){add("Fundamental≥",f.minFundamentalScore);add("再評価≥",f.minReRatingScore)}else if(name==="PostEarningsDrift"){a.push(`決算後${f.minElapsed}〜${f.maxElapsed}営業日`);add("5日騰落≥",f.minEarningsReturn5D,"%");add("FollowThrough≥",f.minFollowThrough5D,"%");add("出来高倍率≥",f.minVolumeRatio5To20,"倍")}else if(name==="GrowthAcceleration"){add("売上YoY≥",f.minSalesYoY,"%");add("利益YoY≥",f.minPrimaryProfitYoY,"%");add("利益率改善≥",f.minOperatingMarginChangePt,"pt");add("予想売上成長≥",f.minForecastSalesGrowthPct,"%");add("予想利益成長≥",f.minForecastPrimaryProfitGrowthPct,"%");if(f.requirePositiveCFO)a.push("CFO黒字") }else if(name==="QualityValueReRating"){add("PER≤",f.maxForecastPER,"倍");add("PBR≤",f.maxPBR,"倍");add("配当利回り≥",f.minForecastDividendYieldPct,"%");add("ROE≥",f.minROE,"%");add("再評価≥",f.minQVRReRatingScore)}return a.join(" / ")||"追加条件なし"}
function screeningPresetExplain(name,preset){const strict={FundamentalPriceGap:"Fundamental≥55 / Gap≥55 / 再評価≥55",FundamentalMomentum:"Fundamental≥60 / 再評価≥55",PostEarningsDrift:"決算後3〜15日 / 5日騰落≥2% / FollowThrough≥0% / 出来高≥1.1倍",GrowthAcceleration:"売上YoY≥5% / 利益YoY≥10% / 利益率改善≥0pt / 予想売上≥3% / 予想利益≥5% / CFO黒字",QualityValueReRating:"PER≤20倍 / PBR≤2倍 / ROE≥8% / 再評価≥55"};if(preset==="strict")return strict[name]||"追加条件を厳格化";if(preset==="relaxed")return (name==="FundamentalPriceGap"||name==="FundamentalMomentum")?"追加Filterなし / 過熱Penaltyを緩和":"追加Filterなし / 従来ランキング";return "従来Screening互換 / 追加Filterなし"}
function updateScreeningPresetExplain(card){if(!card)return;const k=card.dataset.strategy,p=card.querySelector('.screen-strategy-preset')?.value||"standard",el=card.querySelector('.screen-preset-explain');if(el)el.textContent=screeningPresetExplain(k,p)}
function screeningConfigLabel(c=loadScreeningConfig()){c=normalizeScreeningConfig(c);const ms=c.markets.length===3?"全市場":c.markets.join("・"),sec=c.sectors.length?`${c.sectorMode}業種 ${c.sectors.length}件`:"全セクター",jp={FundamentalPriceGap:"出遅れ再評価",FundamentalMomentum:"業績モメンタム",PostEarningsDrift:"決算後",GrowthAcceleration:"業績加速",QualityValueReRating:"割安・リレーティング"};const on=SCREENING_STRATEGY_NAMES.filter(k=>c.strategies[k]?.enabled);return `${ms} / ${sec}\n${on.map(k=>`${jp[k]} Top${c.strategies[k].topN}（${screeningFilterText(k,c.strategies[k])}）`).join("\n")||"探索戦略なし"}`}

const SCREENING_SECTOR_FALLBACK={
  "17":["食品","エネルギー資源","建設・資材","素材・化学","医薬品","自動車・輸送機","鉄鋼・非鉄","機械","電機・精密","情報通信・サービスその他","電力・ガス","運輸・物流","商社・卸売","小売","銀行","金融（除く銀行）","不動産"],
  "33":["水産・農林業","鉱業","建設業","食料品","繊維製品","パルプ・紙","化学","医薬品","石油・石炭製品","ゴム製品","ガラス・土石製品","鉄鋼","非鉄金属","金属製品","機械","電気機器","輸送用機器","精密機器","その他製品","電気・ガス業","陸運業","海運業","空運業","倉庫・運輸関連業","情報・通信業","卸売業","小売業","銀行業","証券、商品先物取引業","保険業","その他金融業","不動産業","サービス業"]
};
let screeningSectorOptions={"17":[...SCREENING_SECTOR_FALLBACK["17"]],"33":[...SCREENING_SECTOR_FALLBACK["33"]]};
function readScreeningConfigUi(){const markets=[...document.querySelectorAll('#screenCfgMarkets input[type="checkbox"]:checked')].map(x=>x.value),sectorMode=document.querySelector('input[name="screenCfgSectorMode"]:checked')?.value||"17",all=$("screenCfgAllSectors")?.checked!==false,sectors=all?[]:[...document.querySelectorAll('#screenCfgSectorList input[type="checkbox"]:checked')].map(x=>x.value),strategies={};document.querySelectorAll('#screenCfgStrategies .screen-strategy-card').forEach(card=>{const k=card.dataset.strategy,preset=card.querySelector('.screen-strategy-preset')?.value||"standard",topN=20;strategies[k]={enabled:card.querySelector('.screen-strategy-enabled')?.checked!==false,preset,topN,filters:{...SCREENING_DEFAULT_FILTERS[k]}}});return normalizeScreeningConfig({markets,sectorMode,sectors,strategies})}
function renderScreeningSectorChoices(cfg=loadScreeningConfig()){const host=$("screenCfgSectorList");if(!host)return;const mode=String(document.querySelector('input[name="screenCfgSectorMode"]:checked')?.value||cfg.sectorMode||"17"),opts=(screeningSectorOptions[mode]&&screeningSectorOptions[mode].length?screeningSectorOptions[mode]:SCREENING_SECTOR_FALLBACK[mode])||[],selected=new Set(cfg.sectorMode===mode?cfg.sectors:[]);host.innerHTML=opts.map(v=>`<label><input type="checkbox" value="${escapeHtml(v)}" ${selected.has(v)?"checked":""}> ${escapeHtml(v)}</label>`).join("");const all=$("screenCfgAllSectors")?.checked!==false;const controls=$("screenCfgSectorControls");if(controls)controls.hidden=all;host.querySelectorAll('input').forEach(x=>x.disabled=all)}
function applyScreeningConfigUi(cfg=loadScreeningConfig()){cfg=normalizeScreeningConfig(cfg);document.querySelectorAll('input[name="screenCfgSectorMode"]').forEach(x=>x.checked=x.value===cfg.sectorMode);document.querySelectorAll('#screenCfgMarkets input[type="checkbox"]').forEach(x=>x.checked=cfg.markets.includes(x.value));if($("screenCfgAllSectors"))$("screenCfgAllSectors").checked=!cfg.sectors.length;document.querySelectorAll('#screenCfgStrategies .screen-strategy-card').forEach(card=>{const k=card.dataset.strategy,z=cfg.strategies[k]||SCREENING_DEFAULT_STRATEGIES[k],e=card.querySelector('.screen-strategy-enabled');if(e)e.checked=z.enabled;{const ps=card.querySelector('.screen-strategy-preset');if(ps)ps.value=z.preset||"standard"}card.classList.toggle('disabled',!z.enabled);updateScreeningPresetExplain(card)});renderScreeningSectorChoices(cfg);renderScreeningNamedProfiles();if($("screenCfgResult"))box("screenCfgResult",screeningConfigSig(cfg)===screeningConfigSig(SCREENING_DEFAULT_CONFIG)?"pass":"warn",`${screeningConfigSig(cfg)===screeningConfigSig(SCREENING_DEFAULT_CONFIG)?"標準設定":"カスタム設定"}
${screeningConfigLabel(cfg)}`)}
async function initScreeningConfigUi(){if(!$("screenCfgResult"))return;applyScreeningConfigUi(loadScreeningConfig());try{const r=await workerCall("screening-master-options",120000);if(Array.isArray(r.sector17)&&r.sector17.length)screeningSectorOptions["17"]=r.sector17;if(Array.isArray(r.sector33)&&r.sector33.length)screeningSectorOptions["33"]=r.sector33}catch(_){}renderScreeningSectorChoices(loadScreeningConfig())}
function loadNamedScreeningProfiles(){try{const x=JSON.parse(localStorage.getItem(SCREENING_PROFILE_KEY)||"[]");return (Array.isArray(x)?x:[]).filter(p=>p&&p.config).slice(0,3).map(p=>({name:String(p.name||"保存設定"),config:normalizeScreeningConfig(p.config)}))}catch(_){return []}}
function saveNamedScreeningProfiles(x){localStorage.setItem(SCREENING_PROFILE_KEY,JSON.stringify(x.slice(0,3)))}
function renderScreeningNamedProfiles(){const sel=$("screenProfileSelect"),ps=loadNamedScreeningProfiles();if(!sel)return;const cur=sel.value;sel.innerHTML='<option value="">保存済み設定を選択</option>'+ps.map((p,i)=>`<option value="${i}">${escapeHtml(p.name)}</option>`).join('');if(ps[Number(cur)])sel.value=cur;if($("screenProfileLoad"))$("screenProfileLoad").disabled=sel.value==="";if($("screenProfileDelete"))$("screenProfileDelete").disabled=sel.value===""}
if($("screenProfileSelect"))$("screenProfileSelect").onchange=renderScreeningNamedProfiles;
if($("screenProfileSave"))$("screenProfileSave").onclick=()=>{const ps=loadNamedScreeningProfiles(),name=$("screenProfileName")?.value.trim();if(!name){box("screenCfgResult","warn","保存する設定名を入力してください。");return}const hit=ps.findIndex(p=>p.name===name);const item={name,config:readScreeningConfigUi()};if(hit>=0)ps[hit]=item;else if(ps.length<3)ps.push(item);else{box("screenCfgResult","warn","保存できる設定は3件までです。不要な設定を削除してから保存してください。");return}saveNamedScreeningProfiles(ps);renderScreeningNamedProfiles();if($("screenProfileName"))$("screenProfileName").value="";box("screenCfgResult","pass",`「${name}」を保存しました。`)};
if($("screenProfileLoad"))$("screenProfileLoad").onclick=()=>{const i=Number($("screenProfileSelect")?.value),p=loadNamedScreeningProfiles()[i];if(!p)return;localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(p.config));latestScreeningBaseRows=[];latestScreeningBaseAsOf="";latestScreeningBaseProfileSig="";applyScreeningConfigUi(p.config);box("screenCfgResult","pass",`「${p.name}」を読み込みました。`)};
if($("screenProfileDelete"))$("screenProfileDelete").onclick=()=>{const i=Number($("screenProfileSelect")?.value),ps=loadNamedScreeningProfiles();if(!Number.isInteger(i)||!ps[i])return;const name=ps[i].name;ps.splice(i,1);saveNamedScreeningProfiles(ps);renderScreeningNamedProfiles();box("screenCfgResult","pass",`「${name}」を削除しました。`)};
document.querySelectorAll('input[name="screenCfgSectorMode"]').forEach(el=>el.onchange=()=>{const c=readScreeningConfigUi();c.sectorMode=el.value;c.sectors=[];renderScreeningSectorChoices(c)});
if($("screenCfgAllSectors"))$("screenCfgAllSectors").onchange=()=>renderScreeningSectorChoices(readScreeningConfigUi());if($("screenCfgSectorSelectAll"))$("screenCfgSectorSelectAll").onclick=()=>document.querySelectorAll('#screenCfgSectorList input[type="checkbox"]').forEach(x=>x.checked=true);if($("screenCfgSectorClearAll"))$("screenCfgSectorClearAll").onclick=()=>document.querySelectorAll('#screenCfgSectorList input[type="checkbox"]').forEach(x=>x.checked=false);
document.querySelectorAll('#screenCfgStrategies .screen-strategy-enabled').forEach(x=>x.onchange=()=>{const card=x.closest('.screen-strategy-card');if(card)card.classList.toggle('disabled',!x.checked)});
document.querySelectorAll('#screenCfgStrategies .screen-strategy-preset').forEach(x=>x.onchange=()=>updateScreeningPresetExplain(x.closest('.screen-strategy-card')));
if($("screenCfgSaveBtn"))$("screenCfgSaveBtn").onclick=()=>{const c=readScreeningConfigUi();if(!c.markets.length){box("screenCfgResult","warn","対象市場を1つ以上選択してください。");return}if(!$("screenCfgAllSectors").checked&&!c.sectors.length){box("screenCfgResult","warn","全セクターをOFFにする場合は、対象セクターを1つ以上選択してください。");return}if(!SCREENING_STRATEGY_NAMES.some(k=>c.strategies[k]?.enabled)){box("screenCfgResult","warn","探索戦略を1つ以上『使用』にしてください。");return}localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(c));latestScreeningBaseRows=[];latestScreeningBaseAsOf="";latestScreeningBaseProfileSig="";applyScreeningConfigUi(c);box("screenCfgResult","pass",`設定を保存しました。次回Screeningから適用します。\n${screeningConfigLabel(c)}`)};
if($("screenCfgResetBtn"))$("screenCfgResetBtn").onclick=()=>{const c=normalizeScreeningConfig(SCREENING_DEFAULT_CONFIG);localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(c));latestScreeningBaseRows=[];latestScreeningBaseAsOf="";latestScreeningBaseProfileSig="";applyScreeningConfigUi(c);box("screenCfgResult","pass",`標準設定に戻しました。\n${screeningConfigLabel(c)}`)};
setTimeout(initScreeningConfigUi,0);
...
function buildScreeningStrategies(rows){
 ...
 rr.forEach(r=>{
   ...
   const pedCfg=cfg.strategies?.PostEarningsDrift?.filters||SCREENING_DEFAULT_FILTERS.PostEarningsDrift;
   r.PostEarningsDriftScore=(r.EarningsElapsedTradingDays!=null&&r.EarningsElapsedTradingDays>=Number(pedCfg.minElapsed??3)&&r.EarningsElapsedTradingDays<=Number(pedCfg.maxElapsed??20))
 ...
 rr.forEach(r=>{r.GapCompositeScore=(r.GapPercentile||0)*.65+(r.ReRatingScore||50)*.35-(r.ChasePenalty||0);r.GapCompositeScoreRelaxed=(r.GapPercentile||0)*.65+(r.ReRatingScore||50)*.35-(r.ChasePenalty||0)*.5;r.FundamentalMomentumScore=r.FundamentalScore*.55+r.ReRatingScore*.45-(r.ChasePenalty||0);r.FundamentalMomentumScoreRelaxed=r.FundamentalScore*.55+r.ReRatingScore*.45-(r.ChasePenalty||0)*.5;r.GrowthAccelerationScore=r.FundamentalScore});
 ...
 const map=new Map();
 const validNum=v=>v!=null&&String(v).trim()!==""&&Number.isFinite(Number(v));
 const ge=(v,t)=>t===""||t==null||(validNum(v)&&Number(v)>=Number(t));
 const le=(v,t)=>t===""||t==null||(validNum(v)&&Number(v)<=Number(t));
 const presetFilters=(name,preset)=>{if(preset!=="strict")return {...SCREENING_DEFAULT_FILTERS[name]};const x={FundamentalPriceGap:{minFundamentalScore:55,minFundamentalPriceGap:55,minReRatingScore:55},FundamentalMomentum:{minFundamentalScore:60,minReRatingScore:55},PostEarningsDrift:{minElapsed:3,maxElapsed:15,minEarningsReturn5D:2,minFollowThrough5D:0,minVolumeRatio5To20:1.1},GrowthAcceleration:{minSalesYoY:5,minPrimaryProfitYoY:10,minOperatingMarginChangePt:0,minForecastSalesGrowthPct:3,minForecastPrimaryProfitGrowthPct:5,requirePositiveCFO:true},QualityValueReRating:{maxForecastPER:20,maxPBR:2,minForecastDividendYieldPct:"",minROE:8,minQVRReRatingScore:55}};return x[name]||{}};
 const strategyPass=(name,r,z)=>{const f=(z?.preset&&z.preset!=="standard")?presetFilters(name,z.preset):(z?.filters||{});
   if(name==="FundamentalPriceGap")return ge(r.FundamentalScore,f.minFundamentalScore)&&ge(r.FundamentalPriceGap,f.minFundamentalPriceGap)&&ge(r.ReRatingScore,f.minReRatingScore);
   if(name==="FundamentalMomentum")return ge(r.FundamentalScore,f.minFundamentalScore)&&ge(r.ReRatingScore,f.minReRatingScore);
   if(name==="PostEarningsDrift")return ge(r.EarningsElapsedTradingDays,f.minElapsed)&&le(r.EarningsElapsedTradingDays,f.maxElapsed)&&ge(r.EarningsReturn5D,f.minEarningsReturn5D)&&ge(r.EarningsFollowThrough5D,f.minFollowThrough5D)&&ge(r.VolumeRatio5To20,f.minVolumeRatio5To20);
   if(name==="GrowthAcceleration")return ge(r.SalesYoY,f.minSalesYoY)&&ge(r.PrimaryProfitYoY,f.minPrimaryProfitYoY)&&ge(r.OperatingMarginChangePt,f.minOperatingMarginChangePt)&&ge(r.ForecastSalesGrowthPct,f.minForecastSalesGrowthPct)&&ge(r.ForecastPrimaryProfitGrowthPct,f.minForecastPrimaryProfitGrowthPct)&&(!f.requirePositiveCFO||Number(r.LatestAvailableCFO)>0);
   if(name==="QualityValueReRating")return le(r.ForecastPER,f.maxForecastPER)&&le(r.PBR,f.maxPBR)&&ge(r.ForecastDividendYieldPct,f.minForecastDividendYieldPct)&&ge(r.ROE,f.minROE)&&ge(r.QVRReRatingScore,f.minQVRReRatingScore);
   return true};
 for(const [name,key] of defs){
   const z=cfg.strategies?.[name]||SCREENING_DEFAULT_STRATEGIES[name];
   const rankKey=(z.preset==="relaxed"&&name==="FundamentalPriceGap")?"GapCompositeScoreRelaxed":(z.preset==="relaxed"&&name==="FundamentalMomentum")?"FundamentalMomentumScoreRelaxed":key;
   const eligible=rr.filter(r=>r[rankKey]!=null&&!r.EarningsReactionPending&&r.FinancialDataFlag!=="WebRequired"&&r.FinancialDataFlag!=="NoLatestFinancial"&&strategyPass(name,r,z))
     .sort((a,b)=>Number(b[rankKey])-Number(a[rankKey])||String(a.NormalizedCode).localeCompare(String(b.NormalizedCode)));
   eligible.forEach((r,i)=>r[name+"UniverseRank"]=i+1);
   if(!z.enabled)continue;
   const ranked=eligible.slice(0,Math.max(1,Math.min(100,Number(z.topN)||20)));
   ranked.forEach((r,i)=>{let c=map.get(r.NormalizedCode);if(!c){c={...r,_ranks:{}};map.set(r.NormalizedCode,c)}c._ranks[name]=i+1});
 }
 const pending=rr.filter(r=>r.EarningsReactionPending)
...
if($("screeningStrategyBtn")) $("screeningStrategyBtn").onclick=async()=>{
 ...
 try{
   ...
   box("screeningStrategyResult","pass",`Web Screening 5戦略\n母集団: ${enriched.length}\n候補ユニーク: ${latestScreeningCandidates.length}\n${Object.entries(counts).map(([k,v])=>{const z=loadScreeningConfig().strategies?.[k]||SCREENING_DEFAULT_STRATEGIES[k];return k+": "+v+"/"+(z.enabled?(Number(z.topN)||20):0)}).join("\n")}\nReactionPending: ${pending}\n\nPEDイベント窓: PC版定義で計算\n次段階: PC版候補とのParity`);
...
const INVESTMENT_DISC_REQUIRED=["SelectionType","Conviction","ExpectedHorizon","PrimaryCatalyst","PrimaryRisk"],INVESTMENT_WATCH_REQUIRED=["WatchReason","CurrentDecision","ThesisSummary","InvalidationSummary"];
const DISCOVERY_ROUTE_FIELDS=["Code","EpisodeStartDate","AnalystDecision","SelectionType","Conviction","ExpectedHorizon","PrimaryCatalyst","PrimaryRisk","AnalystNote","EpisodeStartReason","PrimaryStrategy","InvestmentStatus"];
const WATCHLIST_ROUTE_FIELDS=["Action","Code","WatchStartDate","WatchReason","CurrentDecision","ReferencePrice","ThesisSummary","InvalidationSummary","ReviewExpiry","PriceTriggerPct1","PriceTriggerPct2","PriceTriggerPct3","TargetPER","TargetPBR","TargetDividendYieldPct","EarningsLeadDays","Notes","InvestmentStatus","ActionableFlag","InputReferencePrice","ReferencePriceDate","ReferencePriceSource","ReferencePriceAudit"];
let latestWatchlistWeb={master:[],state:[]},latestInvestmentRoutes={discovery:[],watchlist:[]},latestInvestmentPreview=null;
function normalizeInvestmentCode(v){let c=String(v??"").trim().toUpperCase();if(c.length===5&&c.endsWith("0"))c=c.slice(0,4);return c}
function simpleCsv(rows,fields){const esc=v=>'"'+String(v??"").replaceAll('"','""')+'"';return "\uFEFF"+[fields.map(esc).join(","),...(rows||[]).map(r=>fields.map(k=>esc(r[k]??"")).join(","))].join("\n")}
function watchlistRowDiff(pcRows,webRows,fields,key="WatchID"){
 const wm=new Map((webRows||[]).map(r=>[String(r[key]??"").trim(),r])),diffs=[];let perfect=0,missing=0;
 for(const p of pcRows||[]){const id=String(p[key]??"").trim(),w=wm.get(id);if(!w){missing++;diffs.push({id,bad:["Web missing"]});continue}const bad=[];for(const f of fields){const a=String(p[f]??"").trim(),b=String(w[f]??"").trim();if(a!==b)bad.push(`${f}: PC=${a||"(blank)"} / Web=${b||"(blank)"}`)}if(bad.length)diffs.push({id,bad});else perfect++}
 const pcKeys=new Set((pcRows||[]).map(r=>String(r[key]??"").trim())),webOnly=(webRows||[]).filter(r=>!pcKeys.has(String(r[key]??"").trim())).map(r=>String(r[key]??"").trim());return{perfect,missing,diffs,webOnly}
}
function renderWatchlistParity(pcMaster,pcState,web){
 const m=watchlistRowDiff(pcMaster,web.master||[],WATCHLIST_MASTER_FIELDS),st=watchlistRowDiff(pcState,web.state||[],WATCHLIST_STATE_FIELDS),active=(web.master||[]).filter(r=>["Active","ReviewDue"].includes(String(r.Status||""))).length,statusCounts={};for(const r of web.master||[]){const x=String(r.InvestmentStatus||"WATCH")||"WATCH";statusCounts[x]=(statusCounts[x]||0)+1}
 const exact=!m.diffs.length&&!m.missing&&!m.webOnly.length&&!st.diffs.length&&!st.missing&&!st.webOnly.length,cls=exact?"pass":"warn";
 box("watchlistParityResult",cls,[exact?"完全一致 PASS":"要確認",`PC Master: ${pcMaster.length} / Web Master: ${(web.master||[]).length} / 完全一致: ${m.perfect}`,`PC State: ${pcState.length} / Web State: ${(web.state||[]).length} / 完全一致: ${st.perfect}`,`Active/ReviewDue: ${active}`,`InvestmentStatus: ${Object.entries(statusCounts).map(([k,v])=>k+"="+v).join(" / ")||"-"}`,`Master差分: ${m.diffs.length} / Web欠損: ${m.missing} / Webのみ: ${m.webOnly.length}`,`State差分: ${st.diffs.length} / Web欠損: ${st.missing} / Webのみ: ${st.webOnly.length}`,"","state移行Parityは完了済み。Re-Evaluation Alertは⑧-3でWeb-first Factor/Seasonality stateへ接続します。"].join("\n"));
 const rows=[...m.diffs.slice(0,20).map(x=>({type:"Master",...x})),...st.diffs.slice(0,20).map(x=>({type:"State",...x}))];if(!rows.length){$("watchlistParityTable").innerHTML="";return}const esc=x=>String(x??"").replace(/[&<>\"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]));$("watchlistParityTable").innerHTML=`<div style="overflow:auto"><table style="width:100%;font-size:12px"><tr><th>Type</th><th>WatchID</th><th>差分</th></tr>${rows.map(x=>`<tr><td>${esc(x.type)}</td><td>${esc(x.id)}</td><td>${x.bad.map(esc).join("<br>")}</td></tr>`).join("")}</table></div>`
}
if($("watchlistSeedBtn"))$("watchlistSeedBtn").onclick=async()=>{const mf=$("watchlistMasterFile")?.files?.[0],sf=$("watchlistStateFile")?.files?.[0];if(!mf||!sf){box("watchlistParityResult","warn","PCの watchlist_master.csv と watchlist_state.csv を両方選択してください。\nPC: private/Watchlist/master/ と private/Watchlist/state/");return}const btn=$("watchlistSeedBtn");btn.disabled=true;try{const master=parseCsv(await mf.text()).rows,state=parseCsv(await sf.text()).rows;if(!master.length)throw new Error("watchlist_master.csv が空です");const r=await workerCall("watchlist-seed-import",300000,null,null,{master,state});latestWatchlistWeb={master:r.master||[],state:r.state||[]};$("watchlistExportBtn").disabled=false;renderWatchlistParity(master,state,latestWatchlistWeb)}catch(e){box("watchlistParityResult","fail","FAIL\n"+(e?.message||e))}finally{btn.disabled=false}};
...
const WATCHLIST_ALERT_DIAG_FIELDS=["WatchID","Code","Status","ScreeningRowFound","FactorKey","FactorDate","FactorEngineVersion","CurrentPrice","ReferencePrice","PriceChangePct","PriceAlertLevel","PrevPriceAlertLevel","CurrentPER","TargetPER","CurrentPBR","TargetPBR","DividendYield","TargetDividendYieldPct","LatestPeriodType","ProfitType","CurrentPrimaryProfit","ForecastPrimaryProfit","PrimaryProfitProgressPct","FinancialChanged","PrevFinancialFingerprint","CurrentFinancialFingerprint","FactorState","PrevFactorState","SeasonState","SeasonalAlert","PrevSeasonalAlert","TechnicalState","PrevTechnicalState","DaysToEarnings","CatalystLevel","PrevCatalystLevel","ReviewExpiry","Expired","MigrationBaseline","SuppressedReason","AlertTypes","AlertCount"];
let latestWatchlistAlertPreview=null;
function wNum(v){if(v==null||String(v).trim()==="")return null;const x=Number(v);return Number.isFinite(x)?x:null}
function wPyFloatStr(v){const x=wNum(v);if(x==null)return"None";return Number.isInteger(x)?x.toFixed(1):String(x)}
function wCanonicalPrimary(row){const current=wNum(row?.CurrentPrimaryProfit??row?.OperatingProfit??row?.OrdinaryProfit),forecast=wNum(row?.ForecastPrimaryProfit??row?.ForecastOperatingProfit??row?.ForecastOrdinaryProfit),pt=String(row?.LatestPeriodType||"").toUpperCase(),progress=(pt==="FY"||current==null||forecast==null||forecast===0)?null:current/forecast*100;return{current,forecast,progress}}
function wFinancialFingerprint(row){const c=wCanonicalPrimary(row);return [String(row?.LatestDisclosureDate||row?.LatestFinancialDisclosureDate||""),wPyFloatStr(c.forecast),"",wPyFloatStr(row?.ForecastEPS),wPyFloatStr(row?.ForecastAnnualDividend),wPyFloatStr(c.progress)].join("|")}
function wTechnicalState(row,decision){const rsi=wNum(row?.RSI14),ma25dev=wNum(row?.MA25DeviationPct),slope=wNum(row?.MA25Slope5DPct),macd=wNum(row?.MACDHistogramChange5D),vol=wNum(row?.LatestTradingValueRatioTo20D??row?.VolumeRatio5To20),d60h=wNum(row?.DistanceFrom60DHighPct),d=String(decision||"");if(d.includes("WaitPullback")){if(rsi!=null&&rsi<=60&&(ma25dev==null||Math.abs(ma25dev)<=5)&&(macd||0)>=0)return"PullbackReady";if(rsi!=null&&rsi>=70)return"Overheated"}if(d.includes("WaitBreakout")){if(d60h!=null&&d60h>=-1.5&&(vol||0)>=1.2)return"BreakoutReady"}if((slope||0)>0&&(macd||0)>0&&(vol||0)>=1.0)return"Improving";return"Neutral"}
function wPriceAlertLevel(change,m){if(change==null)return 0;const hit=[];for(let i=1;i<=3;i++){const t=wNum(m?.[`PriceTriggerPct${i}`]);if(t!=null&&change<=t)hit.push(i)}return hit.length?Math.max(...hit):0}
function wParseLeads(v){const a=[];for(const p of String(v||"14;7;3").replaceAll(",",";").split(";")){const n=Math.trunc(Number(p.trim()));if(Number.isFinite(n)&&n>=0)a.push(n)}return [...new Set(a)].sort((a,b)=>b-a).length?[...new Set(a)].sort((a,b)=>b-a):[14,7,3]}
function wClone(x){return JSON.parse(JSON.stringify(x??null))}
function wAlert(m,row,asOf,type,reason,severity,factor,tech){const cur=wNum(row?.Close),ref=wNum(m?.ReferencePrice),chg=cur!=null&&ref!=null&&ref!==0?(cur/ref-1)*100:null;const a={WatchID:m.WatchID||"",Code:m.Code||"",AlertDate:asOf,AlertType:type,TriggerReason:reason,AlertSeverity:severity,InvestmentStatus:m.InvestmentStatus||"",ActionableFlag:m.ActionableFlag||"",CurrentPrice:cur??"",ReferencePrice:ref??"",PriceChangePct:chg??"",CurrentPER:wNum(row?.ForecastPER)??"",CurrentPBR:wNum(row?.PBR)??"",DividendYield:wNum(row?.ForecastDividendYieldPct)??"",FactorState:factor?.Phase||"",SeasonState:factor?.SeasonState||"",SeasonalAlert:factor?.SeasonalAlert||"",TechnicalState:tech||"",DaysToEarnings:row?.DaysToNextEarnings??"",WatchReason:m.WatchReason||"",CurrentDecision:m.CurrentDecision||"",ThesisSummary:m.ThesisSummary||"",InvalidationSummary:m.InvalidationSummary||"",ReviewExpiry:m.ReviewExpiry||""};a.AlertKey=`${asOf}|${a.WatchID}|${type}|${reason}`;return a}
function wEvaluateWatchlist(masterRows,stateRows,screeningRows,factorRows,asOf,opts={}){const master=wClone(masterRows)||[],states=new Map((wClone(stateRows)||[]).map(r=>[String(r.WatchID||""),r])),screen=new Map((screeningRows||[]).map(r=>[normalizeInvestmentCode(r.NormalizedCode||r.Code),r])),factors=new Map((factorRows||[]).map(r=>[String(r.FactorKey||""),r])),alerts=[],diag=[];const migrationBaseline=!!opts.migrationBaseline;
 for(const m of master){if(String(m.Status||"")!=="Active")continue;const code=normalizeInvestmentCode(m.Code),row=screen.get(code),st=states.get(String(m.WatchID||""))||Object.fromEntries(WATCHLIST_STATE_FIELDS.map(k=>[k,""]));st.WatchID=m.WatchID||"";st.Code=code;if(!row){diag.push({WatchID:m.WatchID||"",Code:code,Status:m.Status||"",ScreeningRowFound:0,MigrationBaseline:migrationBaseline?1:0,SuppressedReason:"ScreeningRowMissing",AlertTypes:"",AlertCount:0});states.set(String(m.WatchID||""),st);continue}
  const sec=String(row.Sector33||"").trim(),fk=sec?`Sector:${sec}`:"",factor=factors.get(fk)||{},tech=wTechnicalState(row,m.CurrentDecision),cur=wNum(row.Close),ref=wNum(m.ReferencePrice),chg=cur!=null&&ref!=null&&ref!==0?(cur/ref-1)*100:null,prevPrice=Number(st.LastPriceAlertLevel||0)||0,level=wPriceAlertLevel(chg,m),local=[];if(level>prevPrice){const t=wNum(m[`PriceTriggerPct${level}`]);local.push(wAlert(m,row,asOf,"Price",`ReferencePrice比 ${chg.toFixed(2)}% / level${level}(${t!=null?t:"?"}%)`,String(level),factor,tech));st.LastPriceAlertLevel=String(level);st.LastAlertDatePrice=asOf}
  const vr=[];const per=wNum(row.ForecastPER),pbr=wNum(row.PBR),yld=wNum(row.ForecastDividendYieldPct),tper=wNum(m.TargetPER),tpbr=wNum(m.TargetPBR),tyld=wNum(m.TargetDividendYieldPct);if(tper!=null&&per!=null&&per<=tper&&String(st.LastPERTriggered||"")!=="1"){vr.push(`PER<=${tper}`);st.LastPERTriggered="1"}if(tpbr!=null&&pbr!=null&&pbr<=tpbr&&String(st.LastPBRTriggered||"")!=="1"){vr.push(`PBR<=${tpbr}`);st.LastPBRTriggered="1"}if(tyld!=null&&yld!=null&&yld>=tyld&&String(st.LastYieldTriggered||"")!=="1"){vr.push(`DividendYield>=${tyld}%`);st.LastYieldTriggered="1"}if(vr.length){local.push(wAlert(m,row,asOf,"Valuation",vr.join("; "),"ReEvaluation",factor,tech));st.LastAlertDateValuation=asOf}
  const fp=wFinancialFingerprint(row),oldFp=String(st.LastFinancialFingerprint||""),financialChanged=!!oldFp&&fp!==oldFp;if(financialChanged){local.push(wAlert(m,row,asOf,"Fundamental","決算/業績予想/配当等の基礎情報が前回観測から変化","ReEvaluation",factor,tech));st.LastAlertDateFundamental=asOf}st.LastFinancialFingerprint=fp;st.LastDisclosureDate=row.LatestDisclosureDate||row.LatestFinancialDisclosureDate||"";
  const fstate=String(factor.Phase||""),season=String(factor.SeasonalAlert||""),oldFactor=String(st.LastFactorState||""),oldSeason=String(st.LastSeasonalAlert||""),factorReason=[];if(fstate&&["Emerging","Strong"].includes(fstate)&&fstate!==oldFactor)factorReason.push(`SectorFactor=${fstate}`);if(season&&season!==oldSeason)factorReason.push(season);let suppressed="";if(factorReason.length&&migrationBaseline)suppressed="Factor/Seasonal transition suppressed once for Web-first engine baseline";else if(factorReason.length){local.push(wAlert(m,row,asOf,"Factor",factorReason.join("; "),"ReEvaluation",factor,tech));st.LastAlertDateFactor=asOf}st.LastFactorState=fstate;st.LastSeasonalAlert=season;
  const oldTech=String(st.LastTechnicalState||"");if(["PullbackReady","BreakoutReady","Improving"].includes(tech)&&tech!==oldTech){local.push(wAlert(m,row,asOf,"Technical",tech,"ReEvaluation",factor,tech));st.LastAlertDateTechnical=asOf}st.LastTechnicalState=tech;
  const dip=wNum(m.BuyOnDipPrice),brk=wNum(m.BreakoutPrice),dipNow=cur!=null&&dip!=null&&cur<=dip,brkNow=cur!=null&&brk!=null&&cur>=brk,prevDip=String(st.LastDipTriggered||"")==="1",prevBrk=String(st.LastBreakoutTriggered||"")==="1";if(dipNow&&!prevDip)local.push(wAlert(m,row,asOf,"PriceCondition",`押し目価格到達: ${dip}円（${m.BuyOnDipBasis||"根拠未記録"}）`,"ReEvaluation",factor,tech));if(brkNow&&!prevBrk)local.push(wAlert(m,row,asOf,"PriceCondition",`上抜け価格到達: ${brk}円（${m.BreakoutBasis||"根拠未記録"}）`,"ReEvaluation",factor,tech));st.LastDipTriggered=dipNow?"1":"0";st.LastBreakoutTriggered=brkNow?"1":"0";
  const days=wNum(row.DaysToNextEarnings);let catalyst=null,prevCatalyst=wNum(st.LastCatalystLevel);if(days!=null&&days>=0){const reached=wParseLeads(m.EarningsLeadDays).filter(n=>days<=n);catalyst=reached.length?Math.min(...reached):null;if(catalyst!=null&&(prevCatalyst==null||catalyst<prevCatalyst)){local.push(wAlert(m,row,asOf,"Catalyst",`決算まで${Math.trunc(days)}日（${catalyst}日閾値）`,"ReEvaluation",factor,tech));st.LastCatalystLevel=String(catalyst);st.LastAlertDateCatalyst=asOf}}
...
if($("watchlistAlertExportBtn"))$("watchlistAlertExportBtn").onclick=()=>{const p=latestWatchlistAlertPreview;if(!p)return;downloadBlob(new Blob([simpleCsv(p.alerts,WATCHLIST_ALERT_FIELDS)],{type:"text/csv;charset=utf-8"}),`web_reevaluation_alerts_${p.asOf.replaceAll("-","")}.csv`)};
if($("watchlistAlertDiagExportBtn"))$("watchlistAlertDiagExportBtn").onclick=()=>{const p=latestWatchlistAlertPreview;if(!p)return;downloadBlob(new Blob([simpleCsv(p.diag,WATCHLIST_ALERT_DIAG_FIELDS)],{type:"text/csv;charset=utf-8"}),`web_watchlist_alert_trace_${p.asOf.replaceAll("-","")}.csv`)};
const WATCHLIST_CONDITION_INPUT_FIELDS=["Code","InvestmentStatus","BuyOnDipPrice","BuyOnDipBasis","BreakoutPrice","BreakoutBasis","DiscoveryEpisodeId","InvestmentThesisMemo","ConditionUpdatedAt"];
const WATCHLIST_CONDITION_COMPLETE_FIELDS=["Code","CompanyName","InvestmentStatus","ReferencePrice","LatestClose","BuyOnDipPrice","BuyOnDipBasis","BreakoutPrice","BreakoutBasis","DistanceToDipPct","DistanceToBreakoutPct","DipTriggered","BreakoutTriggered","NextEarningsDate","DaysToNextEarnings","AttentionStatus","AttentionReason","ConditionUpdatedAt","DiscoveryEpisodeId","InvestmentThesisMemo"];
function watchNum(v){if(v==null||String(v).trim()==="")return null;const n=Number(v);return Number.isFinite(n)?n:null}
function watchDateFromEpisodeId(id){const m=String(id||"").match(/^(\d{4})(\d{2})(\d{2})-/);return m?`${m[1]}-${m[2]}-${m[3]}`:""}
function auditWatchlistConditionsV2(rows){const discovery=[],watchlist=[],conditions=[],warnings=[],errors=[],seen=new Set();if(!rows.length)return{discovery,watchlist,conditions,warnings,errors:["CSVが空です"]};const keys=Object.keys(rows[0]||{}),missing=WATCHLIST_CONDITION_INPUT_FIELDS.filter(k=>!keys.includes(k)),extra=keys.filter(k=>!WATCHLIST_CONDITION_INPUT_FIELDS.includes(k));if(missing.length)errors.push(`必須列不足: ${missing.join(" / ")}`);if(extra.length)errors.push(`未対応列: ${extra.join(" / ")}（v2の9列だけにしてください）`);for(let i=0;i<rows.length;i++){const r=rows[i],line=i+2,code=normalizeInvestmentCode(r.Code),status=String(r.InvestmentStatus||"").trim().toUpperCase(),dip=watchNum(r.BuyOnDipPrice),brk=watchNum(r.BreakoutPrice),updated=String(r.ConditionUpdatedAt||"").slice(0,10),episodeId=String(r.DiscoveryEpisodeId||"").trim();let bad=false;if(!code){errors.push(`${line}行目: Codeが空欄`);continue}if(seen.has(code)){errors.push(`${line}行目: Code=${code} が重複`);continue}seen.add(code);if(!INVESTMENT_TRACKING_STATUSES.has(status)){errors.push(`${line}行目 Code=${code}: InvestmentStatus=${status||"(blank)"} は未対応`);bad=true}if(!(dip>0)){errors.push(`${line}行目 Code=${code}: BuyOnDipPrice は0より大きい数値が必要`);bad=true}if(!(brk>0)){errors.push(`${line}行目 Code=${code}: BreakoutPrice は0より大きい数値が必要`);bad=true}if(!String(r.BuyOnDipBasis||"").trim()){errors.push(`${line}行目 Code=${code}: BuyOnDipBasis が空欄`);bad=true}if(!String(r.BreakoutBasis||"").trim()){errors.push(`${line}行目 Code=${code}: BreakoutBasis が空欄`);bad=true}if(!String(r.InvestmentThesisMemo||"").trim()){errors.push(`${line}行目 Code=${code}: InvestmentThesisMemo が空欄`);bad=true}if(!/^\d{4}-\d{2}-\d{2}$/.test(updated)){errors.push(`${line}行目 Code=${code}: ConditionUpdatedAt はYYYY-MM-DDが必要`);bad=true}if(bad)continue;const base={Code:code,InvestmentStatus:status,BuyOnDipPrice:dip,BuyOnDipBasis:String(r.BuyOnDipBasis||"").trim(),BreakoutPrice:brk,BreakoutBasis:String(r.BreakoutBasis||"").trim(),DiscoveryEpisodeId:episodeId,InvestmentThesisMemo:String(r.InvestmentThesisMemo||"").trim(),ConditionUpdatedAt:updated};conditions.push(base);if(["TRACK_ONLY","WATCH","ACTIONABLE"].includes(status))discovery.push({...base,EpisodeStartDate:watchDateFromEpisodeId(episodeId)||updated,EventID:episodeId,PreferExistingEpisode:"1",AnalystDecision:{TRACK_ONLY:"TrackOnly",WATCH:"Watch",ACTIONABLE:"Actionable"}[status],SelectionType:"WatchlistConditionV2",AnalystNote:base.InvestmentThesisMemo,EpisodeStartReason:`WatchlistConditionV2:${status}`});if(["WATCH","ACTIONABLE","WATCH_ONLY"].includes(status))watchlist.push({...base,Action:"UPSERT",WatchStartDate:updated,WatchReason:base.InvestmentThesisMemo,CurrentDecision:status==="ACTIONABLE"?"Actionable":"Watch",ThesisSummary:base.InvestmentThesisMemo,InvalidationSummary:"価格条件到達時に投資仮説を再評価",EarningsLeadDays:"14;7;3",ActionableFlag:status==="ACTIONABLE"?"1":"0"})}return{discovery,watchlist,conditions,warnings,errors}}
function watchConditionCalc(cond,price,earn){const close=watchNum(price?.close),dip=watchNum(cond?.BuyOnDipPrice),brk=watchNum(cond?.BreakoutPrice),dDip=close!=null&&dip>0?(close/dip-1)*100:null,dBrk=close!=null&&brk>0?(brk/close-1)*100:null,dipTrig=close!=null&&dip>0&&close<=dip,brkTrig=close!=null&&brk>0&&close>=brk,days=watchNum(earn?.DaysToNextEarnings),reasons=[];if(dipTrig)reasons.push("押し目価格到達");if(brkTrig)reasons.push("上抜け価格到達");if(days!=null&&days>=0&&days<=14)reasons.push(`決算まで${Math.trunc(days)}日`);return{LatestClose:close,DistanceToDipPct:dDip,DistanceToBreakoutPct:dBrk,DipTriggered:dipTrig?"TRUE":"FALSE",BreakoutTriggered:brkTrig?"TRUE":"FALSE",NextEarningsDate:earn?.NextEarningsDate||"",DaysToNextEarnings:days??"",AttentionStatus:reasons.length?"ATTENTION":"NORMAL",AttentionReason:reasons.join(" / ")}}
async function buildWatchlistConditionsComplete(asOf,st=null){st=st||await workerCall("investment-tracking-state-load",120000);const latestD=new Map();for(const r of st.discovery||[]){const c=normalizeInvestmentCode(r.Code),d=String(r.EpisodeStartDate||r.DiscoveryDate||"");if(!c)continue;const old=latestD.get(c);if(!old||String(old.EpisodeStartDate||old.DiscoveryDate||"")<d)latestD.set(c,r)}const activeW=(st.master||[]).filter(r=>["Active","ReviewDue"].includes(String(r.Status||""))),activeWm=new Map(activeW.map(r=>[normalizeInvestmentCode(r.Code),r])),activeD=new Map([...latestD].filter(([c,r])=>String(r.PerfEpisodeStatus||"").toLowerCase()!=="closed"&&String(r.PerfActiveWatchFlag||"")!=="0")),codes=[...new Set([...activeD.keys(),...activeWm.keys()])].sort();let names=new Map(),prices=new Map(),earn=new Map();if(codes.length){try{const nm=await workerCall("equities-master-names",120000,null,null,{codes});names=new Map((nm.rows||[]).map(r=>[normalizeInvestmentCode(r.code),r.company_name||""]))}catch(_){}try{const pr=await workerCall("portfolio-latest-prices",300000,null,null,{asOf,codes});prices=new Map((pr.rows||[]).map(r=>[normalizeInvestmentCode(r.code),r]))}catch(_){}try{const er=await workerCall("earnings-date-next",120000,null,null,{asOf,codes});earn=new Map((er.rows||[]).map(r=>[normalizeInvestmentCode(r.Code),r]))}catch(_){}}return codes.map(code=>{const d=activeD.get(code),w=activeWm.get(code),cond=w||d||{},calc=watchConditionCalc(cond,prices.get(code),earn.get(code)),ref=watchNum(w?.ReferencePrice)??watchNum(d?.InitialPrice)??"";return{Code:code,CompanyName:names.get(code)||"",InvestmentStatus:cond.InvestmentStatus||d?.InvestmentStatus||"",ReferencePrice:ref,...calc,BuyOnDipPrice:cond.BuyOnDipPrice??"",BuyOnDipBasis:cond.BuyOnDipBasis||"",BreakoutPrice:cond.BreakoutPrice??"",BreakoutBasis:cond.BreakoutBasis||"",ConditionUpdatedAt:cond.ConditionUpdatedAt||"",DiscoveryEpisodeId:cond.DiscoveryEpisodeId||d?.EventID||"",InvestmentThesisMemo:cond.InvestmentThesisMemo||cond.ThesisSummary||d?.AnalystNote||""}})}
function auditInvestmentTracking(rows){const discovery=[],watchlist=[],warnings=[],errors=[],seen=new Set(),counts={};
 for(let i=0;i<rows.length;i++){const r=rows[i],line=i+2,code=normalizeInvestmentCode(r.Code),action=String(r.Action||"").trim().toUpperCase(),status=String(r.InvestmentStatus||"").trim().toUpperCase();if(!code){errors.push(`${line}行目: Codeが空欄`);continue}if(seen.has(code)){errors.push(`${line}行目: Code=${code} が同一CSV内で重複`);continue}seen.add(code);if(!INVESTMENT_TRACKING_ACTIONS.has(action)){errors.push(`${line}行目 Code=${code}: Action=${action||"(blank)"} は未対応`);continue}if(!INVESTMENT_TRACKING_STATUSES.has(status)){errors.push(`${line}行目 Code=${code}: InvestmentStatus=${status||"(blank)"} は未対応`);continue}counts[status]=(counts[status]||0)+1;
  if(!["REMOVE","CLOSE"].includes(action)&&!/^\d{4}-\d{2}-\d{2}$/.test(String(r.StartDate||"").slice(0,10))){errors.push(`${line}行目 Code=${code}: StartDate(YYYY-MM-DD)が必要`);continue}
  if(["REMOVE","CLOSE"].includes(action)){if(status==="TRACK_ONLY")warnings.push(`Code=${code}: TRACK_ONLY+${action} はDiscovery履歴を削除しないため処理対象なし`);else{if(["WATCH","ACTIONABLE"].includes(status))warnings.push(`Code=${code}: ${status}+${action} はWatchlistのみ閉鎖しDiscovery履歴を保持`);watchlist.push({Action:action,Code:code,InvestmentStatus:status,ActionableFlag:status==="ACTIONABLE"?"1":"0"})}continue}
  const needD=["TRACK_ONLY","WATCH","ACTIONABLE"].includes(status),needW=["WATCH","ACTIONABLE","WATCH_ONLY"].includes(status);if(needD){const miss=INVESTMENT_DISC_REQUIRED.filter(k=>!String(r[k]??"").trim());if(miss.length){errors.push(`${line}行目 Code=${code}: Discovery必須値不足 ${miss.join("/")}`);continue}}if(needW){const miss=INVESTMENT_WATCH_REQUIRED.filter(k=>!String(r[k]??"").trim());if(miss.length){errors.push(`${line}行目 Code=${code}: Watchlist必須値不足 ${miss.join("/")}`);continue}}
  if(needD){const decision={TRACK_ONLY:"TrackOnly",WATCH:"Watch",ACTIONABLE:"Actionable"}[status],o={Code:code,EpisodeStartDate:String(r.StartDate||"").slice(0,10),AnalystDecision:decision,SelectionType:r.SelectionType||"",Conviction:r.Conviction||"",ExpectedHorizon:r.ExpectedHorizon||"",PrimaryCatalyst:r.PrimaryCatalyst||"",PrimaryRisk:r.PrimaryRisk||"",AnalystNote:r.AnalystNote||"",EpisodeStartReason:`UnifiedImport:${status}`,PrimaryStrategy:r.PrimaryStrategy||"",InvestmentStatus:status};discovery.push(o)}
  if(needW){watchlist.push({Action:action,Code:code,WatchStartDate:String(r.StartDate||"").slice(0,10),WatchReason:r.WatchReason||"",CurrentDecision:r.CurrentDecision||"",ReferencePrice:r.ReferencePrice||"",ThesisSummary:r.ThesisSummary||"",InvalidationSummary:r.InvalidationSummary||"",ReviewExpiry:String(r.ReviewExpiry||"").slice(0,10),PriceTriggerPct1:r.PriceTriggerPct1||"",PriceTriggerPct2:r.PriceTriggerPct2||"",PriceTriggerPct3:r.PriceTriggerPct3||"",TargetPER:r.TargetPER||"",TargetPBR:r.TargetPBR||"",TargetDividendYieldPct:r.TargetDividendYieldPct||"",EarningsLeadDays:r.EarningsLeadDays||"",Notes:r.AnalystNote||"",InvestmentStatus:status,ActionableFlag:status==="ACTIONABLE"?"1":"0",InputReferencePrice:r.ReferencePrice||"",ReferencePriceDate:"",ReferencePriceSource:"PendingWebDataLakeResolve",ReferencePriceAudit:"PendingResolve"})}
 }
 return{discovery,watchlist,warnings,errors,counts}}
const INVESTMENT_APPLY_FIELDS=["RouteType","Action","Operation","Code","InvestmentStatus","StartDate","ResolvedPrice","ResolvedDate","ReferencePriceSource","InputReferencePrice","ReferencePriceAudit","TargetID","PreviousID","Warning"];
function renderInvestmentPreview(r,mode="PREVIEW"){const cls=r.errorCount?"fail":r.warningCount?"warn":"pass";box("investmentTrackingResult",cls,[`${mode} ${r.commitReady!==false?"READY":"BLOCKED"}`,`基準日: ${r.asOf||"-"}`,`Discovery: CREATE ${r.discoveryCreate||0} / UPDATE ${r.discoveryUpdate||0}`,`Watchlist: CREATE ${r.watchCreate||0} / UPDATE ${r.watchUpdate||0} / CLOSE ${r.watchClose||0} / REREGISTER ${r.watchReregister||0}`,`ReferencePrice解決: ${r.referenceResolved||0}`,`Warning: ${r.warningCount||0} / Error: ${r.errorCount||0}`,r.warnings?.length?"\nWarnings:\n"+r.warnings.join("\n"):"",r.errors?.length?"\nErrors:\n"+r.errors.join("\n"):"",mode==="PREVIEW"?"\n※ PREVIEWはprivate state未更新。ReferencePrice/Discovery InitialPriceはWeb DataLakeのStartDate終値（休場日は次取引日終値）を正とします。":""].join("\n"));const esc=x=>String(x??"").replace(/[&<>\"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]));const rows=(r.plan||[]).slice(0,100);if($("investmentTrackingTable"))$("investmentTrackingTable").innerHTML=rows.length?`<div style="overflow:auto"><table style="width:100%;font-size:12px"><tr><th>Route</th><th>Code</th><th>Operation</th><th>Reference</th></tr>${rows.map(x=>`<tr><td>${esc(x.RouteType)}</td><td>${esc(x.Code)}</td><td>${esc(x.Operation)}</td><td>${esc(x.ResolvedDate)} ${esc(x.ResolvedPrice)}<br>${esc(x.ReferencePriceAudit||"")}</td></tr>`).join("")}</table></div>`:""}
if($("investmentTrackingAuditBtn"))$("investmentTrackingAuditBtn").onclick=async()=>{const f=$("investmentTrackingFile")?.files?.[0];if(!f){box("investmentTrackingResult","warn","investment_tracking_input.csv を選択してください。");return}try{const rows=parseCsv(await f.text()).rows.filter(r=>Object.values(r).some(v=>String(v??"").trim())),r=auditInvestmentTracking(rows);latestInvestmentRoutes={discovery:r.discovery,watchlist:r.watchlist};latestInvestmentPreview=null;$("investmentTrackingDiscoveryExportBtn").disabled=!r.discovery.length;$("investmentTrackingWatchlistExportBtn").disabled=!r.watchlist.length;if($("investmentTrackingPreviewBtn"))$("investmentTrackingPreviewBtn").disabled=!!r.errors.length;$("investmentTrackingCommitBtn").disabled=true;$("investmentTrackingApplyExportBtn").disabled=true;const cls=r.errors.length?"fail":"pass";box("investmentTrackingResult",cls,[r.errors.length?"検証FAIL":"ルーティング監査 PASS",`入力: ${rows.length}`,`Discoveryへ: ${r.discovery.length}`,`Watchlistへ: ${r.watchlist.length}`,`Status: ${Object.entries(r.counts).map(([k,v])=>k+"="+v).join(" / ")||"-"}`,`Warning: ${r.warnings.length} / Error: ${r.errors.length}`,r.warnings.length?"\nWarnings:\n"+r.warnings.join("\n"):"",r.errors.length?"\nErrors:\n"+r.errors.join("\n"):"","\n次に適用プレビューでReferencePriceとCREATE/UPDATE/CLOSEを解決します。"].join("\n"))}catch(e){box("investmentTrackingResult","fail","FAIL\n"+(e?.message||e))}};
if($("investmentTrackingPreviewBtn"))$("investmentTrackingPreviewBtn").onclick=async()=>{if(!latestInvestmentRoutes.discovery.length&&!latestInvestmentRoutes.watchlist.length){box("investmentTrackingResult","warn","先に共通入力を検証してください。");return}const btn=$("investmentTrackingPreviewBtn");btn.disabled=true;try{const asOf=$("investmentTrackingAsOf")?.value||todayIsoLocal(),r=await workerCall("investment-tracking-preview",300000,s=>box("investmentTrackingResult","run",`Investment Tracking PREVIEW計算中…\n${s.stage||""}`),null,{asOf,discovery:latestInvestmentRoutes.discovery,watchlist:latestInvestmentRoutes.watchlist});latestInvestmentPreview=r;$("investmentTrackingCommitBtn").disabled=!r.commitReady;$("investmentTrackingApplyExportBtn").disabled=!r.plan?.length;renderInvestmentPreview(r,"PREVIEW")}catch(e){latestInvestmentPreview=null;$("investmentTrackingCommitBtn").disabled=true;box("investmentTrackingResult","fail","PREVIEW FAIL\n"+(e?.message||e))}finally{btn.disabled=false}};
if($("investmentTrackingCommitBtn"))$("investmentTrackingCommitBtn").onclick=async()=>{const p=latestInvestmentPreview;if(!p?.commitReady)return;const btn=$("investmentTrackingCommitBtn");btn.disabled=true;try{const r=await workerCall("investment-tracking-commit",300000,null,null,{asOf:p.asOf,discovery:latestInvestmentRoutes.discovery,watchlist:latestInvestmentRoutes.watchlist});latestInvestmentPreview={...r};latestWatchlistWeb={master:r.master||[],state:r.state||[]};let recalcNote="";if((r.discoveryCreate||0)+(r.discoveryUpdate||0)>0){try{const dr=await workerCall("discovery-recalc",600000,s=>box("investmentTrackingResult","run",`COMMIT済み。Discoveryを再計算中…\n${s.stage||""}`),null,{asOf:r.asOf});latestDiscoveryWebRows=dr.rows||[];recalcNote=` / Discovery再計算 ${dr.count||latestDiscoveryWebRows.length} Episodes`}catch(e){recalcNote=` / Discovery再計算は要再実行: ${e?.message||e}`}}renderInvestmentPreview(r,"COMMIT PASS");box("investmentTrackingResult","pass",[$("investmentTrackingResult").textContent,`\nImportID: ${r.importId||"-"}${recalcNote}`,"\nWatchlist Alertは必要に応じて⑧-3でPreviewしてください。"].join("\n"));latestInvestmentPreview=null;btn.disabled=true}catch(e){box("investmentTrackingResult","fail","COMMIT FAIL\n"+(e?.message||e));btn.disabled=false}};
if($("investmentTrackingApplyExportBtn"))$("investmentTrackingApplyExportBtn").onclick=()=>{const p=latestInvestmentPreview;if(!p?.plan?.length)return;downloadBlob(new Blob([simpleCsv(p.plan,INVESTMENT_APPLY_FIELDS)],{type:"text/csv;charset=utf-8"}),`web_investment_tracking_apply_trace_${String(p.asOf||todayIsoLocal()).replaceAll("-","")}.csv`)};
...
// v7e-beta2: daily-use direct Investment Tracking + production UI architecture cleanup.
...
async function quickInvCanonicalDate(){try{const c=await resolveCanonicalShareAsOf();return c.asOf}catch(_){return todayIsoLocal()}}
function jstTodayIso(){try{return new Intl.DateTimeFormat("sv-SE",{timeZone:"Asia/Tokyo",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date())}catch(_){return todayIsoLocal()}}
let pfTradeDateManual=false;
function pfSyncTradeDateToJst(force=false){const el=$("pfTradeDate");if(!el)return;const today=jstTodayIso();if(force||!pfTradeDateManual||!el.value)el.value=today}
window.addEventListener("jq:page-activate",e=>{if(e.detail?.page==="stocks")pfSyncTradeDateToJst(false)});
function quickInvBuildRow(){const action=quickInvValue("quickInvAction"),status=quickInvValue("quickInvStatus"),closing=action==="CLOSE";return{Action:action,Code:quickInvValue("quickInvCode"),InvestmentStatus:status,StartDate:closing?"":quickInvValue("quickInvStartDate"),ReferencePrice:"",PrimaryStrategy:quickInvValue("quickInvPrimaryStrategy"),SelectionType:quickInvValue("quickInvSelectionType"),Conviction:quickInvValue("quickInvConviction"),ExpectedHorizon:quickInvValue("quickInvExpectedHorizon"),WatchReason:quickInvValue("quickInvWatchReason"),CurrentDecision:quickInvValue("quickInvCurrentDecision"),ThesisSummary:quickInvValue("quickInvThesisSummary"),InvalidationSummary:quickInvValue("quickInvInvalidationSummary"),PrimaryCatalyst:quickInvValue("quickInvPrimaryCatalyst"),PrimaryRisk:quickInvValue("quickInvPrimaryRisk"),PriceTriggerPct1:quickInvValue("quickInvPrice1"),PriceTriggerPct2:quickInvValue("quickInvPrice2"),PriceTriggerPct3:quickInvValue("quickInvPrice3"),ReviewExpiry:quickInvValue("quickInvReviewExpiry"),AnalystNote:""}}
function quickInvRenderPreview(r,mode="PREVIEW"){const cls=r.errorCount?"fail":r.warningCount?"warn":"pass";box("quickInvResult",cls,[`${mode} ${r.commitReady!==false?"READY":"BLOCKED"}`,`基準日: ${r.asOf||"-"}`,`Discovery: CREATE ${r.discoveryCreate||0} / UPDATE ${r.discoveryUpdate||0}`,`Watchlist: CREATE ${r.watchCreate||0} / UPDATE ${r.watchUpdate||0} / CLOSE ${r.watchClose||0} / REREGISTER ${r.watchReregister||0}`,`ReferencePrice解決: ${r.referenceResolved||0}`,`Warning: ${r.warningCount||0} / Error: ${r.errorCount||0}`,...(r.warnings||[]).map(x=>"WARN: "+x),...(r.errors||[]).map(x=>"ERROR: "+x),mode==="PREVIEW"?"\n※ state未更新。内容確認後に確定してください。":""].join("\n"))}
function quickInvEsc(x){return String(x??"").replace(/[&<>\"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]))}
async function quickInvRenderState(st){
 latestQuickInvestmentState=st||{discovery:[],master:[],state:[]};
 const latestD=new Map();for(const r of st.discovery||[]){const c=normalizeInvestmentCode(r.Code),d=String(r.EpisodeStartDate||r.DiscoveryDate||"");if(!c)continue;const old=latestD.get(c);if(!old||String(old.EpisodeStartDate||old.DiscoveryDate||"")<d)latestD.set(c,r)}
 const allW=(st.master||[]),activeW=allW.filter(r=>["Active","ReviewDue"].includes(String(r.Status||""))),activeWm=new Map();for(const r of activeW)activeWm.set(normalizeInvestmentCode(r.Code),r);
 const discoveryIsActive=r=>!!r&&String(r.PerfEpisodeStatus||"").toLowerCase()!=="closed"&&String(r.PerfActiveWatchFlag||"")!=="0",activeD=new Map([...latestD].filter(([c,r])=>discoveryIsActive(r)));
 const activeCodes=[...new Set([...activeD.keys(),...activeWm.keys()])].sort(),allCodes=[...new Set([...activeCodes,...latestD.keys(),...allW.map(r=>normalizeInvestmentCode(r.Code))].filter(Boolean))];
 let names=new Map();try{const nm=await workerCall("equities-master-names",120000,null,null,{codes:allCodes});for(const r of nm.rows||[])names.set(normalizeInvestmentCode(r.code),r.company_name||"")}catch(_){}
 const asOf=await quickInvCanonicalDate();let prices=new Map(),earnings=new Map();try{const pr=await workerCall("portfolio-latest-prices",300000,null,null,{asOf,codes:allCodes});prices=new Map((pr.rows||[]).map(r=>[normalizeInvestmentCode(r.code),r]))}catch(_){}try{const er=await workerCall("earnings-date-next",120000,null,null,{asOf,codes:allCodes});earnings=new Map((er.rows||[]).map(r=>[normalizeInvestmentCode(r.Code),r]))}catch(_){}
 const esc=quickInvEsc,card=x=>{const status=x.w?.InvestmentStatus||x.d?.InvestmentStatus||x.d?.AnalystDecision||"TRACK",watch=x.w?.Status||"-",watchActive=["Active","ReviewDue"].includes(String(watch)),discoveryActive=discoveryIsActive(x.d),cond=x.w||x.d||{},refW=Number(x.w?.ReferencePrice),refD=Number(x.d?.InitialPrice),ref=Number.isFinite(refW)&&refW>0?refW:(Number.isFinite(refD)&&refD>0?refD:NaN),close=Number(x.p?.close),chg=Number.isFinite(ref)&&ref>0&&Number.isFinite(close)?(close/ref-1)*100:null,calc=x._calc||watchConditionCalc(cond,x.p,x.e),dip=watchNum(cond.BuyOnDipPrice),brk=watchNum(cond.BreakoutPrice);return `<div class="inv-item"><div class="inv-title">${esc(x.code)}　${esc(x.name||"銘柄名未取得")}</div><div class="watch-badges">${calc.AttentionStatus==="ATTENTION"?`<span class="watch-badge attention-badge">⚠ 要注意</span>`:''}<span class="watch-badge status">${esc(status)}</span>${x.d?`<span class="watch-badge ${discoveryActive?'active':'inactive'}">${discoveryActive?'● Discovery Active':'Discovery Closed'}</span>`:''}${x.w?`<span class="watch-badge ${watchActive?'active':'inactive'}">${watchActive?'● Watch Active':esc(watch)}</span>`:''}</div><div class="inv-meta">登録日: ${esc(x.w?.WatchStartDate||x.d?.EpisodeStartDate||x.d?.DiscoveryDate||"-")}<br>登録基準価格: ${Number.isFinite(ref)?pfMoney(ref)+'円':'未記録'}<br>最新終値: ${Number.isFinite(close)?pfMoney(close)+'円':'価格なし'}${x.p?.date?` <span class="price-date">${esc(x.p.date)}</span>`:''}<br>登録時比: <b class="${chg==null?'':(chg>=0?'pnl-plus':'pnl-minus')}">${chg==null?'-':pfPct(chg)}</b><hr style="border:0;border-top:1px solid #e5e7eb;margin:8px 0"><b>押し目:</b> ${dip?pfMoney(dip)+'円':'-'} ${calc.DipTriggered==='TRUE'?'<b>【到達】</b>':calc.DistanceToDipPct!=null?`（現在値は${pfPct(calc.DistanceToDipPct)}上）`:''}<br><span class="desc">${esc(cond.BuyOnDipBasis||"")}</span><br><b>上抜け:</b> ${brk?pfMoney(brk)+'円':'-'} ${calc.BreakoutTriggered==='TRUE'?'<b>【到達】</b>':calc.DistanceToBreakoutPct!=null?`（あと${pfPct(calc.DistanceToBreakoutPct)}）`:''}<br><span class="desc">${esc(cond.BreakoutBasis||"")}</span>${calc.NextEarningsDate?`<br><b>次回決算:</b> ${esc(calc.NextEarningsDate)}（${esc(calc.DaysToNextEarnings)}日）`:''}${calc.AttentionReason?`<br><b>要注目理由:</b> ${esc(calc.AttentionReason)}`:''}${cond.ConditionUpdatedAt?`<br><b>条件更新:</b> ${esc(cond.ConditionUpdatedAt)}`:''}${cond.InvestmentThesisMemo?`<br><b>投資仮説:</b> ${esc(cond.InvestmentThesisMemo)}`:''}</div><div class="inv-actions">${watchActive?`<button class="watch-remove quickInvCloseScope" data-code="${esc(x.code)}" data-scope="watch">Watch終了</button>`:''}${discoveryActive?`<button class="watch-remove quickInvCloseScope" data-code="${esc(x.code)}" data-scope="discovery">Discovery終了</button>`:''}</div></div>`};
 const activeRows=activeCodes.map(c=>{const x={code:c,name:names.get(c)||"",d:activeD.get(c)||latestD.get(c),w:activeWm.get(c),p:prices.get(c)||{},e:earnings.get(c)||{}};x._calc=watchConditionCalc(x.w||x.d||{},x.p,x.e);return x}).sort((a,b)=>(b._calc?.AttentionStatus==="ATTENTION")-(a._calc?.AttentionStatus==="ATTENTION")||String(a.code).localeCompare(String(b.code)));$("quickInvTable").innerHTML=activeRows.length?`<div class="inv-grid">${activeRows.map(card).join("")}</div>`:"<p class='desc'>現在追跡中の銘柄はありません。</p>";
 const latestClosedW=new Map();for(const w of allW.filter(r=>!["Active","ReviewDue"].includes(String(r.Status||"")))){const c=normalizeInvestmentCode(w.Code),old=latestClosedW.get(c);if(!old||String(old.ClosedAt||"")<String(w.ClosedAt||""))latestClosedW.set(c,w)}const closed=[];for(const c of allCodes){if(activeD.has(c)||activeWm.has(c))continue;const d=latestD.get(c),w=latestClosedW.get(c);if(!d&&!w)continue;const when=[String(d?.PerfEpisodeEndDate||"").slice(0,10),String(w?.ClosedAt||"").slice(0,10),String(d?.ManualClosedAt||"").slice(0,10)].filter(Boolean).sort().at(-1)||"";closed.push({code:c,name:names.get(c)||"",d,w,when})}closed.sort((a,b)=>String(b.when).localeCompare(String(a.when)));const hist=closed.slice(0,20);if($("quickInvClosedTable"))$("quickInvClosedTable").innerHTML=hist.length?`<div class="inv-grid">${hist.map(x=>`<div class="inv-item"><div class="inv-title">${esc(x.code)}　${esc(x.name||"銘柄名未取得")}</div><div class="watch-badges"><span class="watch-badge inactive">追跡終了</span></div><div class="inv-meta">終了日: ${esc(x.when||"-")}<br>最終Status: ${esc(x.w?.InvestmentStatus||x.d?.InvestmentStatus||"-")}<br>${x.w?.CloseReason?`Watch: ${esc(x.w.CloseReason)}<br>`:""}${x.d?.PerfEpisodeEndReason?`Discovery: ${esc(x.d.PerfEpisodeEndReason)}`:""}</div></div>`).join("")}</div>`:"<p class='desc'>終了履歴はありません。</p>";
 for(const b of document.querySelectorAll(".quickInvCloseScope"))b.onclick=async()=>{const code=b.dataset.code||"",scope=b.dataset.scope||"watch",label=scope==="discovery"?"Discovery Episode":"Watchlist";if(!confirm(`${code} の${label}追跡を終了しますか？\n履歴は削除せず監査用に保持します。`))return;try{const asOf=await quickInvCanonicalDate(),r=await workerCall("investment-tracking-close-code",120000,null,null,{asOf,code,scope});box("quickInvResult","pass",`追跡終了 PASS\nCode: ${code}\n対象: ${label}\nDiscovery終了: ${r.discoveryClosed||0} / Watch終了: ${r.watchClosed||0}\n\n両方終了した銘柄は通常一覧から外れ、終了履歴（直近20件）へ移ります。`);await quickInvRefresh()}catch(e){box("quickInvResult","fail",`追跡終了 FAIL\n${e?.message||e}`)}}
}
async function quickInvRefresh(){const btn=$("quickInvRefreshBtn");if(btn)btn.disabled=true;try{const st=await workerCall("investment-tracking-state-load",120000);await quickInvRenderState(st);const d=await quickInvCanonicalDate();if($("quickInvStartDate")&&!$("quickInvStartDate").value)$("quickInvStartDate").value=d;box("quickInvResult","pass",`現在の登録一覧を読込 PASS\nDiscovery Episodes: ${st.discoveryCount||0}\nWatchlist: ${st.masterCount||0}\n基準候補日: ${d}`)}catch(e){box("quickInvResult","fail","一覧読込 FAIL\n"+(e?.message||e))}finally{if(btn)btn.disabled=false}}
if($("quickInvStatus"))$("quickInvStatus").onchange=quickInvSyncFields;if($("quickInvAction"))$("quickInvAction").onchange=quickInvSyncFields;
if($("quickInvRefreshBtn"))$("quickInvRefreshBtn").onclick=quickInvRefresh;
if($("quickInvPreviewBtn"))$("quickInvPreviewBtn").onclick=async()=>{const btn=$("quickInvPreviewBtn");btn.disabled=true;latestQuickInvestmentPreview=null;$("quickInvCommitBtn").disabled=true;try{const row=quickInvBuildRow(),audit=auditInvestmentTracking([row]);if(audit.errors.length){box("quickInvResult","fail","入力チェック FAIL\n"+audit.errors.join("\n"));return}const asOf=await quickInvCanonicalDate();latestQuickInvestmentRoutes={discovery:audit.discovery,watchlist:audit.watchlist};const r=await workerCall("investment-tracking-preview",300000,s=>box("quickInvResult","run",`登録・変更 PREVIEW計算中…\n${s.stage||""}`),null,{asOf,discovery:audit.discovery,watchlist:audit.watchlist});latestQuickInvestmentPreview=r;$("quickInvCommitBtn").disabled=!r.commitReady;quickInvRenderPreview(r,"PREVIEW")}catch(e){box("quickInvResult","fail","PREVIEW FAIL\n"+(e?.message||e))}finally{btn.disabled=false}};
if($("quickInvCommitBtn"))$("quickInvCommitBtn").onclick=async()=>{const p=latestQuickInvestmentPreview;if(!p?.commitReady)return;const btn=$("quickInvCommitBtn");btn.disabled=true;try{const r=await workerCall("investment-tracking-commit",300000,null,null,{asOf:p.asOf,discovery:latestQuickInvestmentRoutes.discovery,watchlist:latestQuickInvestmentRoutes.watchlist});let recalc="";if((r.discoveryCreate||0)+(r.discoveryUpdate||0)>0){try{const dr=await workerCall("discovery-recalc",600000,s=>box("quickInvResult","run",`COMMIT済み。Discovery再計算中…\n${s.stage||""}`),null,{asOf:r.asOf});latestDiscoveryWebRows=dr.rows||[];recalc=`\nDiscovery再計算: ${dr.count||latestDiscoveryWebRows.length} Episodes`}catch(e){recalc=`\nDiscovery再計算は要再実行: ${e?.message||e}`}}quickInvRenderPreview(r,"COMMIT PASS");box("quickInvResult","pass",$("quickInvResult").textContent+`\nImportID: ${r.importId||"-"}${recalc}\n\nDiscovery履歴は保持。Watchlist Alertは次の日次運用/Previewで再評価されます。`);latestQuickInvestmentPreview=null;await quickInvRefresh()}catch(e){box("quickInvResult","fail","COMMIT FAIL\n"+(e?.message||e));btn.disabled=false}};

window.quickInvRefresh=quickInvRefresh;
let latestQuickCsvPreview=null,latestQuickCsvRoutes={discovery:[],watchlist:[]},latestQuickCsvConditions=[];
if($("quickInvImportPreviewBtn"))$("quickInvImportPreviewBtn").onclick=async()=>{const f=$("quickInvImportFile")?.files?.[0];if(!f){box("quickInvImportResult","warn","watchlist_conditions_input_v2.csv を選択してください。");return}const btn=$("quickInvImportPreviewBtn");btn.disabled=true;$("quickInvImportCommitBtn").disabled=true;latestQuickCsvPreview=null;try{const rows=parseCsv(await f.text()).rows.filter(r=>Object.values(r).some(v=>String(v??"").trim())),audit=auditWatchlistConditionsV2(rows);if(audit.errors.length){box("quickInvImportResult","fail",["CSV検証 FAIL",`入力: ${rows.length}`,`Warning: ${audit.warnings.length} / Error: ${audit.errors.length}`,...audit.warnings.map(x=>"WARN: "+x),...audit.errors.map(x=>"ERROR: "+x)].join("\n"));return}const asOf=await quickInvCanonicalDate();latestQuickCsvRoutes={discovery:audit.discovery,watchlist:audit.watchlist};latestQuickCsvConditions=audit.conditions;const r=await workerCall("investment-tracking-preview",300000,s=>box("quickInvImportResult","run",`CSV v2 PREVIEW計算中…\n${s.stage||""}`),null,{asOf,discovery:audit.discovery,watchlist:audit.watchlist,conditions:audit.conditions,syncStatusV2:true});latestQuickCsvPreview=r;$("quickInvImportCommitBtn").disabled=!r.commitReady;box("quickInvImportResult",r.errorCount?"fail":r.warningCount?"warn":"pass",[`CSV v2 PREVIEW ${r.commitReady!==false?"READY":"BLOCKED"}`,`入力: ${rows.length}`,`基準日: ${r.asOf||"-"}`,`Discovery: CREATE ${r.discoveryCreate||0} / UPDATE ${r.discoveryUpdate||0}`,`Watchlist: CREATE ${r.watchCreate||0} / UPDATE ${r.watchUpdate||0}`,`価格条件: ${audit.conditions.length}件`,`Warning: ${r.warningCount||0} / Error: ${r.errorCount||0}`,...(r.warnings||[]).map(x=>"WARN: "+x),...(r.errors||[]).map(x=>"ERROR: "+x),"","【役割分担】","AI: 価格条件・根拠・投資仮説 / PWA: 最新終値・距離・決算日・Trigger・Attention","※まだ未保存です。内容確認後に一括確定してください。"].join("\n"))}catch(e){box("quickInvImportResult","fail","CSV v2 PREVIEW FAIL\n"+(e?.message||e))}finally{btn.disabled=false}};
if($("quickInvImportCommitBtn"))$("quickInvImportCommitBtn").onclick=async()=>{const p=latestQuickCsvPreview;if(!p?.commitReady)return;const btn=$("quickInvImportCommitBtn");btn.disabled=true;try{const r=await workerCall("investment-tracking-commit",300000,null,null,{asOf:p.asOf,discovery:latestQuickCsvRoutes.discovery,watchlist:latestQuickCsvRoutes.watchlist,conditions:latestQuickCsvConditions,syncStatusV2:true});box("quickInvImportResult","pass",[`CSV v2 COMMIT PASS`,`基準日: ${r.asOf||"-"}`,`Discovery: CREATE ${r.discoveryCreate||0} / UPDATE ${r.discoveryUpdate||0}`,`Watchlist: CREATE ${r.watchCreate||0} / UPDATE ${r.watchUpdate||0}`,`価格条件: ${latestQuickCsvConditions.length}件`,`ImportID: ${r.importId||"-"}`,"","価格条件到達は買いシグナルではなく再評価Triggerとして扱います。"].join("\n"));latestQuickCsvPreview=null;await quickInvRefresh()}catch(e){box("quickInvImportResult","fail","CSV v2 COMMIT FAIL\n"+(e?.message||e));btn.disabled=false}};
quickInvSyncFields();
let latestPfTradePreview=null,latestPfTradeVoidPreview=null;
function pfEsc(x){return String(x??"").replace(/[&<>\"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]))}
async function pfResolveName(code){try{const r=await workerCall("equities-master-names",120000,null,null,{codes:[code]});return r.rows?.[0]?.company_name||""}catch(_){return ""}}
let pfActiveAccount="全体";
function pfMoney(v){return Number.isFinite(Number(v))?Number(v).toLocaleString(undefined,{maximumFractionDigits:0}):"-"}
function pfPct(v){return Number.isFinite(Number(v))?`${Number(v)>=0?"+":""}${Number(v).toFixed(2)}%`:"-"}
async function pfTradeRefresh(){
 ...
 try{
  ...
  $('pfPortfolioSummary').innerHTML=`<div class="portfolio-summary"><div class="summary-cell"><div class="summary-label">評価額 / 建玉額</div><div class="summary-value">${pfMoney(sumMV)}円</div></div><div class="summary-cell"><div class="summary-label">評価損益</div><div class="summary-value ${sumPnl>=0?'pnl-plus':'pnl-minus'}">${sumPnl>=0?'+':''}${pfMoney(sumPnl)}円</div></div><div class="summary-cell"><div class="summary-label">損益率</div><div class="summary-value ${sumPnl>=0?'pnl-plus':'pnl-minus'}">${pfPct(sumPct)}</div></div><div class="summary-cell"><div class="summary-label">ポジション数</div><div class="summary-value">${filtered.length}</div></div></div>`;
  $("pfTradePositions").innerHTML=filtered.length?`<div class="portfolio-grid">${filtered.map(x=>`<div class="portfolio-row"><div class="portfolio-title">${pfEsc(x.code)}　${pfEsc(x.name)}</div><div class="portfolio-meta">${pfEsc(x.account)} / ${Number(x.shares||0).toLocaleString()}株</div><div class="portfolio-numbers"><span>最新終値</span><b>${x.close==null?"価格なし":pfMoney(x.close)+"円"}${x.priceDate&&x.priceDate!==asOf?`<small class="price-date"> (${pfEsc(x.priceDate)})</small>`:""}</b><span>平均取得</span><b>${pfMoney(x.avg_cost)}円</b><span>評価額</span><b>${x.marketValue==null?"-":pfMoney(x.marketValue)+"円"}</b><span>評価損益</span><b class="${x.pnl==null?'':(x.pnl>=0?'pnl-plus':'pnl-minus')}">${x.pnl==null?'-':`${x.pnl>=0?'+':''}${pfMoney(x.pnl)}円 (${pfPct(x.pnlPct)})`}</b></div><div class="portfolio-memo"><label>メモ</label><div class="portfolio-memo-row"><input class="pfMemoInput" data-code="${pfEsc(x.code)}" data-account="${pfEsc(x.account)}" maxlength="500" value="${pfEsc(x.memo||'')}" placeholder="この銘柄・口座のメモ"><button type="button" class="secondary pfMemoSave" data-code="${pfEsc(x.code)}" data-account="${pfEsc(x.account)}">保存</button><button type="button" class="secondary pfReduceRow" data-code="${pfEsc(x.code)}" data-account="${pfEsc(x.account)}">縮小/決済</button></div></div></div>`).join('')}</div>`:"<p class='desc'>この区分の保有データなし</p>";
  $("pfTradeHistory").innerHTML=(r.history||[]).length?`<div style="overflow:auto"><table style="width:100%;font-size:12px"><tr><th>日付</th><th>銘柄</th><th>区分</th><th>操作</th><th>株数</th><th>単価</th><th>実現損益</th><th></th></tr>${r.history.map(x=>`<tr><td>${pfEsc(x.trade_date)}</td><td>${pfEsc(x.code)}</td><td>${pfEsc(x.account)}</td><td>${x.status==="VOID"?"取消済":(x.action==="ADD"?"追加":"縮小")}</td><td>${Number(x.shares||0).toLocaleString()}</td><td>${Number(x.price||0).toLocaleString()}</td><td>${Number(x.realized_pnl||0).toLocaleString(undefined,{maximumFractionDigits:0})}</td><td>${x.can_void?`<button type="button" class="secondary pfVoidTrade" data-id="${pfEsc(x.trade_id)}">入力取消</button>`:`<span class="pf-void-disabled">${pfEsc(x.can_void_reason||"")}</span>`}</td></tr>`).join("")}</table></div>`:"<p class='desc'>売買履歴なし（既存保有は移行時点スナップショット）</p>";
  for(const b of document.querySelectorAll(".pfMemoSave"))b.onclick=async()=>{const code=b.dataset.code||"",account=b.dataset.account||"",input=b.closest(".portfolio-memo-row")?.querySelector(".pfMemoInput"),memo=String(input?.value||"");b.disabled=true;try{await workerCall("portfolio-memo-upsert",120000,null,null,{code,account,memo});box("pfTradeResult","pass",`${code} ${account} のメモを保存しました。`)}catch(e){box("pfTradeResult","fail","メモ保存 FAIL\n"+(e?.message||e))}finally{b.disabled=false}};for(const b of document.querySelectorAll(".pfReduceRow"))b.onclick=()=>{const code=b.dataset.code||"",account=b.dataset.account||"現物",memo=b.closest(".portfolio-row")?.querySelector(".pfMemoInput")?.value||"";$("pfTradeCode").value=code;$("pfTradeAccount").value=account;$("pfTradeAction").value="REDUCE";if($("pfTradeMemo"))$("pfTradeMemo").value=memo;$("pfTradeCode").scrollIntoView({behavior:"smooth",block:"center"})};box("pfTradeResult","pass",`保有一覧を読込 PASS\nポジション: ${r.count||0} / 表示: ${filtered.length}\n価格基準日: ${asOf||"-"} / 終値取得: ${pr.count||0}/${codes.length}\n売買履歴: ${(r.history||[]).length}`)
 }catch(e){box("pfTradeResult","fail","一覧読込 FAIL\n"+(e?.message||e))}finally{if(btn)btn.disabled=false}
}
if($("pfTradeRefreshBtn"))$("pfTradeRefreshBtn").onclick=pfTradeRefresh;
async function pfLoadMemoForIdentity(){const code=String($("pfTradeCode")?.value||"").trim().toUpperCase(),account=$("pfTradeAccount")?.value;if(!/^[0-9A-Z]{4,5}$/.test(code)||!account)return;try{const r=await workerCall("portfolio-trade-list",120000),x=(r.memos||[]).find(v=>normalizeInvestmentCode(v.code)===normalizeInvestmentCode(code)&&v.account===account)||(r.positions||[]).find(v=>normalizeInvestmentCode(v.code)===normalizeInvestmentCode(code)&&v.account===account);if($("pfTradeMemo"))$("pfTradeMemo").value=String(x?.memo||"")}catch(_){}}
if($("pfTradeNameBtn"))$("pfTradeNameBtn").onclick=async()=>{const code=String($("pfTradeCode").value||"").trim().toUpperCase();if(!/^[0-9A-Z]{4,5}$/.test(code)){if($("pfTradeNameResult"))$("pfTradeNameResult").textContent="コードを確認してください";return}const name=await pfResolveName(code);if($("pfTradeNameResult"))$("pfTradeNameResult").textContent=name&&name!=="銘柄名未取得"?`${code}：${name}`:"銘柄名を確認できません";await pfLoadMemoForIdentity()};if($("pfTradeAccount"))$("pfTradeAccount").addEventListener("change",pfLoadMemoForIdentity);if($("pfTradeCode"))$("pfTradeCode").addEventListener("blur",pfLoadMemoForIdentity);
if($("pfTradePreviewBtn"))$("pfTradePreviewBtn").onclick=async()=>{const code=String($("pfTradeCode").value||"").trim().toUpperCase(),account=$("pfTradeAccount").value,action=$("pfTradeAction").value,lots=Number($("pfTradeShares").value),shares=lots*100,price=Number($("pfTradePrice").value),tradeDate=$("pfTradeDate").value,memo=String($("pfTradeMemo")?.value||"").slice(0,500);if(!/^[0-9A-Z]{4,5}$/.test(code)||!Number.isInteger(lots)||!(lots>0)||!(price>=0)||!tradeDate){box("pfTradeResult","fail","入力チェック FAIL\nコード・株数（100株単位の整数）・約定単価・売買日を確認してください");return}try{const r=await workerCall("portfolio-trade-list",120000),cur=(r.positions||[]).find(x=>normalizeInvestmentCode(x.code)===normalizeInvestmentCode(code)&&x.account===account),beforeShares=Number(cur?.shares||0),beforeAvg=cur?.avg_cost==null?null:Number(cur.avg_cost);if(action==="REDUCE"&&shares>beforeShares){box("pfTradeResult","fail",`PREVIEW BLOCKED\n現在株数 ${beforeShares} に対して縮小 ${lots}×100=${shares}株 は超過です`);return}const isShort=account==="信用売",afterShares=action==="ADD"?beforeShares+shares:beforeShares-shares,afterAvg=action==="ADD"?(((beforeAvg||0)*beforeShares+price*shares)/(afterShares||1)):(afterShares>0?beforeAvg:null),realized=action==="REDUCE"&&beforeAvg!=null?(isShort?(beforeAvg-price):(price-beforeAvg))*shares:0,name=cur?.name||await pfResolveName(code);latestPfTradePreview={code,name,account,action,shares,price,tradeDate,memo,beforeShares,beforeAvg,afterShares,afterAvg,realized};$("pfTradeCommitBtn").disabled=false;box("pfTradeResult","pass",`PREVIEW（まだ未保存）\n${code} ${name}\n${account} / ${action==="ADD"?"新規・追加":"縮小・決済"}\n入力 ${lots} ×100株 = ${shares}株\n${beforeShares}株 → ${afterShares}株\n平均取得 ${beforeAvg??"-"} → ${afterAvg??"-"}\n取引金額: ${(shares*price).toLocaleString(undefined,{maximumFractionDigits:0})}円\n${action==="REDUCE"?`概算実現損益: ${realized.toLocaleString(undefined,{maximumFractionDigits:0})}円\n`:""}内容確認後に確定してください。`)}catch(e){box("pfTradeResult","fail","PREVIEW FAIL\n"+(e?.message||e))}};
async function pfTradeResetAfterCommit(){const code=$("pfTradeCode"),name=$("pfTradeNameResult"),account=$("pfTradeAccount"),action=$("pfTradeAction"),shares=$("pfTradeShares"),price=$("pfTradePrice"),date=$("pfTradeDate"),memo=$("pfTradeMemo");if(code)code.value="";if(name)name.textContent="未確認";if(account)account.value="NISA";if(action)action.value="ADD";if(shares)shares.value="1";if(price)price.value="";if(date){pfTradeDateManual=false;pfSyncTradeDateToJst(true)}if(memo)memo.value="";if($("pfTradeCommitBtn"))$("pfTradeCommitBtn").disabled=true;}
if($("pfTradeCommitBtn"))$("pfTradeCommitBtn").onclick=async()=>{if(!latestPfTradePreview)return;const btn=$("pfTradeCommitBtn");btn.disabled=true;try{const r=await workerCall("portfolio-trade-commit",120000,null,null,latestPfTradePreview);box("pfTradeResult","pass",`COMMIT PASS\nTradeID: ${r.tradeId}\n${r.beforeShares}株 → ${r.afterShares}株\n平均取得: ${r.afterAvg??"-"}\n実現損益: ${Number(r.realizedPnl||0).toLocaleString(undefined,{maximumFractionDigits:0})}円`);latestPfTradePreview=null;await pfTradeResetAfterCommit();await pfTradeRefresh()}catch(e){box("pfTradeResult","fail","COMMIT FAIL\n"+(e?.message||e));btn.disabled=false}};
if($("pfTradeHistory"))$("pfTradeHistory").addEventListener("click",async ev=>{const cancel=ev.target?.closest?.(".pfVoidInlineCancel"),commit=ev.target?.closest?.(".pfVoidInlineCommit"),b=ev.target?.closest?.(".pfVoidTrade");if(cancel){ev.preventDefault();const panel=cancel.closest(".pf-void-inline");if(panel)panel.remove();latestPfTradeVoidPreview=null;box("pfTradeResult","pass","入力取消をキャンセルしました。未保存です。");return}if(commit){ev.preventDefault();const tradeId=String(commit.dataset.id||"");if(!tradeId)return;commit.disabled=true;try{const r=await workerCall("portfolio-trade-void-commit",120000,null,null,{tradeId,reason:"誤入力取消"});latestPfTradeVoidPreview=null;box("pfTradeResult","pass",`入力取消 COMMIT PASS\nTradeID: ${r.tradeId}\n状態: VOID\n${r.beforeCurrentShares}株 → ${r.restoredShares}株\n平均取得: ${r.restoredAvg??"-"}\n監査履歴は残し、投資成績計算対象から除外します。`);await pfTradeRefresh()}catch(e){box("pfTradeResult","fail","入力取消 FAIL\n"+(e?.message||e));commit.disabled=false}return}if(!b)return;ev.preventDefault();ev.stopPropagation();const tradeId=String(b.dataset.id||"");if(!tradeId){box("pfTradeResult","fail","取消PREVIEW FAIL\nTradeIDを取得できません");return}b.disabled=true;try{const v=await workerCall("portfolio-trade-void-preview",120000,null,null,{tradeId});latestPfTradeVoidPreview=v;document.querySelectorAll(".pf-void-inline").forEach(x=>x.remove());const panel=document.createElement("div");panel.className="pf-void-inline";panel.innerHTML=`<b>誤入力取消 PREVIEW</b><br>${pfEsc(v.code)} ${pfEsc(v.account)} / ${v.action==="ADD"?"追加":"縮小"} ${Number(v.shares).toLocaleString()}株 @ ${Number(v.price).toLocaleString()}<br>現在 ${v.currentShares}株 → <b>取消後 ${v.restoreShares}株</b><br>平均取得 ${v.currentAvg??"-"} → ${v.restoreAvg??"-"}<div class="row"><button type="button" class="pfVoidInlineCommit" data-id="${pfEsc(tradeId)}">取消を確定</button><button type="button" class="secondary pfVoidInlineCancel">やめる</button></div>`;b.closest("td")?.appendChild(panel);box("pfTradeResult","warn","誤入力取消のPreviewを履歴行に表示しました。内容を確認して、その場で確定できます。")}catch(e){latestPfTradeVoidPreview=null;box("pfTradeResult","fail","取消PREVIEW FAIL\n"+(e?.message||e))}finally{b.disabled=false}});
if($("pfTradeVoidCommitBtn"))$("pfTradeVoidCommitBtn").onclick=async()=>{if(!latestPfTradeVoidPreview)return;const btn=$("pfTradeVoidCommitBtn");btn.disabled=true;try{const r=await workerCall("portfolio-trade-void-commit",120000,null,null,{tradeId:latestPfTradeVoidPreview.tradeId,reason:"誤入力取消"});box("pfTradeResult","pass",`入力取消 COMMIT PASS\nTradeID: ${r.tradeId}\n状態: VOID\n${r.beforeCurrentShares}株 → ${r.restoredShares}株\n平均取得: ${r.restoredAvg??"-"}\n監査履歴は残し、投資成績計算対象から除外します。`);latestPfTradeVoidPreview=null;await pfTradeRefresh()}catch(e){box("pfTradeResult","fail","入力取消 FAIL\n"+(e?.message||e));btn.disabled=false}};
(async()=>{pfSyncTradeDateToJst(true);if($("pfTradeDate"))$("pfTradeDate").addEventListener("change",()=>{pfTradeDateManual=true});if($("pfTradePositions"))setTimeout(pfTradeRefresh,80)})();

// v7e-alpha84: Factor Monitor parity + monthly Seasonality cache migration/diagnostics.
...
const FACTOR_ENGINE_VERSION="FactorWebV7-alpha87";
...
function factorPct(rows,pred){const a=[];for(const r of rows){const v=pred(r);if(v!=null)a.push(!!v)}return a.length?a.filter(Boolean).length/a.length*100:null}
function factorClip(v,lo=0,hi=100){return v==null?null:Math.min(hi,Math.max(lo,v))}
function factorRankPercentiles(rows,src,out){const a=[];rows.forEach((r,i)=>{const v=factorNum(r[src]);if(v!=null)a.push({v,i})});a.sort((x,y)=>x.v-y.v);for(const r of rows)r[out]=null;let s=0;while(s<a.length){let e=s+1;while(e<a.length&&a[e].v===a[s].v)e++;const pct=(((s+e-1)/2)+1)/a.length*100;for(let i=s;i<e;i++)rows[a[i].i][out]=pct;s=e}}
function factorDefinition(t,n){if(t==="Size")return "EstimatedMarketCap tercile within current Screening universe";if(t==="Valuation"&&n==="Low PBR")return "Bottom tail of positive PBR within current Screening universe";if(t==="Valuation"&&n==="Low PER")return "Bottom tail of positive ForecastPER within current Screening universe";if(t==="Valuation"&&n==="High Dividend")return "Top tail of positive ForecastDividendYieldPct within current Screening universe";if(t==="Quality")return "Top tail of ROE within current Screening universe";if(t==="Growth")return "Top tail of ForecastPrimaryProfitGrowthPct within current Screening universe";if(t==="Sector")return "Sector33 membership; minimum constituent rule applied";return ""}
function factorMetrics(rows,t,n,k,date){const vals=(field)=>rows.map(r=>factorNum(r[field])).filter(Number.isFinite),m5=factorMedian(vals("RelativeToTOPIX5D")),m20=factorMedian(vals("RelativeToTOPIX20D")),b5=factorPct(rows,r=>{const x=factorNum(r.Return5D);return x==null?null:x>0}),b20=factorPct(rows,r=>{const x=factorNum(r.Return20D);return x==null?null:x>0}),rb5=factorPct(rows,r=>{const x=factorNum(r.RelativeToTOPIX5D);return x==null?null:x>0}),rb20=factorPct(rows,r=>{const x=factorNum(r.RelativeToTOPIX20D);return x==null?null:x>0}),mt=factorMedian(vals("LatestTradingValueRatioTo20D")),ts=mt==null?null:factorClip(mt*50),parts=[ts,b5,rb5].filter(x=>x!=null),flow=parts.length?parts.reduce((a,b)=>a+b,0)/parts.length:null,ar=m5!=null&&m20!=null?4*m5-m20:null,ba=rb5!=null&&rb20!=null?rb5-rb20:null;return{Date:date,FactorMonitorVersion:"FactorMonitorV1",FactorType:t,FactorName:n,FactorKey:k,FactorDefinition:factorDefinition(t,n),ConstituentCount:rows.length,MedianReturn5D:factorMedian(vals("Return5D")),MedianReturn20D:factorMedian(vals("Return20D")),MedianRelativeTOPIX5D:m5,MedianRelativeTOPIX20D:m20,Breadth5D:b5,Breadth20D:b20,RelativeBreadth5D:rb5,RelativeBreadth20D:rb20,MedianTradingValueRatio20D:mt,FlowProxy:flow,AccelerationRaw:ar,BreadthAcceleration:ba,Strength:null,Acceleration:null,StrengthChange1D:null,Phase:"Neutral",Alert:"",FlowDefinition:"PriceTurnoverProxy: mean(turnover_participation,breadth5D,relative_breadth5D)"}}
function factorPhase(r){const s=r.Strength,a=r.Acceleration,r5=r.MedianRelativeTOPIX5D,r20=r.MedianRelativeTOPIX20D,b20=r.RelativeBreadth20D,f=r.FlowProxy;if(s==null||a==null)return"Neutral";if(r20!=null&&r20>0&&r5!=null&&r5<0&&a<=35&&s>=50)return"Fading";if(s>=70&&(r20||0)>0&&(b20||0)>=55)return"Strong";if(a>=70&&(r5||0)>0&&(f||0)>=50&&s<75)return"Emerging";if(a>=55&&(r5||0)>0)return"Improving";if(s<35&&(r20||0)<0)return"Weak";return"Neutral"}
function factorAlert(r){const a=[];if((r.Acceleration||0)>=90&&(r.FlowProxy||0)>=60)a.push("AccelerationSpike");if((r.BreadthAcceleration||0)>=20)a.push("BreadthExpansion");if((r.FlowProxy||0)>=80&&(r.MedianTradingValueRatio20D||0)>=1.3)a.push("FlowSpike");if(r.Phase==="Fading")a.push("FadingRisk");return a.join(";")}
function buildFactorCore(base,date){const usable=base.filter(r=>String(r.NormalizedCode||"").trim()),groups=[],caps=usable.map(r=>factorNum(r.EstimatedMarketCap)).filter(v=>v>0),q33=factorQuantile(caps,1/3),q67=factorQuantile(caps,2/3);if(q33!=null&&q67!=null){groups.push(["Size","Small","Size:Small",usable.filter(r=>{const v=factorNum(r.EstimatedMarketCap);return v>0&&v<=q33})],["Size","Mid","Size:Mid",usable.filter(r=>{const v=factorNum(r.EstimatedMarketCap);return v>q33&&v<=q67})],["Size","Large","Size:Large",usable.filter(r=>factorNum(r.EstimatedMarketCap)>q67)])}
 const add=(field,q,high,t,n,k,pos=false)=>{const vs=usable.map(r=>factorNum(r[field])).filter(v=>v!=null&&(!pos||v>0)),th=factorQuantile(vs,q);if(th==null)return;groups.push([t,n,k,usable.filter(r=>{const v=factorNum(r[field]);return v!=null&&(!pos||v>0)&&(high?v>=th:v<=th)})])};add("PBR",.30,false,"Valuation","Low PBR","Valuation:LowPBR",true);add("ForecastPER",.30,false,"Valuation","Low PER","Valuation:LowPER",true);add("ForecastDividendYieldPct",.70,true,"Valuation","High Dividend","Valuation:HighDividend",true);add("ROE",.70,true,"Quality","High ROE","Quality:HighROE",false);add("ForecastPrimaryProfitGrowthPct",.70,true,"Growth","Earnings Growth","Growth:EarningsGrowth",false);
 const secs=new Map();for(const r of usable){const s=String(r.Sector33||"").trim();if(s){if(!secs.has(s))secs.set(s,[]);secs.get(s).push(r)}}for(const [s,m] of [...secs.entries()].sort((a,b)=>String(a[0])<String(b[0])?-1:String(a[0])>String(b[0])?1:0))if(m.length>=10)groups.push(["Sector",s,`Sector:${s}`,m]);const out=groups.filter(x=>x[3].length>=10).map(x=>factorMetrics(x[3],x[0],x[1],x[2],date));for(const [src,dst] of [["MedianRelativeTOPIX5D","_r5"],["MedianRelativeTOPIX20D","_r20"],["RelativeBreadth20D","_br"],["AccelerationRaw","_ar"],["BreadthAcceleration","_ba"]])factorRankPercentiles(out,src,dst);for(const r of out){const p=[[r._r5,.3],[r._r20,.4],[r._br,.3]].filter(x=>x[0]!=null),ap=[[r._ar,.7],[r._ba,.3]].filter(x=>x[0]!=null);r.Strength=p.length?p.reduce((s,x)=>s+x[0]*x[1],0)/p.reduce((s,x)=>s+x[1],0):null;r.Acceleration=ap.length?ap.reduce((s,x)=>s+x[0]*x[1],0)/ap.reduce((s,x)=>s+x[1],0):null;r.Phase=factorPhase(r);r.Alert=factorAlert(r);delete r._r5;delete r._r20;delete r._br;delete r._ar;delete r._ba}out.sort((a,b)=>String(a.FactorType)<String(b.FactorType)?-1:String(a.FactorType)>String(b.FactorType)?1:String(a.FactorName)<String(b.FactorName)?-1:String(a.FactorName)>String(b.FactorName)?1:0);return out}
function enrichFactorSeasonality(rows,profile,asOf){const pm=new Map((profile||[]).map(r=>[`${r.FactorKey}|${Number(r.SeasonMonth)}`,r])),d=new Date(asOf+"T00:00:00Z"),month=Number(asOf.slice(5,7)),next=month===12?1:month+1,nextDate=month===12?`${Number(asOf.slice(0,4))+1}-01-01`:`${asOf.slice(0,4)}-${String(month+1).padStart(2,"0")}-01`,days=Math.round((Date.parse(nextDate+"T00:00:00Z")-Date.parse(asOf+"T00:00:00Z"))/86400000);for(const r of rows){for(const f of ["SeasonalityScore","SeasonalWinRate","SeasonalMedianExcessReturn","SeasonalMaxDD","SeasonalityConfidence","EffectiveYears"])r[f]=null;r.HistoricalMembershipQuality="";r.DaysToSeasonStart=null;r.SeasonState=r.FactorType!=="Sector"?"NotApplicable":"None";r.SeasonalAlignmentScore=null;r.SeasonalAlert="";if(r.FactorType!=="Sector")continue;const cur=pm.get(`${r.FactorKey}|${month}`),up=pm.get(`${r.FactorKey}|${next}`);let ch=null,state="None",dt=null;if(cur&&(factorNum(cur.SeasonalityScore)||0)>=62&&(factorNum(cur.SeasonalityConfidence)||0)>=55){ch=cur;state="ActiveStrong";dt=0}else if(up&&(factorNum(up.SeasonalityScore)||0)>=62&&(factorNum(up.SeasonalityConfidence)||0)>=55){ch=up;state="UpcomingStrong";dt=days}else ch=cur;if(ch){for(const f of ["SeasonalityScore","SeasonalWinRate","SeasonalMedianExcessReturn","SeasonalMaxDD","SeasonalityConfidence","EffectiveYears"])r[f]=factorNum(ch[f]);r.HistoricalMembershipQuality=String(ch.HistoricalMembershipQuality||"")}r.DaysToSeasonStart=dt;r.SeasonState=state;const parts=[[r.Strength,.3],[r.Acceleration,.3],[r.RelativeBreadth5D,.2],[r.FlowProxy,.2]].filter(x=>x[0]!=null),curScore=parts.length?parts.reduce((s,x)=>s+x[0]*x[1],0)/parts.reduce((s,x)=>s+x[1],0):null;if(ch&&curScore!=null){const ss=factorNum(ch.SeasonalityScore),cf=factorNum(ch.SeasonalityConfidence);if(ss!=null&&cf!=null)r.SeasonalAlignmentScore=.45*ss+.40*curScore+.15*cf}if(["ActiveStrong","UpcomingStrong"].includes(state))r.SeasonalAlert=["Emerging","Improving","Strong"].includes(r.Phase)&&(r.SeasonalAlignmentScore||0)>=60?"EmergingSeasonalFactor":"SeasonalWatch"}return rows}
function buildFactorSummaryWeb(rows){
 const cmpKey=(a,b)=>String(a.FactorKey||"").localeCompare(String(b.FactorKey||""));
 const ranked=phase=>rows.filter(r=>r.Phase===phase).sort((a,b)=>{const av=phase==="Emerging"||phase==="Improving"?-(a.Acceleration??-1):-(a.Strength??-1),bv=phase==="Emerging"||phase==="Improving"?-(b.Acceleration??-1):-(b.Strength??-1);return (av-bv)||cmpKey(a,b)}).slice(0,5),out=[],add=(bucket,arr)=>arr.forEach((r,i)=>out.push(Object.fromEntries(FACTOR_SUMMARY_FIELDS.map(f=>[f,f==="Bucket"?bucket:f==="Rank"?i+1:r[f]??""]))));
 add("TopEmerging",ranked("Emerging"));add("TopStrong",ranked("Strong"));add("TopFading",ranked("Fading"));
 const alerts=rows.filter(r=>r.Alert).sort((a,b)=>{const aa=-(a.Acceleration??-1),ba=-(b.Acceleration??-1);if(aa!==ba)return aa-ba;const af=-(a.FlowProxy??-1),bf=-(b.FlowProxy??-1);return af!==bf?af-bf:cmpKey(a,b)}).slice(0,10);add("Alert",alerts);return out}
function factorMembershipDiagnostics(base){
 const usable=base.filter(r=>String(r.NormalizedCode||"").trim()),num=factorNum;
 const caps=usable.map(r=>num(r.EstimatedMarketCap)).filter(v=>v>0),q33=factorQuantile(caps,1/3),q67=factorQuantile(caps,2/3);
 const defs=[["PBR",.30,false,true,"LowPBR"],["ForecastPER",.30,false,true,"LowPER"],["ForecastDividendYieldPct",.70,true,true,"HighDividend"],["ROE",.70,true,false,"HighROE"],["ForecastPrimaryProfitGrowthPct",.70,true,false,"EarningsGrowth"]],th={};
 for(const [f,q,,,] of defs){const def=defs.find(x=>x[0]===f),pos=def[3];th[f]=factorQuantile(usable.map(r=>num(r[f])).filter(v=>v!=null&&(!pos||v>0)),q)}
 return usable.map(r=>{const cap=num(r.EstimatedMarketCap),o={Date:r.Date||"",Code:r.NormalizedCode||"",CompanyName:r.CompanyName||"",Sector33:r.Sector33||"",EstimatedMarketCap:cap??"",SizeQ33:q33??"",SizeQ67:q67??"",SizeGroup:cap==null?"":cap<=q33?"Small":cap<=q67?"Mid":"Large",LatestTradingValueRatioTo20D:r.LatestTradingValueRatioTo20D??"",PBR:r.PBR??"",ForecastPER:r.ForecastPER??"",ForecastDividendYieldPct:r.ForecastDividendYieldPct??"",ROE:r.ROE??"",ForecastPrimaryProfitGrowthPct:r.ForecastPrimaryProfitGrowthPct??""};for(const [f,q,high,pos,label] of defs){const v=num(r[f]),x=th[f];o[label+"Threshold"]=x??"";o[label+"Member"]=(v!=null&&(!pos||v>0)&&x!=null&&(high?v>=x:v<=x))?1:0}return o})}
...
async function runScreeningWebDaily(asOf,progress){
 ...
 progress?.("5戦略を選抜中");const ev=await workerCall("screening-event-features",300000,null,null,{asOf,events:base.map(r=>({code:r.NormalizedCode,disclosureDate:r.LatestEarningsEventDate||r.LatestDisclosureDate}))});
 const em=new Map((ev.rows||[]).map(x=>[String(x.code??x.NormalizedCode??""),x]).filter(([k])=>k));
 const enriched=base.map(r=>({...r,...(em.get(String(r.NormalizedCode))||{EarningsElapsedTradingDays:null,EarningsReactionPending:false})}));
 const eventInputCount=base.filter(r=>r.LatestEarningsEventDate||r.LatestDisclosureDate).length,eventJoinedCount=enriched.filter(r=>r.EarningsElapsedTradingDays!=null||r.EarningsReactionPending===true).length;
 if((ev.rows||[]).length>0&&eventInputCount>0&&eventJoinedCount===0)throw new Error(`Screening event JOIN invariant failed: workerRows=${(ev.rows||[]).length}, input=${eventInputCount}, joined=0`);
 const built=buildScreeningStrategies(enriched);latestScreeningCandidates=built.rows;latestScreeningScoredRows=built.scored||[];
...
// v9.4: temporary read-only storage inventory. Remove after incident analysis.
async function jqTempWalkOpfs(dir,path="/",out=[],depth=0){
  if(depth>12)return out;
  let entries=[];
  try{for await(const pair of dir.entries())entries.push(pair)}catch(e){
    if(typeof dir.values==="function")for await(const h of dir.values())entries.push([h.name||"(unknown)",h]); else throw e;
  }
  for(const [name,h] of entries){
    const p=(path==="/"?"/":path+"/")+name;
    if(h.kind==="file"){try{const f=await h.getFile();out.push({path:p,kind:"file",bytes:Number(f.size||0),lastModified:f.lastModified||0})}catch(e){out.push({path:p,kind:"file",bytes:null,error:String(e?.message||e)})}}
    else if(h.kind==="directory"){out.push({path:p,kind:"directory",bytes:0});await jqTempWalkOpfs(h,p,out,depth+1)}
  }
  return out;
}
async function jqTempStorageAudit(){
  const est=await navigator.storage?.estimate?.()||{};
  const result={usage:Number(est.usage||0),quota:Number(est.quota||0),usageDetails:est.usageDetails||null,opfs:[],caches:[],indexedDB:[]};
  if(navigator.storage?.getDirectory){try{result.opfs=await jqTempWalkOpfs(await navigator.storage.getDirectory())}catch(e){result.opfsError=String(e?.message||e)}}
  if(globalThis.caches?.keys){try{for(const name of await caches.keys()){try{const c=await caches.open(name),keys=await c.keys();result.caches.push({name,entries:keys.length})}catch(e){result.caches.push({name,error:String(e?.message||e)})}}}catch(e){result.cacheError=String(e?.message||e)}}
  if(indexedDB?.databases){try{result.indexedDB=(await indexedDB.databases()).map(x=>({name:x.name||"(unnamed)",version:x.version??null}))}catch(e){result.idbError=String(e?.message||e)}}
  return result;
}
if($("temporaryStorageAuditBtn")) $("temporaryStorageAuditBtn").onclick=async()=>{
  const id="temporaryStorageAuditResult";box(id,"run","read-onlyでOriginストレージを棚卸し中…\
巨大ファイル本体は読み込みません。");
  try{
    const r=await jqTempStorageAudit(),files=r.opfs.filter(x=>x.kind==="file"),opfsBytes=files.reduce((a,x)=>a+(Number(x.bytes)||0),0),dirs=r.opfs.filter(x=>x.kind==="directory").length;
    const top=files.slice().sort((a,b)=>(Number(b.bytes)||0)-(Number(a.bytes)||0)).slice(0,30);
    const details=r.usageDetails?Object.entries(r.usageDetails).map(([k,v])=>`${k}: ${jqFmtBytes(v)}`).join("\
"):"（ブラウザからusageDetails未提供）";
    const cacheLines=r.caches.length?r.caches.map((x,i)=>`${i+1}. ${x.name} / ${x.entries??"?"} entries${x.error?` / ${x.error}`:""}`).join("\
"):"（なし/取得不可）";
    const idbLines=r.indexedDB.length?r.indexedDB.map((x,i)=>`${i+1}. ${x.name} / version ${x.version??"?"}`).join("\
"):"（なし/取得不可）";
    const topLines=top.length?top.map((x,i)=>`${i+1}. ${x.path} / ${jqFmtBytes(x.bytes)}`).join("\
"):"（ファイルなし）";
    box(id,"pass",`PASS（read-only）\
Origin: ${location.origin}\
Origin使用量: ${jqFmtBytes(r.usage)}\
Quota: ${jqFmtBytes(r.quota)}\
\
[ブラウザ提供の使用量内訳]\
${details}\
\
...
ディレクトリ: ${dirs}\
ファイル: ${files.length}\
列挙可能ファイル合計: ${jqFmtBytes(opfsBytes)}\
\
容量上位ファイル（最大30件）\
${topLines}\
\
...
// v9.4: destructive local reset with bottom-up OPFS deletion and physical verification.
