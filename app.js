...
if($("aiShareZipBtn")) $("aiShareZipBtn").onclick=async()=>{
 ...
 try{
  ...
  const headers=Object.keys(rows[0]),csv=simpleCsv(rows,headers),manifest={schema:"web-ai-share-manifest-v1",version:"v7e-alpha97",generatedAt:new Date().toISOString(),asOf,canonicalAsOfSource:canon.source,dataLakeDate:canon.dataLakeDate,lastPassPipelineDate:canon.passDate,portfolioCount:rows.length,coverage:{technical:`${tech}/${rows.length}`,financial:`${fin}/${rows.length}`,marginInterest:`${margin}/${rows.length}`,shortSelling:`${shorts}/${rows.length}`},files:["web_jqp.json","web_portfolio_integrated.csv","manifest.json","README.txt"],note:"Web-first AI share stage 1. Generation/download separated; generated artifact can be downloaded repeatedly without recomputation."};
...
// v7e-alpha97: daily-use direct Investment Tracking registration / update / Watchlist close UI.
