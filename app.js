...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v7e-alpha102");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if($("aiShareZipBtn")) $("aiShareZipBtn").onclick=async()=>{
 ...
 try{
  ...
  if((h.counts?.marketShortRatioHistory??0)>0 && !(h.counts?.marketShortRatioValid>0))throw new Error(`Data Quality FAIL: 市場空売り比率 ${h.counts?.marketShortRatioHistory??0}行を取得しましたが、有効な数値行が0件です。共有ZIPは生成しません。`);
  ...
  const manifest={schema:"web-ai-share-manifest-v3",version:"v7e-alpha102",generatedAt:new Date().toISOString(),asOf,canonicalAsOfSource:canon.source,dataLakeDate:canon.dataLakeDate,lastPassPipelineDate:canon.passDate,portfolioCount:rows.length,coverage:{technical:`${tech}/${rows.length}`,financial:`${fin}/${rows.length}`,marginInterest:`${margin}/${rows.length}`,shortSelling:`${shorts}/${rows.length}`},historyCounts:h.counts||{},historyWindow:{priceFrom:h.meta?.from||null,priceTo:asOf,financial:"available Web DataLake history",supply:"available Web DataLake history",marketFlow:"available Web DataLake history",trade:"Web trade ledger since Portfolio Manager adoption"},files,notes:["Stage 2 DQ: J-Quants API v2 short-ratio abbreviated-field mapping (S33/SellExShortVa/ShortWithResVa/ShortWoResVa) and value-level fail-closed invariant added.","Price history is limited to five years to control mobile ZIP size.","Existing holdings before Web Portfolio Manager remain opening snapshots; web_trade_history.csv is not a reconstructed broker ledger.","VOID trades remain in audit history and must be excluded from performance statistics.","Candidate earnings history and management guidance are Screening-share work and are not duplicated in the portfolio share."]};
  const readme=`J-Quants Web-first ChatGPT Share Stage 2 DQ Hotfix\nVersion: v7e-alpha102\nAsOf: ${asOf}\nPortfolio: ${rows.length}\nTechnical: ${tech}/${rows.length}\nFinancial: ${fin}/${rows.length}\nMargin interest: ${margin}/${rows.length}\nLarge-short canonical: ${shorts}/${rows.length}\nPrice history rows: ${h.counts?.priceHistory??0}\nFinancial history rows: ${h.counts?.financialHistory??0}\nMargin history rows: ${h.counts?.marginHistory??0}\nMarket short-ratio rows: ${h.counts?.marketShortRatioHistory??0}\nMarket short-ratio valid rows: ${h.counts?.marketShortRatioValid??0}\nLarge-short rows: ${h.counts?.largeShortHistory??0}\nMarket-flow rows: ${h.counts?.marketFlow??0}\nWeb trade rows: ${h.counts?.tradeHistory??0}\n`;
...
// v7e-alpha102: daily-use direct Investment Tracking registration / update / Watchlist close UI.
