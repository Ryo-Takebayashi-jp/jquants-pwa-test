...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v7e-alpha104");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
// v7e-alpha104: daily-use direct Investment Tracking registration / update / Watchlist close UI.
...
if($("pfTradeHistory"))$("pfTradeHistory").addEventListener("click",async ev=>{const b=ev.target?.closest?.(".pfVoidTrade");if(!b)return;ev.preventDefault();ev.stopPropagation();const tradeId=String(b.dataset.id||"");if(!tradeId){box("pfTradeResult","fail","取消PREVIEW FAIL\nTradeIDを取得できません");return}b.disabled=true;box("pfTradeResult","run",`入力取消PREVIEWを確認中…\nTradeID: ${tradeId}`);try{const v=await workerCall("portfolio-trade-void-preview",120000,null,null,{tradeId});latestPfTradeVoidPreview=v;$("pfTradeVoidCommitBtn").disabled=false;box("pfTradeResult","warn",`入力取消 PREVIEW（まだ未保存）\n${v.code} ${v.account} / ${v.action==="ADD"?"追加":"縮小"} ${Number(v.shares).toLocaleString()}株 @ ${Number(v.price).toLocaleString()}\n現在 ${v.currentShares}株 → 取消後 ${v.restoreShares}株\n平均取得 ${v.currentAvg??"-"} → ${v.restoreAvg??"-"}\n\nこれは証券取引の反対売買ではなく、誤入力した記録そのものをVOIDにします。`)}catch(e){latestPfTradeVoidPreview=null;$("pfTradeVoidCommitBtn").disabled=true;box("pfTradeResult","fail","取消PREVIEW FAIL\n"+(e?.message||e))}finally{b.disabled=false}});
