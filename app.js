...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v7e-alpha106");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v7e-alpha106").catch(()=>{}));
...
if($("userBackupZipBtn")) $("userBackupZipBtn").onclick=async()=>{
 ...
 try{
  ...
  const bytes=new Uint8Array(r.buffer),manifest={format:"JQ-USER-BACKUP-v1",appVersion:"v7e-alpha106",createdAt:new Date().toISOString(),db:"jq_private_v1.sqlite",bytes:r.bytes,sha256:r.sha256||null,quickCheck:priv.quickCheck,tables:priv.tables||[],includes:["portfolio","watchlist","discovery","investment-tracking","web-trade-ledger","void-audit","pipeline-checkpoints"],excludes:["J-Quants API key","market DataLake"]};
  const blob=zipStoreBlob([{name:"jq_private_v1.sqlite",data:bytes},{name:"manifest.json",data:JSON.stringify(manifest,null,2)},{name:"README.txt",data:"J-Quants User Data Backup v7e-alpha106\nAPI key is intentionally excluded.\nRestore only from the Settings / Maintenance page.\n"}]);
...
// v7e-alpha106: daily-use direct Investment Tracking + production UI architecture cleanup.
...
async function pfTradeRefresh(){
 ...
 try{
  ...
  $("pfTradePositions").innerHTML=filtered.length?`<div class="portfolio-grid">${filtered.map(x=>`<div class="portfolio-row"><div class="portfolio-title">${pfEsc(x.code)}　${pfEsc(x.name)}</div><div class="portfolio-meta">${pfEsc(x.account)} / ${Number(x.shares||0).toLocaleString()}株</div><div class="portfolio-numbers"><span>現在値</span><b>${pfMoney(x.close)}円</b><span>平均取得</span><b>${pfMoney(x.avg_cost)}円</b><span>評価額</span><b>${pfMoney(x.marketValue)}円</b><span>評価損益</span><b class="${(x.pnl||0)>=0?'pnl-plus':'pnl-minus'}">${(x.pnl||0)>=0?'+':''}${pfMoney(x.pnl)}円 (${pfPct(x.pnlPct)})</b></div><div class="inv-actions"><button class="secondary pfReduceRow" data-code="${pfEsc(x.code)}" data-account="${pfEsc(x.account)}">縮小/決済</button></div></div>`).join('')}</div>`:"<p class='desc'>この区分の保有データなし</p>";
