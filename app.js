...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v7e-alpha108");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v7e-alpha108").catch(()=>{}));
...
if($("userBackupZipBtn")) $("userBackupZipBtn").onclick=async()=>{
 ...
 try{
  ...
  const bytes=new Uint8Array(r.buffer),manifest={format:"JQ-USER-BACKUP-v1",appVersion:"v7e-alpha108",createdAt:new Date().toISOString(),db:"jq_private_v1.sqlite",bytes:r.bytes,sha256:r.sha256||null,quickCheck:priv.quickCheck,tables:priv.tables||[],includes:["portfolio","watchlist","discovery","investment-tracking","web-trade-ledger","void-audit","pipeline-checkpoints"],excludes:["J-Quants API key","market DataLake"]};
  const blob=zipStoreBlob([{name:"jq_private_v1.sqlite",data:bytes},{name:"manifest.json",data:JSON.stringify(manifest,null,2)},{name:"README.txt",data:"J-Quants User Data Backup v7e-alpha108\nAPI key is intentionally excluded.\nRestore only from the Settings / Maintenance page.\n"}]);
...
// v7e-alpha108: daily-use direct Investment Tracking + production UI architecture cleanup.
...
async function quickInvRenderState(st){
 ...
 const rows=codes.map(c=>({code:c,name:names.get(c)||"",d:latestD.get(c),w:wm.get(c)}));
 $("quickInvTable").innerHTML=rows.length?`<div class="inv-grid">${rows.map(x=>`<div class="inv-item"><div class="inv-title">${quickInvEsc(x.code)}　${quickInvEsc(x.name||"銘柄名未取得")}</div><div class="inv-meta">Status: ${quickInvEsc(x.w?.InvestmentStatus||x.d?.InvestmentStatus||x.d?.AnalystDecision||"TRACK")}<br>登録日: ${quickInvEsc(x.w?.WatchStartDate||x.d?.EpisodeStartDate||x.d?.DiscoveryDate||"-")} / Watch: ${quickInvEsc(x.w?.Status||"-")}</div><div class="inv-actions"><button class="watch-remove quickInvCloseRow" data-code="${quickInvEsc(x.code)}" data-status="${quickInvEsc(x.w?.InvestmentStatus||x.d?.InvestmentStatus||"WATCH")}">削除</button></div></div>`).join("")}</div>`:"<p class='desc'>登録データなし</p>";
...
async function pfTradeRefresh(){
 ...
 try{
  ...
  let ar={rows:[]};try{const asOf=await quickInvCanonicalDate();ar=await workerCall("my-stocks-analysis",600000,null,null,{asOf})}catch(_){}const am=new Map((ar.rows||[]).map(x=>[`${normalizeInvestmentCode(x.code)}|${x.account}`,x]));
  const pos=(r.positions||[]).map(x=>{const a=am.get(`${normalizeInvestmentCode(x.code)}|${x.account}`)||{},shares=Number(x.shares||0),avg=Number(x.avg_cost||0),close=Number.isFinite(Number(a.close))?Number(a.close):null,mv=close==null?null:close*shares,cost=avg*shares,isShort=x.account==="信用売",pnl=close==null?null:(isShort?(avg-close):(close-avg))*shares,pct=cost?pnl/cost*100:null;return {...x,name:x.name||names.get(normalizeInvestmentCode(x.code))||"銘柄名未取得",close,marketValue:mv,cost,pnl,pnlPct:pct}});
  ...
  $("pfTradePositions").innerHTML=filtered.length?`<div class="portfolio-grid">${filtered.map(x=>`<div class="portfolio-row"><div class="portfolio-title">${pfEsc(x.code)}　${pfEsc(x.name)}</div><div class="portfolio-meta">${pfEsc(x.account)} / ${Number(x.shares||0).toLocaleString()}株</div><div class="portfolio-numbers"><span>最新終値</span><b>${x.close==null?"価格なし":pfMoney(x.close)+"円"}</b><span>平均取得</span><b>${pfMoney(x.avg_cost)}円</b><span>評価額</span><b>${x.marketValue==null?"-":pfMoney(x.marketValue)+"円"}</b><span>評価損益</span><b class="${x.pnl==null?'':(x.pnl>=0?'pnl-plus':'pnl-minus')}">${x.pnl==null?'-':`${x.pnl>=0?'+':''}${pfMoney(x.pnl)}円 (${pfPct(x.pnlPct)})`}</b></div><div class="inv-actions"><button class="secondary pfReduceRow" data-code="${pfEsc(x.code)}" data-account="${pfEsc(x.account)}">縮小/決済</button></div></div>`).join('')}</div>`:"<p class='desc'>この区分の保有データなし</p>";
