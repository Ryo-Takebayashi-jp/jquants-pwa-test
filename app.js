...
const SCREENING_DEFAULT_STRATEGIES=Object.fromEntries(SCREENING_STRATEGY_NAMES.map(k=>[k,{enabled:true,topN:20,filters:{...SCREENING_DEFAULT_FILTERS[k]}}]));
...
function loadNamedScreeningProfiles(){try{const x=JSON.parse(localStorage.getItem(SCREENING_PROFILE_KEY)||"[]");return [0,1,2].map(i=>x[i]&&x[i].config?{name:String(x[i].name||`保存設定${i+1}`),config:normalizeScreeningConfig(x[i].config)}:null)}catch(_){return [null,null,null]}}
function saveNamedScreeningProfiles(x){localStorage.setItem(SCREENING_PROFILE_KEY,JSON.stringify(x))}
function renderScreeningNamedProfiles(){document.querySelectorAll('.screen-profile-slot').forEach((row,i)=>{const ps=loadNamedScreeningProfiles(),p=ps[i],inp=row.querySelector('.screen-profile-name'),load=row.querySelector('.screen-profile-load');if(inp&&!document.activeElement?.isSameNode(inp))inp.value=p?.name||`保存設定${i+1}`;if(load)load.disabled=!p})}
document.querySelectorAll('.screen-profile-save').forEach((b,i)=>b.onclick=()=>{const ps=loadNamedScreeningProfiles(),row=b.closest('.screen-profile-slot'),name=row?.querySelector('.screen-profile-name')?.value.trim()||`保存設定${i+1}`;ps[i]={name,config:readScreeningConfigUi()};saveNamedScreeningProfiles(ps);renderScreeningNamedProfiles();box("screenCfgResult","pass",`「${name}」に現在設定を保存しました。`) });
document.querySelectorAll('.screen-profile-load').forEach((b,i)=>b.onclick=()=>{const p=loadNamedScreeningProfiles()[i];if(!p)return;localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(p.config));latestScreeningBaseRows=[];latestScreeningBaseAsOf="";latestScreeningBaseProfileSig="";applyScreeningConfigUi(p.config);box("screenCfgResult","pass",`「${p.name}」を読み込みました。`) });
...
document.querySelectorAll('#screenCfgStrategies .screen-strategy-enabled').forEach(x=>x.onchange=()=>{const card=x.closest('.screen-strategy-card');card?.querySelectorAll('.screen-strategy-topn,[data-filter]').forEach(el=>el.disabled=!x.checked);if(card)card.style.opacity=x.checked?"1":".55"});
...
function buildScreeningStrategies(rows){
 ...
 const strategyPass=(name,r,z)=>{const f=z?.filters||{};
 ...
 for(const [name,key] of defs){
   ...
   const eligible=rr.filter(r=>r[key]!=null&&!r.EarningsReactionPending&&r.FinancialDataFlag!=="WebRequired"&&r.FinancialDataFlag!=="NoLatestFinancial"&&strategyPass(name,r,z))
