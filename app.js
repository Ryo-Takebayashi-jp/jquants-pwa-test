...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v9.2");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v9.2").catch(()=>{}));
...
let lastGapPlan=[];
// v9.2: exactly one canonical J-Quants API key per browser session.
...
// v9.2: simplified Settings / Maintenance + read-only OPFS diagnostics.
...
if($("userBackupZipBtn")) $("userBackupZipBtn").onclick=async()=>{
 ...
 try{
  ...
  const bytes=new Uint8Array(r.buffer),manifest={format:"JQ-USER-BACKUP-v1",appVersion:"v9.2",createdAt:new Date().toISOString(),db:"jq_private_v1.sqlite",bytes:r.bytes,sha256:r.sha256||null,quickCheck:priv.quickCheck,tables:priv.tables||[],includes:["portfolio","watchlist","discovery","investment-tracking","web-trade-ledger","void-audit","pipeline-checkpoints"],excludes:["J-Quants API key","market DataLake"]};
  const blob=zipStoreBlob([{name:"jq_private_v1.sqlite",data:bytes},{name:"manifest.json",data:JSON.stringify(manifest,null,2)},{name:"README.txt",data:"J-Quants User Data Backup v9.2\nAPI key is intentionally excluded.\nRestore only from the Settings / Maintenance page.\n"}]);
...
document.querySelectorAll('#screenCfgStrategies .screen-strategy-enabled').forEach(x=>x.onchange=()=>{const card=x.closest('.screen-strategy-card'),st=card?.querySelector('.screen-strategy-strength');if(st)st.disabled=!x.checked;if(card)card.style.opacity=x.checked?"1":".55"});
...
// v9.2: temporary read-only storage inventory. Remove after incident analysis.
...
// v9.2: destructive local reset with bottom-up OPFS deletion and physical verification.
