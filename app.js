...
function ensureSqliteWorker(){
 ...
 const w=new Worker("./sqlite-worker.js?v=v7e-alpha105");
 jqSqliteWorker=w;
 w.onmessage=e=>{
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v7e-alpha105").catch(()=>{}));
...
if($("userBackupZipBtn")) $("userBackupZipBtn").onclick=async()=>{
 ...
 try{
  ...
  const bytes=new Uint8Array(r.buffer),manifest={format:"JQ-USER-BACKUP-v1",appVersion:"v7e-alpha105",createdAt:new Date().toISOString(),db:"jq_private_v1.sqlite",bytes:r.bytes,sha256:r.sha256||null,quickCheck:priv.quickCheck,tables:priv.tables||[],includes:["portfolio","watchlist","discovery","investment-tracking","web-trade-ledger","void-audit","pipeline-checkpoints"],excludes:["J-Quants API key","market DataLake"]};
  const blob=zipStoreBlob([{name:"jq_private_v1.sqlite",data:bytes},{name:"manifest.json",data:JSON.stringify(manifest,null,2)},{name:"README.txt",data:"J-Quants User Data Backup v7e-alpha105\nAPI key is intentionally excluded.\nRestore only from the Settings / Maintenance page.\n"}]);
...
// v7e-alpha105: daily-use direct Investment Tracking + production UI architecture cleanup.
...
async function quickInvRenderState(st){latestQuickInvestmentState=st||{discovery:[],master:[],state:[]};const latestD=new Map();for(const r of st.discovery||[]){const c=normalizeInvestmentCode(r.Code),d=String(r.EpisodeStartDate||r.DiscoveryDate||"");if(!c)continue;const old=latestD.get(c);if(!old||String(old.EpisodeStartDate||old.DiscoveryDate||"")<d)latestD.set(c,r)}const active=(st.master||[]).filter(r=>["Active","ReviewDue"].includes(String(r.Status||""))),codes=[...new Set([...latestD.keys(),...active.map(r=>normalizeInvestmentCode(r.Code))])].sort(),wm=new Map(active.map(r=>[normalizeInvestmentCode(r.Code),r]));let names=new Map();try{const nm=await workerCall("equities-master-names",120000,null,null,{codes});for(const r of nm.rows||[])names.set(normalizeInvestmentCode(r.code),r.company_name||"")}catch(_){}const rows=codes.map(c=>({code:c,name:names.get(c)||"",d:latestD.get(c),w:wm.get(c)}));$("quickInvTable").innerHTML=rows.length?`<div class="inv-grid">${rows.map(x=>`<div class="inv-item"><div class="inv-title">${quickInvEsc(x.code)}　${quickInvEsc(x.name||"銘柄名未取得")}</div><div class="inv-meta">Discovery: ${x.d?quickInvEsc((x.d.InvestmentStatus||x.d.AnalystDecision||"TRACK")+" / "+(x.d.EpisodeStartDate||x.d.DiscoveryDate||"")):"-"}<br>Watchlist: ${x.w?quickInvEsc((x.w.InvestmentStatus||"WATCH")+" / "+(x.w.Status||"")):"-"}</div>${x.w?`<div class="inv-actions"><button class="secondary quickInvCloseRow" data-code="${quickInvEsc(x.code)}" data-status="${quickInvEsc(x.w.InvestmentStatus||"WATCH")}">終了</button></div>`:""}</div>`).join("")}</div>`:"<p class='desc'>登録データなし</p>";for(const b of document.querySelectorAll(".quickInvCloseRow"))b.onclick=()=>{$("quickInvCode").value=b.dataset.code||"";$("quickInvStatus").value=b.dataset.status||"WATCH";$("quickInvAction").value="CLOSE";quickInvSyncFields();$("quickInvCode").scrollIntoView({behavior:"smooth",block:"center"})}}
...
async function pfTradeRefresh(){const btn=$("pfTradeRefreshBtn");if(btn)btn.disabled=true;try{const r=await workerCall("portfolio-trade-list",120000);let codes=[...new Set((r.positions||[]).map(x=>normalizeInvestmentCode(x.code)))],names=new Map();try{const nm=await workerCall("equities-master-names",120000,null,null,{codes});for(const x of nm.rows||[])names.set(normalizeInvestmentCode(x.code),x.company_name||"")}catch(_){}$("pfTradePositions").innerHTML=(r.positions||[]).length?(r.positions||[]).map(x=>`<div class="portfolio-row"><div class="portfolio-title">${pfEsc(x.code)}　${pfEsc(x.name||names.get(normalizeInvestmentCode(x.code))||"銘柄名未取得")}</div><div class="portfolio-meta">${pfEsc(x.account)} / ${Number(x.shares||0).toLocaleString()}株 / 平均取得 ${x.avg_cost==null?"-":Number(x.avg_cost).toLocaleString(undefined,{maximumFractionDigits:3})}</div><div class="inv-actions"><button class="secondary pfReduceRow" data-code="${pfEsc(x.code)}" data-account="${pfEsc(x.account)}">縮小/決済</button></div></div>`).join(""):"<p class='desc'>保有データなし</p>";$("pfTradeHistory").innerHTML=(r.history||[]).length?`<div style="overflow:auto"><table style="width:100%;font-size:12px"><tr><th>日付</th><th>銘柄</th><th>区分</th><th>操作</th><th>株数</th><th>単価</th><th>実現損益</th></tr>${r.history.map(x=>`<tr><td>${pfEsc(x.trade_date)}</td><td>${pfEsc(x.code)}</td><td>${pfEsc(x.account)}</td><td>${x.status==="VOID"?"取消済":(x.action==="ADD"?"追加":"縮小")}</td><td>${Number(x.shares||0).toLocaleString()}</td><td>${Number(x.price||0).toLocaleString()}</td><td>${Number(x.realized_pnl||0).toLocaleString(undefined,{maximumFractionDigits:0})}</td><td>${x.can_void?`<button type="button" class="secondary pfVoidTrade" data-id="${pfEsc(x.trade_id)}">入力取消</button>`:`<span class="pf-void-disabled">${pfEsc(x.can_void_reason||"")}</span>`}</td></tr>`).join("")}</table></div>`:"<p class='desc'>売買履歴なし（既存保有は移行時点スナップショット）</p>";for(const b of document.querySelectorAll(".pfReduceRow"))b.onclick=()=>{$("pfTradeCode").value=b.dataset.code||"";$("pfTradeAccount").value=b.dataset.account||"現物";$("pfTradeAction").value="REDUCE";$("pfTradeCode").scrollIntoView({behavior:"smooth",block:"center"})};box("pfTradeResult","pass",`保有一覧を読込 PASS\nポジション: ${r.count||0}\n売買履歴: ${(r.history||[]).length}`)}catch(e){box("pfTradeResult","fail","一覧読込 FAIL\n"+(e?.message||e))}finally{if(btn)btn.disabled=false}}
if($("pfTradeRefreshBtn"))$("pfTradeRefreshBtn").onclick=pfTradeRefresh;
if($("pfTradePreviewBtn"))$("pfTradePreviewBtn").onclick=async()=>{const code=String($("pfTradeCode").value||"").trim().toUpperCase(),account=$("pfTradeAccount").value,action=$("pfTradeAction").value,shares=Number($("pfTradeShares").value),price=Number($("pfTradePrice").value),tradeDate=$("pfTradeDate").value,memo=$("pfTradeMemo").value;if(!/^[0-9A-Z]{4,5}$/.test(code)||!(shares>0)||!(price>=0)||!tradeDate){box("pfTradeResult","fail","入力チェック FAIL\nコード・株数・約定単価・売買日を確認してください");return}try{const r=await workerCall("portfolio-trade-list",120000),cur=(r.positions||[]).find(x=>normalizeInvestmentCode(x.code)===normalizeInvestmentCode(code)&&x.account===account),beforeShares=Number(cur?.shares||0),beforeAvg=cur?.avg_cost==null?null:Number(cur.avg_cost);if(action==="REDUCE"&&shares>beforeShares){box("pfTradeResult","fail",`PREVIEW BLOCKED\n現在株数 ${beforeShares} に対して縮小 ${shares} は超過です`);return}const isShort=account==="信用売",afterShares=action==="ADD"?beforeShares+shares:beforeShares-shares,afterAvg=action==="ADD"?(((beforeAvg||0)*beforeShares+price*shares)/(afterShares||1)):(afterShares>0?beforeAvg:null),realized=action==="REDUCE"&&beforeAvg!=null?(isShort?(beforeAvg-price):(price-beforeAvg))*shares:0,name=cur?.name||await pfResolveName(code);latestPfTradePreview={code,name,account,action,shares,price,tradeDate,memo,beforeShares,beforeAvg,afterShares,afterAvg,realized};$("pfTradeCommitBtn").disabled=false;box("pfTradeResult","pass",`PREVIEW（まだ未保存）\n${code} ${name}\n${account} / ${action==="ADD"?"新規・追加":"縮小・決済"}\n${beforeShares}株 → ${afterShares}株\n平均取得 ${beforeAvg??"-"} → ${afterAvg??"-"}\n${action==="REDUCE"?`概算実現損益: ${realized.toLocaleString(undefined,{maximumFractionDigits:0})}円\n`:""}内容確認後に確定してください。`)}catch(e){box("pfTradeResult","fail","PREVIEW FAIL\n"+(e?.message||e))}};
