...
function box(id,cls,t){const e=$(id);e.className="result "+cls;e.textContent=t;if(id==="screenCfgResult")e.style.display="block"}
...
function screeningAxisWeights(strategy,axes={}){const defs=SCREENING_AXIS_DEFS[strategy]||[],raw=defs.map(a=>({key:a.key,base:a.base,mult:SCREENING_AXIS_MULT[axes?.[a.key]||"medium"]||1})),den=raw.reduce((s,x)=>s+x.base*x.mult,0)||1;return Object.fromEntries(raw.map(x=>[x.key,{base:x.base,current:(x.base*x.mult)/den,mult:x.mult}]))}
function updateScreeningAxisMeta(card){if(!card)return;const k=card.dataset.strategy,axes={};for(const a of SCREENING_AXIS_DEFS[k]||[])axes[a.key]=card.querySelector(`input[data-axis="${a.key}"]:checked`)?.value||"medium";const ws=screeningAxisWeights(k,axes);for(const a of SCREENING_AXIS_DEFS[k]||[]){const row=card.querySelector(`input[data-axis="${a.key}"]`)?.closest('.screen-axis-row');if(!row)continue;let m=row.querySelector(`[data-axis-meta="${a.key}"]`);if(!m){m=document.createElement('div');m.className='screen-axis-meta';m.dataset.axisMeta=a.key;row.appendChild(m)}const w=ws[a.key];m.innerHTML=`弱 0.6× / 中 1.0× / 強 1.4×　<b>標準 ${Math.round(w.base*100)}% → 現在 ${Math.round(w.current*100)}%</b>`}}
function updateAllScreeningAxisMeta(){document.querySelectorAll('#screenCfgStrategies .screen-strategy-card').forEach(updateScreeningAxisMeta)}
...
function renderScreeningStrategyAxes(cfg=loadScreeningConfig()){
 ...
 document.querySelectorAll('#screenCfgStrategies .screen-strategy-card').forEach(card=>{
  ...
  const e=card.querySelector('.screen-strategy-enabled');if(e)e.checked=z.enabled;card.classList.toggle('disabled',!z.enabled);updateScreeningAxisMeta(card);
 });
}
...
function saveNamedScreeningProfiles(x){const payload=JSON.stringify((Array.isArray(x)?x:[]).slice(0,3));localStorage.setItem(SCREENING_PROFILE_KEY,payload);const back=localStorage.getItem(SCREENING_PROFILE_KEY);if(back!==payload)throw new Error("ブラウザ保存領域への書き込み確認に失敗しました")}
...
function bindScreeningConfigControls(){
 ...
 document.querySelectorAll('#screenCfgStrategies input[type="radio"][data-axis]').forEach(x=>x.onchange=()=>updateScreeningAxisMeta(x.closest('.screen-strategy-card')));
 if(reset)reset.onclick=()=>{const c=normalizeScreeningConfig(SCREENING_DEFAULT_CONFIG);localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(c));latestScreeningBaseRows=[];latestScreeningBaseAsOf="";latestScreeningBaseProfileSig="";applyScreeningConfigUi(c);renderScreeningNamedProfiles("default");box("screenCfgResult","pass","標準設定に戻しました。")};
}
function bootScreeningConfigUi(){try{initScreeningConfigUi();bindScreeningConfigControls()}catch(e){console.error("screening-config-init",e);if($("screenCfgResult"))box("screenCfgResult","fail","Screening設定画面の初期化に失敗しました。\n"+(e?.message||e))}}
...
if($("screeningStrategyBtn")) $("screeningStrategyBtn").onclick=async()=>{
 ...
 try{
   ...
   box("screeningStrategyResult","run","現在の保存設定を読み込み、Screening母集団から再構築中…");
   const base=await ensureFactorBase(asOf);
   latestFactorBaseRows=base;
   if(!base.length)throw new Error("Screening母集団が空です。現在の保存設定を確認してください。");
   box("screeningStrategyResult","run","母集団再構築完了。決算イベント窓（1/3/5/10営業日）を計算中…");
   const ev=await workerCall("screening-event-features",300000,null,null,{asOf,events:base.map(r=>({code:r.NormalizedCode,disclosureDate:r.LatestEarningsEventDate||r.LatestDisclosureDate}))});
...
async function runDailyPipeline(){
 ...
 try{
   ...
   const execute=async(stage,fn)=>{const old=stepMap.get(stage.key),forceFreshAnalysis=stage.key!=="DATA_UPDATE";if(old&&!forceFreshAnalysis&&["PASS","SKIP","REPAIR"].includes(String(old.status||""))){const row={RunID:run.run_id,TargetDate:run.target_date,Mode:run.mode,Ordinal:stage.ordinal,Stage:stage.key,Status:"SKIP",Reason:`checkpoint ${old.status} を再利用`,StartedAt:old.started_at||"",FinishedAt:old.finished_at||"",UpdatedAt:pipelineIsoNow(),Detail:JSON.stringify(old.detail||{})};latestDailyPipelineDiagRows=latestDailyPipelineDiagRows.filter(x=>x.Stage!==stage.key);latestDailyPipelineDiagRows.push(row);renderDailyPipeline(run,latestDailyPipelineDiagRows);return old.detail||{}}
