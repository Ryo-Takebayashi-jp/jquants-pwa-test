...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=v9.8").catch(()=>{}));
...
const SCREENING_CONFIG_KEY="jq_screening_config_v4";
const SCREENING_PROFILE_KEY="jq_screening_named_profiles_v2";
...
const SCREENING_SECTORS_17=["食品","エネルギー資源","建設・資材","素材・化学","医薬品","自動車・輸送機","鉄鋼・非鉄","機械","電機・精密","情報通信・サービスその他","電力・ガス","運輸・物流","商社・卸売","小売","銀行","金融（除く銀行）","不動産"];
const SCREENING_AXIS_DEFS={
 FundamentalPriceGap:[
  {key:"gap",label:"業績と株価の乖離",desc:"業績評価に対して株価評価がどれだけ遅れているか",base:.65},
  {key:"rerating",label:"再評価の兆し",desc:"5日値動き・出来高・25日線位置から見直しの動きを評価",base:.35}
 ],
 FundamentalMomentum:[
  {key:"fundamental",label:"業績の強さ",desc:"売上・利益・利益率・会社予想・営業CFを総合評価",base:.55},
  {key:"rerating",label:"再評価の兆し",desc:"5日値動き・出来高・25日線位置を評価",base:.45}
 ],
 PostEarningsDrift:[
  {key:"fundamental",label:"業績の強さ",desc:"決算内容の土台となる業績総合評価",base:.35},
  {key:"initial",label:"決算直後の反応",desc:"決算後5営業日の株価反応",base:.30},
  {key:"follow",label:"上昇の持続",desc:"初動後も値動きが続いているか",base:.20},
  {key:"volume",label:"出来高の増加",desc:"直近5日と20日平均の出来高を比較",base:.15}
 ],
 GrowthAcceleration:[
  {key:"sales",label:"売上成長",desc:"直近実績の売上高YoYを母集団内で相対評価",base:.25},
  {key:"profit",label:"利益成長",desc:"直近実績の主要利益YoYを母集団内で相対評価",base:.30},
  {key:"margin",label:"利益率改善",desc:"営業利益率などの改善度を相対評価",base:.20},
  {key:"guidance",label:"会社予想",desc:"会社予想の売上・利益成長方向を評価",base:.15},
  {key:"cfo",label:"営業CF",desc:"営業CFが黒字か赤字かを評価",base:.10}
 ],
 QualityValueReRating:[
  {key:"quality",label:"企業の質",desc:"ROE・利益率・CF・自己資本比率など",base:.36},
  {key:"value",label:"割安性",desc:"同じ33業種内の予想PER・PBR・配当利回りを相対比較",base:.28},
  {key:"rerating",label:"再評価の兆し",desc:"TOPIX相対・移動平均・MACD・出来高・RSIなど",base:.36}
 ]
};
const SCREENING_AXIS_MULT={weak:.60,medium:1,strong:1.40};
const SCREENING_DEFAULT_STRATEGIES=Object.fromEntries(SCREENING_STRATEGY_NAMES.map(k=>[k,{enabled:true,topN:20,axes:Object.fromEntries((SCREENING_AXIS_DEFS[k]||[]).map(a=>[a.key,"medium"]))}]));
...
function normalizeScreeningConfig(x={}){const markets=Array.isArray(x.markets)?x.markets.filter(v=>SCREENING_DEFAULT_CONFIG.markets.includes(String(v))):[...SCREENING_DEFAULT_CONFIG.markets],src=x.strategies&&typeof x.strategies==="object"?x.strategies:{},strategies={};for(const k of SCREENING_STRATEGY_NAMES){const z=src[k]||{},axes={};for(const a of SCREENING_AXIS_DEFS[k]||[]){const v=z.axes?.[a.key];axes[a.key]=["weak","medium","strong"].includes(v)?v:"medium"}strategies[k]={enabled:z.enabled!==false,topN:20,axes}}return{markets:markets.length?markets:[...SCREENING_DEFAULT_CONFIG.markets],minTradingValue:SCREENING_DEFAULT_CONFIG.minTradingValue,minPrice:SCREENING_DEFAULT_CONFIG.minPrice,minHistoryDays:SCREENING_DEFAULT_CONFIG.minHistoryDays,sectorMode:"17",sectors:Array.isArray(x.sectors)?[...new Set(x.sectors.map(String).filter(v=>SCREENING_SECTORS_17.includes(v)))]:[],strategies}}
...
function axisLevelLabel(v){return v==="weak"?"弱":v==="strong"?"強":"中"}
function screeningConfigLabel(c=loadScreeningConfig()){c=normalizeScreeningConfig(c);const ms=c.markets.length===3?"全市場":c.markets.join("・"),sec=c.sectors.length?`${c.sectors.length}業種選択`:"全セクター",jp={FundamentalPriceGap:"出遅れ再評価",FundamentalMomentum:"業績モメンタム",PostEarningsDrift:"決算後",GrowthAcceleration:"業績加速",QualityValueReRating:"割安・リレーティング"};const on=SCREENING_STRATEGY_NAMES.filter(k=>c.strategies[k]?.enabled);return `${ms} / ${sec}\n${on.map(k=>`${jp[k]}：${(SCREENING_AXIS_DEFS[k]||[]).map(a=>`${a.label}${axisLevelLabel(c.strategies[k].axes[a.key])}`).join("・")}`).join("\n")||"探索戦略なし"}`}
function renderScreeningStrategyAxes(cfg=loadScreeningConfig()){
 cfg=normalizeScreeningConfig(cfg);
 document.querySelectorAll('#screenCfgStrategies .screen-strategy-card').forEach(card=>{
  const k=card.dataset.strategy,z=cfg.strategies[k],host=card.querySelector('.screen-axis-list');
  if(!host)return;
  if(!host.querySelector('.screen-axis-row')){
   host.innerHTML=(SCREENING_AXIS_DEFS[k]||[]).map(a=>`<div class="screen-axis-row"><div class="screen-axis-name"><b>${escapeHtml(a.label)}</b><small>${escapeHtml(a.desc)}</small></div><div class="screen-axis-choice">${[["weak","弱"],["medium","中"],["strong","強"]].map(([v,l])=>`<label><input type="radio" name="screenAxis_${k}_${a.key}" data-axis="${a.key}" value="${v}"><span>${l}</span></label>`).join("")}</div></div>`).join("");
  }
  for(const a of SCREENING_AXIS_DEFS[k]||[]){const r=card.querySelector(`input[data-axis="${a.key}"][value="${z.axes[a.key]||"medium"}"]`);if(r)r.checked=true}
  const e=card.querySelector('.screen-strategy-enabled');if(e)e.checked=z.enabled;card.classList.toggle('disabled',!z.enabled);
 });
}
function readScreeningConfigUi(){const markets=[...document.querySelectorAll('#screenCfgMarkets input[type="checkbox"]:checked')].map(x=>x.value),checked=[...document.querySelectorAll('#screenCfgSectorList input[type="checkbox"]:checked')].map(x=>x.value),all=checked.length===SCREENING_SECTORS_17.length,strategies={};document.querySelectorAll('#screenCfgStrategies .screen-strategy-card').forEach(card=>{const k=card.dataset.strategy,axes={};for(const a of SCREENING_AXIS_DEFS[k]||[])axes[a.key]=card.querySelector(`input[data-axis="${a.key}"]:checked`)?.value||"medium";strategies[k]={enabled:card.querySelector('.screen-strategy-enabled')?.checked!==false,topN:20,axes}});return normalizeScreeningConfig({markets,sectors:all?[]:checked,strategies})}
function renderScreeningSectorChoices(cfg=loadScreeningConfig()){
 cfg=normalizeScreeningConfig(cfg);const host=$("screenCfgSectorList");if(!host)return;
 if(!host.querySelector('input[type="checkbox"]'))host.innerHTML=SCREENING_SECTORS_17.map(v=>`<label><input type="checkbox" value="${escapeHtml(v)}"> ${escapeHtml(v)}</label>`).join("");
 const selected=new Set(cfg.sectors.length?cfg.sectors:SCREENING_SECTORS_17);
 host.querySelectorAll('input[type="checkbox"]').forEach(x=>{x.checked=selected.has(x.value);x.onchange=()=>{const n=host.querySelectorAll('input:checked').length;if($("screenCfgAllSectors"))$("screenCfgAllSectors").checked=n===SCREENING_SECTORS_17.length}});
 if($("screenCfgAllSectors"))$("screenCfgAllSectors").checked=selected.size===SCREENING_SECTORS_17.length;
}
function applyScreeningConfigUi(cfg=loadScreeningConfig()){cfg=normalizeScreeningConfig(cfg);document.querySelectorAll('#screenCfgMarkets input[type="checkbox"]').forEach(x=>x.checked=cfg.markets.includes(x.value));renderScreeningSectorChoices(cfg);renderScreeningStrategyAxes(cfg);renderScreeningNamedProfiles();if($("screenCfgResult"))box("screenCfgResult",screeningConfigSig(cfg)===screeningConfigSig(SCREENING_DEFAULT_CONFIG)?"pass":"warn",`${screeningConfigSig(cfg)===screeningConfigSig(SCREENING_DEFAULT_CONFIG)?"標準設定":"カスタム設定"}\n${screeningConfigLabel(cfg)}`)}
async function initScreeningConfigUi(){if(!$("screenCfgResult"))return;applyScreeningConfigUi(loadScreeningConfig())}
...
function renderScreeningNamedProfiles(selectValue=null){const sel=$("screenProfileSelect"),ps=loadNamedScreeningProfiles();if(!sel)return;const cur=selectValue??sel.value??"default";sel.innerHTML='<option value="default">標準設定</option>'+ps.map((p,i)=>`<option value="user:${i}">${escapeHtml(p.name)}</option>`).join('');if([...sel.options].some(o=>o.value===cur))sel.value=cur;else sel.value="default";if($("screenProfileLoad"))$("screenProfileLoad").disabled=false;if($("screenProfileDelete"))$("screenProfileDelete").disabled=sel.value==="default"}
if($("screenProfileSelect"))$("screenProfileSelect").onchange=()=>{if($("screenProfileDelete"))$("screenProfileDelete").disabled=$("screenProfileSelect").value==="default"};
if($("screenProfileSave"))$("screenProfileSave").onclick=()=>{const ps=loadNamedScreeningProfiles(),name=$("screenProfileName")?.value.trim();if(!name){box("screenCfgResult","warn","保存する設定名を入力してください。");return}if(name==="標準設定"){box("screenCfgResult","warn","「標準設定」は固定のため上書きできません。");return}const hit=ps.findIndex(p=>p.name===name),item={name,config:readScreeningConfigUi()};let idx=hit;if(hit>=0)ps[hit]=item;else if(ps.length<3){ps.push(item);idx=ps.length-1}else{box("screenCfgResult","warn","保存できる設定は3件までです。不要な設定を削除してから保存してください。");return}saveNamedScreeningProfiles(ps);renderScreeningNamedProfiles(`user:${idx}`);if($("screenProfileName"))$("screenProfileName").value="";box("screenCfgResult","pass",`「${name}」を保存しました。`)};
if($("screenProfileLoad"))$("screenProfileLoad").onclick=()=>{const v=$("screenProfileSelect")?.value||"default";if(v==="default"){const c=normalizeScreeningConfig(SCREENING_DEFAULT_CONFIG);localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(c));applyScreeningConfigUi(c);renderScreeningNamedProfiles("default");box("screenCfgResult","pass","標準設定を読み込みました。");return}const i=Number(v.split(':')[1]),p=loadNamedScreeningProfiles()[i];if(!p)return;localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(p.config));latestScreeningBaseRows=[];latestScreeningBaseAsOf="";latestScreeningBaseProfileSig="";applyScreeningConfigUi(p.config);renderScreeningNamedProfiles(v);box("screenCfgResult","pass",`「${p.name}」を読み込みました。`)};
if($("screenProfileDelete"))$("screenProfileDelete").onclick=()=>{const v=$("screenProfileSelect")?.value||"default";if(v==="default"){box("screenCfgResult","warn","標準設定は削除できません。");return}const i=Number(v.split(':')[1]),ps=loadNamedScreeningProfiles();if(!Number.isInteger(i)||!ps[i])return;const name=ps[i].name;ps.splice(i,1);saveNamedScreeningProfiles(ps);renderScreeningNamedProfiles("default");box("screenCfgResult","pass",`「${name}」を削除しました。`)};
if($("screenCfgAllSectors"))$("screenCfgAllSectors").onchange=()=>{document.querySelectorAll('#screenCfgSectorList input[type="checkbox"]').forEach(x=>x.checked=$("screenCfgAllSectors").checked)};
...
if($("screenCfgSaveBtn"))$("screenCfgSaveBtn").onclick=()=>{const c=readScreeningConfigUi();if(!c.markets.length){box("screenCfgResult","warn","対象市場を1つ以上選択してください。");return}if(c.sectors.length===0&&document.querySelectorAll('#screenCfgSectorList input:checked').length===0){box("screenCfgResult","warn","対象セクターを1つ以上選択してください。");return}if(!SCREENING_STRATEGY_NAMES.some(k=>c.strategies[k]?.enabled)){box("screenCfgResult","warn","探索戦略を1つ以上『使用』にしてください。");return}localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(c));latestScreeningBaseRows=[];latestScreeningBaseAsOf="";latestScreeningBaseProfileSig="";applyScreeningConfigUi(c);box("screenCfgResult","pass",`設定を保存しました。次回Screeningから適用します。\n${screeningConfigLabel(c)}`)};
if($("screenCfgResetBtn"))$("screenCfgResetBtn").onclick=()=>{const c=normalizeScreeningConfig(SCREENING_DEFAULT_CONFIG);localStorage.setItem(SCREENING_CONFIG_KEY,JSON.stringify(c));latestScreeningBaseRows=[];latestScreeningBaseAsOf="";latestScreeningBaseProfileSig="";applyScreeningConfigUi(c);renderScreeningNamedProfiles("default");box("screenCfgResult","pass",`標準設定に戻しました。\n${screeningConfigLabel(c)}`)};
function bootScreeningConfigUi(){try{initScreeningConfigUi()}catch(e){console.error("screening-config-init",e);if($("screenCfgResult"))box("screenCfgResult","fail","Screening設定画面の初期化に失敗しました。\n"+(e?.message||e))}}
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",bootScreeningConfigUi,{once:true});else queueMicrotask(bootScreeningConfigUi);
window.addEventListener("pageshow",()=>{try{applyScreeningConfigUi(loadScreeningConfig())}catch(e){console.error("screening-config-pageshow",e)}});
...
function buildScreeningStrategies(rows){
 ...
 const axisWeighted=(strategy,vals,neutral=50)=>{const z=cfg.strategies?.[strategy]||SCREENING_DEFAULT_STRATEGIES[strategy],defs=SCREENING_AXIS_DEFS[strategy]||[];let total=0,w=0;for(const a of defs){const wt=a.base*(SCREENING_AXIS_MULT[z.axes?.[a.key]||"medium"]||1),v=vals[a.key];total+=(v==null?neutral:Number(v))*wt;w+=wt}return w?total/w:neutral};
 ...
 rr.forEach(r=>{
   ...
   r.QualityValueReRatingScore=r.QVRValueScore==null?null:Math.max(0,Math.min(100,axisWeighted("QualityValueReRating",{quality:r.QVRQualityScore,value:r.QVRValueScore,rerating:r.QVRReRatingScore})-r.QVRCrowdingPenalty-mismatch));
   ...
   r.PostEarningsDriftScore=(r.EarningsElapsedTradingDays!=null&&r.EarningsElapsedTradingDays>=3&&r.EarningsElapsedTradingDays<=20)
     ?axisWeighted("PostEarningsDrift",{fundamental:r.FundamentalScore,initial:eventShape,follow:followScore,volume:(r.VolumePickupScore??50)}):null;
 ...
 rr.forEach(r=>{
   r.GapCompositeScore=axisWeighted("FundamentalPriceGap",{gap:(r.GapPercentile||0),rerating:(r.ReRatingScore||50)})-(r.ChasePenalty||0);
   r.FundamentalMomentumScore=axisWeighted("FundamentalMomentum",{fundamental:r.FundamentalScore,rerating:r.ReRatingScore})-(r.ChasePenalty||0);
   r.GrowthAccelerationScore=axisWeighted("GrowthAcceleration",{sales:r.SalesGrowthScore,profit:r.ProfitGrowthScore,margin:r.MarginQualityScore,guidance:r.GuidanceDirectionScore,cfo:r.CFOQualityScore});
 });
