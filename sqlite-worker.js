...
const d=e.data||{},cmd=d.cmd,name=d.dbName||"/jq_market_v7c.sqlite",t0=performance.now();let db;try{
 ...
 if(cmd==="investment-tracking-preview" || cmd==="investment-tracking-commit"){
   ...
   try{
     const payload=d.payload||{},asOf=String(payload.asOf||"").slice(0,10),discRoutes=Array.isArray(payload.discovery)?payload.discovery:[],watchRoutes=Array.isArray(payload.watchlist)?payload.watchlist:[],commit=cmd==="investment-tracking-commit";
     ...
     for(const rr of discRoutes){const code=norm(rr.Code),start=String(rr.EpisodeStartDate||"").slice(0,10);if(!code||!/^\d{4}-\d{2}-\d{2}$/.test(start)){errors.push(`Discovery route invalid: ${code||"(blank)"} ${start||"(blank)"}`);continue}const ref=refFor(code,start,"");if(!ref)continue;const id=start.replaceAll("-","")+"-"+code,wasExisting=dMap.has(id),old=dMap.get(id)||{},prior=[...dMap.values()].filter(x=>norm(x.Code)===code&&String(x.EpisodeStartDate||x.DiscoveryDate||"")<start).sort((a,b)=>String(a.EpisodeStartDate||a.DiscoveryDate||"").localeCompare(String(b.EpisodeStartDate||b.DiscoveryDate||""))).at(-1),row={...old,...rr,EventID:id,DiscoveryDate:start,Code:code,EpisodeStartDate:start,EpisodePlannedEndDate:plus3(start),InitialPrice:ref.price,InitialPriceLocked:"1",InitialPriceSource:ref.source,PreviousEventID:prior?.EventID||old.PreviousEventID||"",ReferencePriceDate:ref.date,InvestmentTrackingVersion:"InvestmentTrackingWebV1-alpha90"};if(wasExisting)dUpdate++;else dCreate++;dMap.set(id,row);plan.push({RouteType:"Discovery",Action:"UPSERT",Operation:wasExisting?"UPDATE":"CREATE",Code:code,InvestmentStatus:rr.InvestmentStatus||"",StartDate:start,ResolvedPrice:ref.price,ResolvedDate:ref.date,ReferencePriceSource:ref.source,InputReferencePrice:"",ReferencePriceAudit:"",TargetID:id,PreviousID:row.PreviousEventID||"",Warning:""})}
 ...
 if(cmd==="ai-share-stage2"){
   ...
   let pdb=null;try{pdb=new p.OpfsSAHPoolDb("/jq_private_v1.sqlite","r");if(Number(scalarBind(pdb,"SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='portfolio_trade_log'",[])||0)>0)out.tradeHistory=execRows(pdb,"SELECT trade_id,trade_date,code,name,account,action,shares,price,before_shares,before_avg_cost,after_shares,after_avg_cost,realized_pnl,memo,created_at,status,voided_at,void_reason FROM portfolio_trade_log ORDER BY trade_date,created_at");pdb.close();pdb=null}catch(e){try{if(pdb)pdb.close()}catch(__){}out.meta.errors.push(`trade:${String(e?.message||e)}`)}
