...
const d=e.data||{},cmd=d.cmd,name=d.dbName||"/jq_market_v7c.sqlite",t0=performance.now();let db;try{
 ...
 if(cmd==="ai-share-stage2"){
   ...
   readRaw("/jq_short_ratio_v1.sqlite","short_ratio",(r,x)=>{const n=(...ks)=>{for(const k of ks){const v=x[k];if(v!==""&&v!=null&&Number.isFinite(Number(v)))return Number(v)}return null};const ex=n("SellingExcludingShortSellingTurnoverValue","SellExShortValue"),wr=n("ShortSellingWithRestrictionsTurnoverValue","ShortWithRestrictionValue"),nr=n("ShortSellingWithoutRestrictionsTurnoverValue","ShortNoRestrictionValue"),den=(ex??0)+(wr??0)+(nr??0),ratio=den>0?((wr??0)+(nr??0))/den*100:n("ShortRatio","ShortRatioPct","Ratio");out.marketShortRatioHistory.push({Date:String(r.data_date||x.Date||"").slice(0,10),Section:String(x.Section??x.S33??x.Sector33Code??""),SellExShortValue:ex,ShortWithRestrictionValue:wr,ShortNoRestrictionValue:nr,ShortRatioPct:ratio})});
   ...
   self.postMessage({ok:true,type:"result",...out,counts:{priceHistory:out.priceHistory.length,financialHistory:out.financialHistory.length,marginHistory:out.marginHistory.length,marketShortRatioHistory:out.marketShortRatioHistory.length,largeShortHistory:out.largeShortHistory.length,marketFlow:out.marketFlow.length,tradeHistory:out.tradeHistory.length}});return;
 }
