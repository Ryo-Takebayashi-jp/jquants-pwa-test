...
const d=e.data||{},cmd=d.cmd,name=d.dbName||"/jq_market_v7c.sqlite",t0=performance.now();let db;try{
 ...
 if(cmd==="portfolio-large-short-canonical"){
   ...
   const rows=[];for(const code of wanted){const arr=by.get(code)||[],state=new Map();for(const x of arr){const o=x.o,key=[code,o.SSName??"",o.SSAddr??"",o.DICName??"",o.DICAddr??"",o.FundName??""].join("|");const old=state.get(key);if(!old||(x.dd+"|"+x.cd)>=(old.dd+"|"+old.cd))state.set(key,x)}let ratio=0,shares=0,rc=0,sc=0,latestDisc="",latestCalc="",oldestCalc="";for(const x of state.values()){const r=num(x.o,"ShrtPosToSO","ShortPositionRatio"),sh=num(x.o,"ShrtPosShares","ShortPositionShares");if(r!=null){ratio+=r;rc++}if(sh!=null){shares+=sh;sc++}if(x.dd>latestDisc)latestDisc=x.dd;if(x.cd>latestCalc)latestCalc=x.cd;if(x.cd&&(!oldestCalc||x.cd<oldestCalc))oldestCalc=x.cd}const age=oldestCalc?Math.floor((Date.parse(asOf)-Date.parse(oldestCalc))/86400000):null,fresh=age==null?"NoData":age<0?"FutureDateError":age<=7?"Fresh":age<=30?"Recent":"Stale";rows.push({code,largeShortLatestDiscDate:latestDisc||null,largeShortLatestCalcDate:latestCalc||null,largeShortSubjectCount:state.size,largeShortRatioSum:rc?ratio:null,largeShortSharesSum:sc?shares:null,largeShortOldestCalcDate:oldestCalc||null,largeShortOldestAgeDays:age,largeShortAggregateFreshness:fresh})}
 ...
 if(cmd==="ai-share-stage2"){
   ...
   readRaw("/jq_short_ratio_v1.sqlite","short_ratio",(r,x)=>{const n=(...ks)=>{for(const k of ks){const v=x[k];if(v!==""&&v!=null&&Number.isFinite(Number(v)))return Number(v)}return null};
     // J-Quants API v2 /markets/short-ratio uses abbreviated keys: S33, SellExShortVa, ShortWithResVa, ShortWoResVa.
     // Legacy/full-name aliases are retained so already-stored older rows remain readable.
     const ex=n("SellExShortVa","SellingExcludingShortSellingTurnoverValue","LongSellingValue","SellExShortValue"),wr=n("ShortWithResVa","ShortSellingWithRestrictionsTurnoverValue","ShortSellingWithRestrictions","ShortWithRestrictionValue"),nr=n("ShortWoResVa","ShortSellingWithoutRestrictionsTurnoverValue","ShortSellingWithoutRestrictions","ShortNoRestrictionValue"),den=(ex??0)+(wr??0)+(nr??0),ratio=den>0?((wr??0)+(nr??0))/den*100:n("ShortRatio","ShortRatioPct","Ratio");out.marketShortRatioHistory.push({Date:String(r.data_date||x.Date||"").slice(0,10),Section:String(x.S33??x.Sector33Code??x.Section??""),SellExShortValue:ex,ShortWithRestrictionValue:wr,ShortNoRestrictionValue:nr,ShortRatioPct:ratio})});
   ...
   const shortRatioValid=out.marketShortRatioHistory.filter(x=>x.SellExShortValue!=null||x.ShortWithRestrictionValue!=null||x.ShortNoRestrictionValue!=null||x.ShortRatioPct!=null).length;
   self.postMessage({ok:true,type:"result",...out,counts:{priceHistory:out.priceHistory.length,financialHistory:out.financialHistory.length,marginHistory:out.marginHistory.length,marketShortRatioHistory:out.marketShortRatioHistory.length,marketShortRatioValid:shortRatioValid,largeShortHistory:out.largeShortHistory.length,marketFlow:out.marketFlow.length,tradeHistory:out.tradeHistory.length}});return;
 }
