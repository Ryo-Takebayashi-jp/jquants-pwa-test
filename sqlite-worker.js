 ...
 _sqliteInitPromise=(async()=>{
   ...
   for(let attempt=1;attempt<=3&&!mod;attempt++){
     try{
       ...
       mod=await import(`/sqlite/index.mjs?v=v8-beta2-${attempt}`);
     }catch(e){
...
const d=e.data||{},cmd=d.cmd,name=d.dbName||"/jq_market_v7c.sqlite",t0=performance.now();let db;try{
 ...
 if(cmd==="portfolio-trade-list"||cmd==="portfolio-trade-commit"||cmd==="portfolio-trade-void-preview"||cmd==="portfolio-trade-void-commit"||cmd==="portfolio-memo-upsert"){
   let db=null,stage="01-open";try{
     ...
     db.exec(`CREATE TABLE IF NOT EXISTS portfolio_position_memo(code TEXT NOT NULL,account TEXT NOT NULL,memo TEXT NOT NULL DEFAULT '',updated_at TEXT NOT NULL,PRIMARY KEY(code,account)) WITHOUT ROWID`);
     db.exec(`INSERT OR IGNORE INTO portfolio_position_memo(code,account,memo,updated_at) SELECT code,account,COALESCE(memo,''),updated_at FROM user_stocks WHERE TRIM(COALESCE(memo,''))<>''`);
 ...
 if(cmd==="investment-tracking-preview" || cmd==="investment-tracking-commit"){
   ...
   try{
     const payload=d.payload||{},asOf=String(payload.asOf||"").slice(0,10),discRoutes=Array.isArray(payload.discovery)?payload.discovery:[],watchRoutes=Array.isArray(payload.watchlist)?payload.watchlist:[],conditionRows=Array.isArray(payload.conditions)?payload.conditions:[],syncStatusV2=!!payload.syncStatusV2,commit=cmd==="investment-tracking-commit";
     if(!/^\d{4}-\d{2}-\d{2}$/.test(asOf))throw new Error("investment tracking asOf invalid");
     const norm=v=>{let c=String(v??"").trim().toUpperCase();if(c.length===5&&c.endsWith("0"))c=c.slice(0,4);return c},jq=v=>{const c=norm(v);return c.length===4?c+"0":c};
     const isoAdd=(iso,days)=>{const z=new Date(iso+"T00:00:00Z");z.setUTCDate(z.getUTCDate()+days);return z.toISOString().slice(0,10)};
     const plus3=(iso)=>{const z=new Date(iso+"T00:00:00Z"),y=z.getUTCFullYear(),m=z.getUTCMonth(),day=z.getUTCDate(),tm=m+3,ty=y+Math.floor(tm/12),mm=((tm%12)+12)%12,last=new Date(Date.UTC(ty,mm+1,0)).getUTCDate();return `${String(ty).padStart(4,"0")}-${String(mm+1).padStart(2,"0")}-${String(Math.min(day,last)).padStart(2,"0")}`};
     const n=v=>{if(v==null||String(v).trim()==="")return null;const x=Number(v);return Number.isFinite(x)?x:null};
     const activeRouteRows=[...discRoutes.map(x=>({code:norm(x.Code),start:String(x.EpisodeStartDate||"").slice(0,10)})),...watchRoutes.filter(x=>!["REMOVE","CLOSE"].includes(String(x.Action||"").toUpperCase())).map(x=>({code:norm(x.Code),start:String(x.WatchStartDate||"").slice(0,10)}))].filter(x=>x.code&&/^\d{4}-\d{2}-\d{2}$/.test(x.start));
     for(const x of activeRouteRows)if(x.start>asOf)throw new Error(`StartDate is after asOf: ${x.code} ${x.start} > ${asOf}`);
     const need=new Map();for(const x of activeRouteRows)need.set(`${x.code}|${x.start}`,x);
     const priceMap=new Map();
     if(need.size){
       const starts=[...need.values()].map(x=>x.start).sort(),from=starts[0],to=isoAdd(starts.at(-1),14),codes=[...new Set([...need.values()].map(x=>jq(x.code)))];
       cdb=new p.OpfsSAHPoolDb("/jq_catalog_v1.sqlite","r");
       const cats=execRows(cdb,`SELECT shard_key,logical_name,range_start,range_end FROM shard_catalog WHERE dataset='bars_daily' AND state='ready' AND COALESCE(range_end,'9999-12-31')>=? AND COALESCE(range_start,'0000-01-01')<=? ORDER BY range_start,shard_key`,[from,to]);cdb.close();cdb=null;
       const rows=[];
       for(const sh of cats){let bdb=null;try{const nm=String(sh.logical_name||"");bdb=new p.OpfsSAHPoolDb(nm.startsWith("/")?nm:"/"+nm,"r");opened.push(bdb);for(let off=0;off<codes.length;off+=400){const chunk=codes.slice(off,off+400),ph=chunk.map(()=>"?").join(",");if(!ph)continue;for(const r of execRows(bdb,`SELECT code,date,c FROM bars_daily WHERE date>=? AND date<=? AND code IN (${ph}) AND c IS NOT NULL ORDER BY code,date`,[from,to,...chunk])){const code=norm(r.code),dt=String(r.date||"").slice(0,10),cl=n(r.c);if(code&&dt&&cl!=null)rows.push({code,date:dt,close:cl})}}bdb.close();opened.pop()}catch(_){try{if(bdb)bdb.close()}catch(__){}}}
       const by=new Map();for(const r of rows){if(!by.has(r.code))by.set(r.code,[]);by.get(r.code).push(r)}for(const a of by.values())a.sort((x,y)=>x.date.localeCompare(y.date));
       for(const x of need.values()){const arr=by.get(x.code)||[],r=arr.find(z=>z.date>=x.start);if(r)priceMap.set(`${x.code}|${x.start}`,{price:r.close,date:r.date,source:r.date===x.start?"WebDataLakeStartDateClose":"WebDataLakeNextTradingDateClose"})}
     }
     pdb=new p.OpfsSAHPoolDb("/jq_private_v1.sqlite","c");
     pdb.exec(`CREATE TABLE IF NOT EXISTS discovery_episode_master(event_id TEXT PRIMARY KEY,code TEXT NOT NULL,episode_start_date TEXT NOT NULL,row_json TEXT NOT NULL,imported_at TEXT NOT NULL,updated_at TEXT NOT NULL) WITHOUT ROWID`);
     pdb.exec(`CREATE TABLE IF NOT EXISTS watchlist_master_web(watch_id TEXT PRIMARY KEY,row_json TEXT NOT NULL,updated_at TEXT NOT NULL) WITHOUT ROWID`);
     pdb.exec(`CREATE TABLE IF NOT EXISTS watchlist_state_web(watch_id TEXT PRIMARY KEY,row_json TEXT NOT NULL,updated_at TEXT NOT NULL) WITHOUT ROWID`);
     pdb.exec(`CREATE TABLE IF NOT EXISTS investment_tracking_import_history(import_id TEXT PRIMARY KEY,as_of TEXT NOT NULL,applied_at TEXT NOT NULL,before_json TEXT NOT NULL,plan_json TEXT NOT NULL,result_json TEXT NOT NULL,after_json TEXT NOT NULL)`);
     const parseJson=x=>{try{return JSON.parse(String(x.row_json||"{}"))}catch(_){return{}}};
     const dRows=execRows(pdb,"SELECT event_id,code,episode_start_date,row_json FROM discovery_episode_master ORDER BY episode_start_date,event_id").map(x=>({...parseJson(x),EventID:parseJson(x).EventID||String(x.event_id||""),Code:parseJson(x).Code||norm(x.code),EpisodeStartDate:parseJson(x).EpisodeStartDate||String(x.episode_start_date||"")})),wMaster=execRows(pdb,"SELECT watch_id,row_json FROM watchlist_master_web ORDER BY watch_id").map(x=>({...parseJson(x),WatchID:parseJson(x).WatchID||String(x.watch_id||"")})),wState=execRows(pdb,"SELECT watch_id,row_json FROM watchlist_state_web ORDER BY watch_id").map(x=>({...parseJson(x),WatchID:parseJson(x).WatchID||String(x.watch_id||"")}));
     const dMap=new Map(dRows.map(r=>[String(r.EventID||""),r])),wMap=new Map(wMaster.map(r=>[String(r.WatchID||""),r])),sMap=new Map(wState.map(r=>[String(r.WatchID||""),r])),plan=[],warnings=[],errors=[];let dCreate=0,dUpdate=0,wCreate=0,wUpdate=0,wClose=0,wReregister=0,resolved=0;
     const refFor=(code,start,input)=>{const z=priceMap.get(`${code}|${start}`);if(!z){errors.push(`ReferencePrice unresolved: ${code} StartDate=${start}`);return null}resolved++;const iv=n(input);let audit="InputBlank";if(iv!=null){const diff=(iv/z.price-1)*100;audit=Math.abs(diff)<0.01?"Match":`Mismatch input=${iv} web=${z.price} diffPct=${diff.toFixed(4)}`};return {...z,input:input??"",audit}};
     if(syncStatusV2){for(const cr of conditionRows){const code=norm(cr.Code),status=String(cr.InvestmentStatus||"").toUpperCase();if(status==="TRACK_ONLY"){for(const x of [...wMap.values()].filter(x=>norm(x.Code)===code&&["Active","ReviewDue"].includes(String(x.Status||"")))){x.Status="Closed";x.ClosedAt=asOf;x.CloseReason="WatchlistConditionV2:TRACK_ONLY";wMap.set(String(x.WatchID),x);wClose++;plan.push({RouteType:"Watchlist",Action:"SYNC",Operation:"CLOSE",Code:code,InvestmentStatus:status,StartDate:"",ResolvedPrice:x.ReferencePrice||"",ResolvedDate:x.ReferencePriceDate||"",ReferencePriceSource:x.ReferencePriceSource||"",InputReferencePrice:"",ReferencePriceAudit:"",TargetID:x.WatchID||"",PreviousID:"",Warning:"Status sync"})}}if(status==="WATCH_ONLY"){const activeDFor=[...dMap.values()].filter(x=>norm(x.Code)===code&&String(x.PerfEpisodeStatus||"").toLowerCase()!=="closed"&&String(x.PerfActiveWatchFlag||"")!=="0");for(const x of activeDFor){x.PerfActiveWatchFlag="0";x.PerfEpisodeStatus="Closed";x.PerfEpisodeEndDate=asOf;x.PerfEpisodeEndReason="WatchlistConditionV2:WATCH_ONLY";dMap.set(String(x.EventID),x);dUpdate++;plan.push({RouteType:"Discovery",Action:"SYNC",Operation:"CLOSE",Code:code,InvestmentStatus:status,StartDate:String(x.EpisodeStartDate||""),ResolvedPrice:x.InitialPrice||"",ResolvedDate:x.ReferencePriceDate||"",ReferencePriceSource:x.InitialPriceSource||"",InputReferencePrice:"",ReferencePriceAudit:"",TargetID:x.EventID||"",PreviousID:x.PreviousEventID||"",Warning:"Status sync"})}}}}
     for(const rr of discRoutes){const code=norm(rr.Code),requestedStart=String(rr.EpisodeStartDate||"").slice(0,10);if(!code||!/^\d{4}-\d{2}-\d{2}$/.test(requestedStart)){errors.push(`Discovery route invalid: ${code||"(blank)"} ${requestedStart||"(blank)"}`);continue}const requestedId=String(rr.EventID||rr.DiscoveryEpisodeId||"").trim(),activeExisting=[...dMap.values()].filter(x=>norm(x.Code)===code&&String(x.PerfEpisodeStatus||"").toLowerCase()!=="closed"&&String(x.PerfActiveWatchFlag||"")!=="0").sort((a,b)=>String(a.EpisodeStartDate||a.DiscoveryDate||"").localeCompare(String(b.EpisodeStartDate||b.DiscoveryDate||""))).at(-1),existing=(requestedId&&dMap.get(requestedId))||(String(rr.PreferExistingEpisode||"")==="1"?activeExisting:null),start=String(existing?.EpisodeStartDate||existing?.DiscoveryDate||requestedStart).slice(0,10),id=String(existing?.EventID||requestedId||start.replaceAll("-","")+"-"+code),wasExisting=!!existing||dMap.has(id),old=existing||dMap.get(id)||{};let ref=old.InitialPrice!=null?{price:n(old.InitialPrice),date:old.ReferencePriceDate||start,source:old.InitialPriceSource||"ExistingEpisode"}:refFor(code,start,"");if(!ref||ref.price==null)continue;const prior=[...dMap.values()].filter(x=>norm(x.Code)===code&&String(x.EpisodeStartDate||x.DiscoveryDate||"")<start).sort((a,b)=>String(a.EpisodeStartDate||a.DiscoveryDate||"").localeCompare(String(b.EpisodeStartDate||b.DiscoveryDate||""))).at(-1),row={...old,...rr,EventID:id,DiscoveryDate:start,Code:code,EpisodeStartDate:start,EpisodePlannedEndDate:old.EpisodePlannedEndDate||plus3(start),InitialPrice:ref.price,InitialPriceLocked:"1",InitialPriceSource:ref.source,PreviousEventID:prior?.EventID||old.PreviousEventID||"",ReferencePriceDate:ref.date,InvestmentTrackingVersion:"InvestmentTrackingWebV2-beta2"};if(wasExisting)dUpdate++;else dCreate++;dMap.set(id,row);plan.push({RouteType:"Discovery",Action:"UPSERT",Operation:wasExisting?"UPDATE":"CREATE",Code:code,InvestmentStatus:rr.InvestmentStatus||"",StartDate:start,ResolvedPrice:ref.price,ResolvedDate:ref.date,ReferencePriceSource:ref.source,InputReferencePrice:"",ReferencePriceAudit:"",TargetID:id,PreviousID:row.PreviousEventID||"",Warning:""})}
     const activeFor=code=>[...wMap.values()].filter(x=>norm(x.Code)===code&&["Active","ReviewDue"].includes(String(x.Status||""))).sort((a,b)=>String(a.RegisteredAt||a.WatchStartDate||"").localeCompare(String(b.RegisteredAt||b.WatchStartDate||"")));
     const uniqueWatchId=(start,code)=>{const base=`W-${start.replaceAll("-","")}-${code}`;if(!wMap.has(base))return base;let i=2;while(wMap.has(`${base}-${i}`))i++;return `${base}-${i}`};
     const blankState=(id,code)=>({WatchID:id,Code:code,LastEvaluatedDate:"",LastPriceAlertLevel:"",LastPERTriggered:"",LastPBRTriggered:"",LastYieldTriggered:"",LastTechnicalState:"",LastFactorState:"",LastSeasonalAlert:"",LastDisclosureDate:"",LastFinancialFingerprint:"",LastCatalystLevel:"",LastExpiryAlerted:"",LastAlertDatePrice:"",LastAlertDateValuation:"",LastAlertDateFundamental:"",LastAlertDateFactor:"",LastAlertDateTechnical:"",LastAlertDateCatalyst:"",LastAlertDateExpiry:"",LastDipTriggered:"",LastBreakoutTriggered:""});
     for(const rr of watchRoutes){const code=norm(rr.Code),action=String(rr.Action||"").toUpperCase(),active=activeFor(code);if(["REMOVE","CLOSE"].includes(action)){if(!active.length){warnings.push(`Watchlist ${action}: active watch not found for ${code}`);plan.push({RouteType:"Watchlist",Action:action,Operation:"NOOP",Code:code,InvestmentStatus:rr.InvestmentStatus||"",StartDate:"",ResolvedPrice:"",ResolvedDate:"",ReferencePriceSource:"",InputReferencePrice:"",ReferencePriceAudit:"",TargetID:"",PreviousID:"",Warning:"Active watch not found"});continue}for(const x of active){x.Status="Closed";x.ClosedAt=asOf;x.CloseReason=`UnifiedImport:${action}`;wMap.set(String(x.WatchID),x);wClose++;plan.push({RouteType:"Watchlist",Action:action,Operation:"CLOSE",Code:code,InvestmentStatus:rr.InvestmentStatus||x.InvestmentStatus||"",StartDate:"",ResolvedPrice:x.ReferencePrice||"",ResolvedDate:x.ReferencePriceDate||"",ReferencePriceSource:x.ReferencePriceSource||"",InputReferencePrice:x.InputReferencePrice||"",ReferencePriceAudit:x.ReferencePriceAudit||"",TargetID:x.WatchID||"",PreviousID:"",Warning:""})}continue}
       const start=String(rr.WatchStartDate||"").slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(start)){errors.push(`Watchlist route invalid StartDate: ${code} ${start||"(blank)"}`);continue}const ref=refFor(code,start,rr.InputReferencePrice??rr.ReferencePrice);if(!ref)continue;let target=null,operation="";
       if(action==="REREGISTER"){for(const x of active){x.Status="Closed";x.ClosedAt=asOf;x.CloseReason="UnifiedImport:REREGISTER";wMap.set(String(x.WatchID),x);wClose++}const id=uniqueWatchId(start,code);target={WatchID:id,Status:"Active",Code:code,RegisteredAt:new Date().toISOString(),ClosedAt:"",CloseReason:"",WatchlistVersion:"WatchlistWebV3-alpha90"};sMap.set(id,blankState(id,code));wCreate++;wReregister++;operation="REREGISTER_CREATE"}
       else if(active.length){target=active.at(-1);wUpdate++;operation="UPDATE"}
       else{const id=uniqueWatchId(start,code);target={WatchID:id,Status:"Active",Code:code,RegisteredAt:new Date().toISOString(),ClosedAt:"",CloseReason:"",WatchlistVersion:"WatchlistWebV3-alpha90"};sMap.set(id,blankState(id,code));wCreate++;operation="CREATE"}
       target={...target,...rr,WatchID:target.WatchID,Status:"Active",Code:code,WatchStartDate:start,ReferencePrice:ref.price,InputReferencePrice:rr.InputReferencePrice??rr.ReferencePrice??"",ReferencePriceDate:ref.date,ReferencePriceSource:ref.source,ReferencePriceAudit:ref.audit,ClosedAt:"",CloseReason:"",WatchlistVersion:"WatchlistWebV3-alpha90"};wMap.set(String(target.WatchID),target);if(!sMap.has(String(target.WatchID)))sMap.set(String(target.WatchID),blankState(String(target.WatchID),code));plan.push({RouteType:"Watchlist",Action:action,Operation:operation,Code:code,InvestmentStatus:rr.InvestmentStatus||"",StartDate:start,ResolvedPrice:ref.price,ResolvedDate:ref.date,ReferencePriceSource:ref.source,InputReferencePrice:rr.InputReferencePrice??rr.ReferencePrice??"",ReferencePriceAudit:ref.audit,TargetID:target.WatchID||"",PreviousID:active.length?active.at(-1)?.WatchID||"":"",Warning:action==="ADD"&&active.length?"ADD normalized to UPDATE existing active watch":""});if(action==="ADD"&&active.length)warnings.push(`Code=${code}: ADD normalized to UPDATE existing active watch`)}
     const nextD=[...dMap.values()],nextM=[...wMap.values()],nextS=[...sMap.values()],commitReady=errors.length===0;
     const result={asOf,commitReady,discoveryCreate:dCreate,discoveryUpdate:dUpdate,watchCreate:wCreate,watchUpdate:wUpdate,watchClose:wClose,watchReregister:wReregister,referenceResolved:resolved,warningCount:warnings.length,errorCount:errors.length,warnings,errors,plan,discovery:nextD,master:nextM,state:nextS};
     if(commit){if(!commitReady)throw new Error("Investment Tracking commit blocked: "+errors.join(" | "));const now=new Date().toISOString(),ds=pdb.prepare(`INSERT INTO discovery_episode_master(event_id,code,episode_start_date,row_json,imported_at,updated_at) VALUES(?,?,?,?,?,?) ON CONFLICT(event_id) DO UPDATE SET code=excluded.code,episode_start_date=excluded.episode_start_date,row_json=excluded.row_json,updated_at=excluded.updated_at`),ms=pdb.prepare(`INSERT OR REPLACE INTO watchlist_master_web(watch_id,row_json,updated_at) VALUES(?,?,?)`),ss=pdb.prepare(`INSERT OR REPLACE INTO watchlist_state_web(watch_id,row_json,updated_at) VALUES(?,?,?)`);const importId=`${asOf.replaceAll("-","")}-${Date.now()}`;try{pdb.exec("BEGIN IMMEDIATE");for(const r of nextD){const id=String(r.EventID||"");if(!id)continue;ds.bind([id,norm(r.Code),String(r.EpisodeStartDate||r.DiscoveryDate||"").slice(0,10),JSON.stringify(r),now,now]);ds.step();ds.reset()}pdb.exec("DELETE FROM watchlist_master_web");pdb.exec("DELETE FROM watchlist_state_web");for(const r of nextM){const id=String(r.WatchID||"");if(!id)continue;ms.bind([id,JSON.stringify(r),now]);ms.step();ms.reset()}for(const r of nextS){const id=String(r.WatchID||"");if(!id)continue;ss.bind([id,JSON.stringify(r),now]);ss.step();ss.reset()}pdb.exec({sql:`INSERT INTO investment_tracking_import_history(import_id,as_of,applied_at,before_json,plan_json,result_json,after_json) VALUES(?,?,?,?,?,?,?)`,bind:[importId,asOf,now,JSON.stringify({discovery:dRows,master:wMaster,state:wState}),JSON.stringify(plan),JSON.stringify({discoveryCreate:dCreate,discoveryUpdate:dUpdate,watchCreate:wCreate,watchUpdate:wUpdate,watchClose:wClose,watchReregister:wReregister,warnings}),JSON.stringify({discovery:nextD,master:nextM,state:nextS})]});pdb.exec("COMMIT");result.importId=importId;result.committed=true}catch(err){try{pdb.exec("ROLLBACK")}catch(_){}throw err}finally{for(const st of [ds,ms,ss])try{st.finalize()}catch(_){}}}
     pdb.close();pdb=null;self.postMessage({ok:true,type:"result",...result,elapsedMs:Math.round(performance.now()-t0)});return;
   }catch(err){try{if(pdb)pdb.close()}catch(_){}try{if(cdb)cdb.close()}catch(_){}for(const x of opened)try{x.close()}catch(_){}self.postMessage({ok:false,type:"error",stage:"investment-tracking",message:String(err?.message||err),stack:String(err?.stack||""),elapsedMs:Math.round(performance.now()-t0)});return}
 }
 ...
 if(cmd==="financial-normalize-latest"){
   ...
   try{
     ...
     for(const [code,hist] of byCode){
       ...
       const actualFieldKeys=["Sales","OP","OdP","NP","NCSales","NCOP","NCOdP","NCNP"];
       const isActualFY=x=>s(x,"CurPerType").toUpperCase()==="FY"&&disc(x)&&actualFieldKeys.some(k=>n(x,k)!=null);
       const fyRows=hist.filter(isActualFY);
       // For actual FY chronology, prefer the reported period end.  Some V2 rows also carry
       // CurFYEn as the post-close/current fiscal-year boundary, so retain both raw ends for
       // diagnosis and use a dual-field resolver below rather than assuming one alias forever.
       fyRows.sort((a,b)=>(s(a,"CurPerEn","CurFYEn")+"|"+disc(a)).localeCompare(s(b,"CurPerEn","CurFYEn")+"|"+disc(b)));
       const latestFY=fyRows.at(-1)||null;
       let prevFY=null,pfbest=1e99,prevFYResolver="",prevFYMatchedEnd="",prevFYCandidateCount=0;
       const targetFY=expectedFY||forecastFY(fc||{});
       const fyTrace=[];
       if(targetFY){
         const td=Date.parse(targetFY);
         for(const x of fyRows){
           const [xp]=primary(x),ends=[["CurFYEn",s(x,"CurFYEn")],["CurPerEn",s(x,"CurPerEn")]].filter((v,i,a)=>v[1]&&a.findIndex(z=>z[1]===v[1])===i);
           const trace={disc:disc(x),curFY:s(x,"CurFYEn"),curPer:s(x,"CurPerEn"),profit:xp};
           fyTrace.push(trace);
           for(const [src,fe] of ends){
             const dd=(td-Date.parse(fe))/86400000;
             if(dd<300||dd>430)continue;
             prevFYCandidateCount++;
       ...
       const currentPeriodEPS=n(cur||{},"EPS","NCEPS"),actualFYEPS=n(latestFY||{},"EPS","NCEPS"),forecastEPS=n(fc||{},"FEPS","FNCEPS");
       ...
       rows.push({code,discDate:disc(cur),earningsEventDate,curPerType:curType,ProfitType:ptype,
         ...
         eps:currentPeriodEPS,currentPeriodEPS,actualFYEPS,bps,equity:eq,totalAssets:ta,cfo,cfi,cff,
         ...
         EffectiveShares:effectiveShares,FactorCurrentDisclosureDate:disc(cur),FactorLatestFYDisclosureDate:latestFY?disc(latestFY):"",FactorLatestFYCurFYEnd:latestFY?s(latestFY,"CurFYEn"):"",FactorLatestFYCurPerEnd:latestFY?s(latestFY,"CurPerEn"):"",FactorForecastDisclosureDate:fc?disc(fc):"",FactorForecastFYEnd:fc?forecastFY(fc):"",FactorTargetFYEnd:targetFY||"",FactorPreviousFYDisclosureDate:prevFY?disc(prevFY):"",FactorPreviousFYEnd:prevFYMatchedEnd||s(prevFY||{},"CurFYEn","CurPerEn"),FactorPreviousFYCurFYEnd:prevFY?s(prevFY,"CurFYEn"):"",FactorPreviousFYCurPerEnd:prevFY?s(prevFY,"CurPerEn"):"",FactorPreviousFYPrimaryProfit:prevFYp,FactorPreviousFYResolver:prevFYResolver,FactorPreviousFYCandidateCount:prevFYCandidateCount,FactorFYHistoryTrace:fyTrace.map(x=>`${x.disc}|CurFY=${x.curFY}|CurPer=${x.curPer}|P=${x.profit??""}`).join(" ; "),ActualAnnualDividend:adiv,ForecastAnnualDividend:fdiv,
 ...
 if(cmd==="portfolio-integrated-snapshot"){
   ...
   const rows=stocks.map(st=>{
     ...
     return {code,name:st.name??st.Name??m.company_name??"",account:st.account??st.Account??"",
       ...
       discDate:f.discDate??null,curPerType:f.curPerType??null,sales:f.sales??null,op:f.op??null,np:f.np??null,eps:f.currentPeriodEPS??f.eps??null,currentPeriodEPS:f.currentPeriodEPS??f.eps??null,actualFYEPS:f.actualFYEPS??f.ActualFYEPS??f.ActualEPS??null,
 ...
 if(cmd==="portfolio-large-short-canonical"){
   const payload=d.payload||{},asOf=String(payload.asOf||""),wanted=new Set((payload.codes||[]).map(v=>{let c=String(v??"").trim();if(c.length===5&&c.endsWith("0"))c=c.slice(0,4);return c}).filter(Boolean));
   const norm=v=>{let c=String(v??"").trim();if(c.length===5&&c.endsWith("0"))c=c.slice(0,4);return c};
   ...
   const by=new Map();let db=null;
   try{db=new p.OpfsSAHPoolDb("/jq_short_sale_report_v1.sqlite","r");for(const rr of execRows(db,"SELECT data_date,raw_json FROM short_sale_report ORDER BY data_date")){let o={};try{o=JSON.parse(String(rr.raw_json||"{}"))}catch(_){continue};const code=norm(o.Code??o.code??"");if(!wanted.has(code))continue;const dd=String(o.DiscDate??rr.data_date??"").slice(0,10),cd=String(o.CalcDate??"").slice(0,10);if(!dd||dd>asOf)continue;if(!by.has(code))by.set(code,[]);by.get(code).push({dd,cd,o})}db.close();db=null}catch(_){try{if(db)db.close()}catch(__){}}
   const rows=[];for(const code of wanted){const arr=by.get(code)||[],state=new Map();for(const x of arr){const o=x.o,key=[code,o.SSName??"",o.SSAddr??"",o.DICName??"",o.DICAddr??"",o.FundName??""].join("|");const old=state.get(key);if(!old||(x.dd+"|"+x.cd)>=(old.dd+"|"+old.cd))state.set(key,x)}let ratio=0,shares=0,rc=0,sc=0,latestDisc="",latestCalc="",oldestCalc="";const active=[];for(const x of state.values()){const r=num(x.o,"ShrtPosToSO","ShortPositionRatio"),sh=num(x.o,"ShrtPosShares","ShortPositionShares");/* latest zero-position report is an exit/closure, not an active short subject */if((r==null||r<=0)&&(sh==null||sh<=0))continue;active.push(x);if(r!=null){ratio+=r;rc++}if(sh!=null){shares+=sh;sc++}if(x.dd>latestDisc)latestDisc=x.dd;if(x.cd>latestCalc)latestCalc=x.cd;if(x.cd&&(!oldestCalc||x.cd<oldestCalc))oldestCalc=x.cd}const age=oldestCalc?Math.floor((Date.parse(asOf)-Date.parse(oldestCalc))/86400000):null,fresh=age==null?"NoData":age<0?"FutureDateError":age<=7?"Fresh":age<=30?"Recent":"Stale";rows.push({code,largeShortLatestDiscDate:latestDisc||null,largeShortLatestCalcDate:latestCalc||null,largeShortSubjectCount:active.length,largeShortRatioSum:rc?ratio:null,largeShortSharesSum:sc?shares:null,largeShortOldestCalcDate:oldestCalc||null,largeShortOldestAgeDays:age,largeShortAggregateFreshness:fresh})}
   self.postMessage({ok:true,type:"result",rows,count:rows.length});return;
 }
 ...
 if(cmd==="ai-share-stage2"){
   ...
   const readRaw=(dbName,table,mapper)=>{let db=null;try{db=new p.OpfsSAHPoolDb(dbName,"r");for(const r of execRows(db,`SELECT data_date,code,raw_json FROM ${table} WHERE data_date<=? ORDER BY data_date`,[asOf])){let x={};try{x=JSON.parse(String(r.raw_json||"{}"))}catch(_){};mapper(r,x,norm(x.Code??x.code??r.code??""))}db.close()}catch(e){try{if(db)db.close()}catch(__){}out.meta.errors.push(`${table}:${String(e?.message||e)}`)}};
   readRaw("/jq_margin_interest_v1.sqlite","margin_interest",(r,x,code)=>{if(!wanted.has(code))return;const n=(...ks)=>{for(const k of ks){const v=Number(x[k]);if(x[k]!==""&&x[k]!=null&&Number.isFinite(v))return v}return null},L=n("LongVol","LongMargin","BuyBalance","LongBalance"),S=n("ShrtVol","ShortVol","ShortMargin","SellBalance","ShortBalance");out.marginHistory.push({Code:code,Date:String(r.data_date||x.Date||"").slice(0,10),Long:L,Short:S,Ratio:(L!=null&&S!=null&&S!==0)?L/S:null})});
   // Short-ratio endpoint is market/section-level, not portfolio-code level.
   readRaw("/jq_short_ratio_v1.sqlite","short_ratio",(r,x)=>{const n=(...ks)=>{for(const k of ks){const v=x[k];if(v!==""&&v!=null&&Number.isFinite(Number(v)))return Number(v)}return null};
     // J-Quants API v2 /markets/short-ratio uses abbreviated keys: S33, SellExShortVa, ShrtWithResVa, ShrtNoResVa.
     // Legacy/full-name aliases are retained so already-stored older rows remain readable.
     const ex=n("SellExShortVa","SellingExcludingShortSellingTurnoverValue","LongSellingValue","SellExShortValue"),wr=n("ShrtWithResVa","ShortWithResVa","ShortWithResValue","ShortWithRes","ShortSellingWithRestrictionsTurnoverValue","ShortSellingWithRestrictions","ShortWithRestrictionValue"),nr=n("ShrtNoResVa","ShortWoResVa","ShortWithoutResVa","ShortWoResValue","ShortWoRes","ShortSellingWithoutRestrictionsTurnoverValue","ShortSellingWithoutRestrictions","ShortNoRestrictionValue"),shortTotal=n("ShortVa","ShortValue","ShortSellingTurnoverValue","ShortSellingValue"),apiRatio=n("ShortRatio","ShortRatioPct","Ratio");let ratio=null;if(wr!=null||nr!=null){const den=(ex??0)+(wr??0)+(nr??0);ratio=den>0?((wr??0)+(nr??0))/den*100:apiRatio}else if(shortTotal!=null){const den=(ex??0)+shortTotal;ratio=den>0?shortTotal/den*100:apiRatio}else ratio=apiRatio;out.marketShortRatioHistory.push({Date:String(r.data_date||x.Date||"").slice(0,10),Section:String(x.S33??x.Sector33Code??x.Section??""),SellExShortValue:ex,ShortWithRestrictionValue:wr,ShortNoRestrictionValue:nr,ShortTotalValue:shortTotal,ShortRatioPct:ratio})});
 ...
 if(cmd==="technical-screening-poc"){
   ...
   const lookback=Math.max(60,Math.min(360,Number(payload.lookback||320)));
   ...
   try{
     ...
     if(chosen.length<60)throw new Error(`Need >=60 trading dates, got ${chosen.length}`);
