...
const d=e.data||{},cmd=d.cmd,name=d.dbName||"/jq_market_v7c.sqlite",t0=performance.now();let db;try{
 ...
 if(cmd==="ai-share-stage2"){
   ...
   readRaw("/jq_short_ratio_v1.sqlite","short_ratio",(r,x)=>{const n=(...ks)=>{for(const k of ks){const v=x[k];if(v!==""&&v!=null&&Number.isFinite(Number(v)))return Number(v)}return null};
     ...
     const ex=n("SellExShortVa","SellingExcludingShortSellingTurnoverValue","LongSellingValue","SellExShortValue"),wr=n("ShortWithResVa","ShortWithResValue","ShortWithRes","ShortSellingWithRestrictionsTurnoverValue","ShortSellingWithRestrictions","ShortWithRestrictionValue"),nr=n("ShortWoResVa","ShortWithoutResVa","ShortWoResValue","ShortWoRes","ShortSellingWithoutRestrictionsTurnoverValue","ShortSellingWithoutRestrictions","ShortNoRestrictionValue"),shortTotal=n("ShortVa","ShortValue","ShortSellingTurnoverValue","ShortSellingValue"),apiRatio=n("ShortRatio","ShortRatioPct","Ratio");let ratio=null;if(wr!=null||nr!=null){const den=(ex??0)+(wr??0)+(nr??0);ratio=den>0?((wr??0)+(nr??0))/den*100:apiRatio}else if(shortTotal!=null){const den=(ex??0)+shortTotal;ratio=den>0?shortTotal/den*100:apiRatio}else ratio=apiRatio;out.marketShortRatioHistory.push({Date:String(r.data_date||x.Date||"").slice(0,10),Section:String(x.S33??x.Sector33Code??x.Section??""),SellExShortValue:ex,ShortWithRestrictionValue:wr,ShortNoRestrictionValue:nr,ShortTotalValue:shortTotal,ShortRatioPct:ratio})});
