...
# v7e-alpha90 — Investment Tracking formal Web connection

## Purpose
Promote the unified `investment_tracking_input.csv` from routing audit to a safe Web-first operational input while keeping Discovery and Watchlist as separate lifecycles.

## Behavior
- Audit validates status/action/required fields.
- Preview resolves DataLake reference prices and shows CREATE/UPDATE/CLOSE/REREGISTER operations without changing state.
- Commit applies Discovery and Watchlist changes atomically in `/jq_private_v1.sqlite` and records an import audit history with before/after state snapshots.
- Discovery uses a date+code EventID and retains prior episodes. REMOVE/CLOSE never erase Discovery history.
- Watchlist UPSERT preserves state; REREGISTER closes the prior active watch and creates a fresh WatchID/state.
- Reference prices use unadjusted market close on StartDate, or the next available trading date if StartDate is a non-trading day. Input ReferencePrice is retained only for audit.
- Discovery performance is recalculated after a committed Discovery change.
